...
describe("health server", () => {
  ...
  it("serves the public Kvestarnia home page with latest news and presence links", async () => {
    ...
    expect(text).toContain("Пошта Квестарні взялася носити манатки");
    expect(text).toContain("У Корчмі зʼявився гонець, який погодився переносити невеликі пакунки манаток");
    expect(text).toContain("У грі вже");
    expect(text).toContain("можна відкрити <code>📮 Пошта Квестарні</code>");
    expect(text).toContain("гонець бере видиму плату за дорогу з відправника одразу при відправленні, але це не переказ золота іншому гравцеві");
    expect(text).toContain("у пошті видно пакунки в дорозі й уже прийняту історію");
    expect(text).toContain("Це не ринок, не обмін, не скринька для анонімних сюрпризів");
