...
describe("health server", () => {
  ...
  it("serves the public Kvestarnia home page with latest news and presence links", async () => {
    ...
    expect(text).toContain("Справи пояснюють, чого вони хочуть");
    expect(text).toContain("Стіл зі справами став менше схожий на кошик із дотепними заголовками");
    expect(text).toContain("У грі вже");
    expect(text).toContain("ризикові спроби можуть завершитися локальною невдачею без нагороди й без бійки");
    expect(text).toContain(
