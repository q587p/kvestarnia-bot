...
describe("health server", () => {
  ...
  it("serves the public Kvestarnia home page with latest news and presence links", async () => {
    ...
    expect(text).toContain("Циферки перестали рости шеренгою");
    expect(text).toContain("Корчмар довго дивився на героїчні характеристики");
    expect(text).toContain("Ріст рівнів став рівнішим");
    expect(text).toContain("<code>/hero</code>");
    expect(text).toContain("Людисько стало чеснішим універсалом");
    expect(text).toContain("Ремортова памʼять краще памʼятає");
