...
describe("health server", () => {
  ...
  it("serves the public Kvestarnia home page with latest news and presence links", async () => {
    ...
    expect(text).toContain("Дуель стала миттєвішою, а рукавиця балакучішою");
    expect(text).toContain("Якщо воно рахується одразу, хай чесно так і називається");
    expect(text).toContain("<code>⚡ Миттєва дуель</code>");
    expect(text).toContain("один із 13 текстів");
    expect(text).toContain("<code>🎲 Інший текст</code>");
    expect(text).toContain("Корчмар тимчасово підрівнює досвід");
    expect(text).toContain("Нагород, ставок, втрат, турнірів");
