...
describe("health server", () => {
  ...
  it("serves the public Kvestarnia home page with latest news and presence links", async () => {
    ...
    expect(text).toContain("Бойові підсумки, щедріші обходи й розбір манаток");
    expect(text).toContain("У грі вже:");
    expect(text).toContain("Корчмар завів окрему книгу для бойових цифр");
    expect(text).toContain("після завершення бою поруч із журналом можна відкрити статистику");
    expect(text).toContain("вища Вдача трохи частіше допомагає натрапити");
    expect(text).toContain("у Чароковальні можна переглянути придатні зайві манатки");
    expect(text).toContain("підтвердити розбір");
