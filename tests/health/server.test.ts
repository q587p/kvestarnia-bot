...
describe("health server", () => {
  ...
  it("serves the public Kvestarnia home page with latest news and presence links", async () => {
    ...
    expect(text).toContain("Низ навчився рахувати до двох без зайвої драматургії");
    expect(text).toContain("Коли в бою зʼявляються двоє супротивників, другий тепер тисне розумніше");
    expect(text).toContain("У грі вже");
    expect(text).toContain("бої з двома супротивниками мають мʼякший додатковий тиск");
    expect(text).toContain("кілька різких монстрів у Низу стали чеснішими для звичайних пригодників");
    expect(text).toContain("це бойове підкручування: нагороди, квести, реморт і косметичні титули лишилися при своїх правилах");
