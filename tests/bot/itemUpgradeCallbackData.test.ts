...
describe("item upgrade callback data", () => {
  it("serializes compact list, preview and attempt callbacks", () => {
    ...
    const sortedList = makeItemUpgradeListCallbackData(2, "date-desc");
    const pagePrompt = makeItemUpgradePagePromptCallbackData(9, "name-asc");
    ...
    expect(Buffer.byteLength(sortedList, "utf8")).toBeLessThanOrEqual(TELEGRAM_CALLBACK_DATA_LIMIT);
    expect(Buffer.byteLength(pagePrompt, "utf8")).toBeLessThanOrEqual(TELEGRAM_CALLBACK_DATA_LIMIT);
    ...
    expect(parseItemUpgradeCallbackData(list)).toEqual({
      ok: true,
      value: { type: "list", page: 0, sort: "default" }
    });
    expect(parseItemUpgradeCallbackData(listPage)).toEqual({
      ok: true,
      value: { type: "list", page: 3, sort: "default" }
    });
    expect(parseItemUpgradeCallbackData(sortedList)).toEqual({
      ok: true,
      value: { type: "list", page: 2, sort: "date-desc" }
    });
    expect(parseItemUpgradeCallbackData(pagePrompt)).toEqual({
      ok: true,
      value: { type: "page-prompt", totalPages: 9, sort: "name-asc" }
    });
  ...
  it("rejects malformed or oversized callback data", () => {
    ...
    expect(parseItemUpgradeCallbackData("v1:up:l:r:nope")).toEqual({ ok: false });
    expect(parseItemUpgradeCallbackData("v1:up:page:r:az:0")).toEqual({ ok: false });
