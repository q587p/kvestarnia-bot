...
describe("guild callback data", () => {
  ...
  it("round-trips bounded pagination, party and accepted-transfer callbacks", () => {
    ...
    expect(parseGuildCallbackData(values[1])).toEqual({ ok: true, value: { type: "party-open", page: 42 } });
    ...
    expect(parseGuildCallbackData(values[3])).toEqual({ ok: true, value: { type: "transfer-accept", version: 587 } });
  });
