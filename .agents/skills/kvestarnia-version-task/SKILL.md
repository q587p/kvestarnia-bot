...
13. Before final handoff, re-check the current Kyiv day against the latest release/news/changelog Holocene headings. While the release PR is unmerged or the version is not yet confirmed in production, same-PR follow-ups and review fixes must refresh both changelog and news headings when the branch has a newer Kyiv-day commit. Preserve an existing news date only for a separate follow-up PR after that version is both merged and confirmed in production, unless the player-facing promise changes or the entry is actually republished.
