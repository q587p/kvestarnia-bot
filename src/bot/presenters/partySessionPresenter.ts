  ...
  if (session.status === "recruiting") {
    lines.push("", big
      ...
      : groupCombat
        ...
        : leftPassage
          ? "Це справжня атака 1–3 пригодників на вже помічений гурт. Бій почнеться, щойно всі учасники натиснуть «Готово», або автоматично, коли час добіжить; склад ворогів підлаштується під силу ватаги. Лідер може рушити раніше."
...
function getStatusLine(session: PartySessionRecord): string {
  ...
  if (session.originLocationId === GROUP_COMBAT_PARTY_ORIGIN_LOCATION_ID) {
    ...
    if (session.originKind === LEFT_PASSAGE_PARTY_ORIGIN_KIND) {
      return [
        `Стан: атака почнеться, щойно готові всі 1–3 учасники, або автоматично о ${formatTime(session.expiresAt)}.`,
        "Лідер ватаги також може почати бій раніше."
      ].join("\n");
  }
