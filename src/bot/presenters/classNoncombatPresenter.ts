  ...
  const lines: Array<string | null> = result.actorBlocked
    ? [
        ...
        result.mode === "priest"
          ...
          : result.mode === "varenyk"
            ? "🍽️ Годування: недоступне під час бою або рейду."
...
function presentBlocked(
  ...
  const detail = (() => {
    switch (reason) {
      ...
      case "actor-blocked":
        return icon === "🍽️"
          ? "Спершу завершіть бій або рейд. Миска почекає без образ."
          : "Спершу завершіть бій або рейд. Жрецька поміч не лізе поперед черги.";
      case "target-blocked":
        return icon === "🍽️"
          ? "Ціль зараз у бою або рейді. Вареники не лізуть у чужий хід."
