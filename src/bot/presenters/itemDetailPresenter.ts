...
function presentItemUseLine(
  ...
  const resource = availability.item.useEffect?.kind === "restore-mana" ? "ману" : "HP";

  if (combatUseAvailable) {
    return [
      "Використання: <b>можна застосувати в бою або поза боєм</b>.",
      `У бою манатка витрачає хід і відновлює ${resource} одразу. Поза боєм попередній перегляд покаже результат перед витратою.`
    ];
  }

  return [
    `Використання: <b>можна застосувати, щоб відновити ${resource}</b>.`,
    "Попередній перегляд покаже поточний результат перед витратою."
  ];
}
