...
export function presentQuestHub(snapshot: QuestHubSnapshot, mode: QuestHubMode = "active"): string {
  ...
  const lines = [
    ...
    mode === "archive"
      ...
      : "На столі лежать актуальні справи, а збоку тулиться Бійцівський куток для обережного /spar і першого дружнього /duel. Тут обирають напрям, а далі Стіл уже штовхає туди, де справа насправді шумить.",
    "",
    ...rows,
    "",
