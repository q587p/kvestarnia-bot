...
export function presentKorchmaNewsCorner(_character: CharacterSummary): string {
  ...
  return [
    ...
    "",
    "Тут можна глянути вісти Квестарні, погортати перекази, подарувати манатку тому, хто поруч, або передати пакунок через пошту за невелику плату. Дошка робить вигляд, що це все її ідея.",
    "",
    "Що дивимося?"
  ].join("\n");
}
