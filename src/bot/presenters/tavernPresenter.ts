...
export function presentKorchmaFront(character: CharacterSummary): string {
  return [
    ...
    "",
    "Під стріхою сидить <i>Єгер</i>. Він дивиться на двір так, ніби двір уже лишив сліди й тепер нервує.",
    "",
...
export function presentKorchmaDeepClosed(character: CharacterSummary): string {
  return [
    ...
    presentCharacterHeader(character),
    "",
    "Під люком коротко гарчить щось таке, що Корчмар називає «звичайним способом розвʼязувати паперові проблеми».",
    "",
    "Тут починаються сутички за корчмарськими справами. Монстри сидять нижче, щоб їхній ентузіязм не збивав піну з кухлів."
  ].join("\n");
}
