...
export function presentItemUpgradeList(result: ItemUpgradeListResult): string {
  ...
  if (result.items.length === 0) {
    return [
      "✨ <b>Чароковальня</b>",
      "",
      "Ельф-маг дивиться в торбу й не знаходить манатки, яку зараз варто підсилювати.",
      "",
  ...
  return [
    "✨ <b>Чароковальня</b>",
    "",
    "У задвірку корчми ельф-маг тримає іскри в банці й підсилює одну манатку зі стосу до наступного «+». Попередній перегляд нічого не витрачає.",
    "",
    `Іскрокамінь: <b>${result.iskrokamin}</b>`,
    result.canUseSelfTemper
      ? "Як маг, ви можете зробити іскровий підкрут самі: менше золота, більше мани й трохи більше нервів."
      : "Якщо ви не маг, ельф-маг робить відповідальний стукіт сам."
  ].join("\n");
}
...
export function presentItemUpgradePreview(result: ItemUpgradePreviewResult): string {
  ...
  if (result.state === "not-upgradeable") {
    return "Цю манатку Чароковальня поки не бере. Ельф-маг каже: «Не все, що блищить, треба бити молотом».";
  }
  ...
  const methodLine = result.method === "self"
    ...
    : "Спосіб: <b>ельф-маг Чароковальні</b> — золото за відповідальний стукіт.";
...
export function presentItemUpgradeUnlock(result: ItemUpgradeUnlockServiceResult): string {
  if (result.state === "no-character") {
    return "Спершу створіть пригодника через /start. Ельф-маг не приймає аптечки від туману.";
  }
  ...
  if (result.state === "missing-field-kit") {
    return [
      "✨ <b>Чароковальня</b>",
      "",
      "Ельф-маг просить принести <b>Польову аптечку</b>. Без неї він відмовляється пояснювати, чому іскри іноді поводяться як бухгалтерія.",
      "",
  ...
  if (result.state === "already-unlocked") {
    return [
      ...
      "",
      "Ельф-маг дивиться на журнал і киває: аптечка вже пішла на безпеку, іскри вже підписалися під правилами."
    ].join("\n");
  }
  ...
  return [
    ...
    "",
    "Ельф-маг приймає <b>Польову аптечку</b>, кладе її біля банки з Іскрокаменем і урочисто пояснює: манатки можна підсилювати тільки по одній одиниці зі стосу, а стара кнопка не має права вдруге брати плату.",
    "",
...
>): string {
  if (result.state === "wrong-place") {
    return [
      "✨ <b>Чароковальня</b>",
      "",
      "Ельф-маг працює не в торбі, а в <b>задвірку корчми</b>. Тут кнопка тільки чемно показує дорогу й нічого не витрачає."
    ].join("\n");
  }

  if (result.state === "level-locked") {
    const remortLine = result.character.remortCount && result.character.remortCount > 0
      ? `Після реморту ельф-маг пускає з <b>${result.requiredLevel}</b> рівня.`
      : `Ельф-маг пускає до іскор з <b>${result.requiredLevel}</b> рівня.`;
  ...
  return [
    ...
    result.fieldKitQuantity > 0
      ? "Аптечка у вас є. Можна віддати її ельфу-магу й відкрити роботу з Іскрокаменем."
