...
export function presentBarrelBeerTutorialLookup(
  ...
  if (result.state === "available") {
    return [
      ...
      "",
      "На столі лежить записка з круглим слідом від кухля: «Новачкам — аванс на дорогу до Бочки. Повернутися з піною в голові, але на своїх ногах».",
      "",
      "Дійди до Бочки, пройди новачковий соло-рейд, вистав пиво всім, випий своє й повернися до столу, поки діє ефект пива.",
      "",
      "Аванс і запис у журналі зʼявляться тільки якщо ти справді візьмеш записку."
    ].join("\n");
  }
...
export function presentBarrelBeerTutorialAccept(
  ...
  return [
    ...
    "",
    "Завдання просте: знайти Бочку, пройти там новачковий соло-рейд, виставити пиво всім, випити своє й повернутися до столу, доки хміль ще тримає. На полі лишився припис: «Потрібен зломщик. Або хоча б пригодник, який не питає забагато до першого кухля».",
    "",
...
export function presentBarrelBeerTutorialTurnIn(
  ...
  if (result.state === "not-started" || result.state === "missing-progress" || result.state === "wrong-location") {
    ...
    return [
      "Стіл мовчить. Записка вперто чекає повного маршруту: Бочка, новачковий соло-рейд, пиво всім, власний кухоль — і тільки тоді назад.",
      "",
      presentProgressHint(result.progress)
    ].join("\n");
  }
...
function presentProgressHint(progress: BarrelBeerTutorialProgress): string {
  ...
  if (!progress.beerRoundOffered) {
    return "Рейд позаду. Тепер вистав пиво всім і випий своє.";
  }
