...
export function referralInviteShareText(
  ...
  const displayName = `${identity.guildCrest ? `${identity.guildCrest} ` : ""}${identity.name}${
    identity.activeCosmeticTitle ? ` («${identity.activeCosmeticTitle}»)` : ""
  }`;
  return [
    "📨 Поклик до Квестарні",
    "",
    referralInviteShareBody(index, displayName)
  ].join("\n");
}
