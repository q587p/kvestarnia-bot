...
- Marked `⬅️ До зали` return buttons when another Korchma location currently has an available quest route.
...
- Scaled Priest direct-heal mana spending to actual HP restored, capped at 13, and scaled Priest blessing spending from 8 to 23 by the granted `+1..+5` luck bonus.
...
- Local `/dev_reset_priest_blessing` and `/dev_reset_quiet_pocket` are non-production grant helpers; production config must not register, show or mutate through them.
