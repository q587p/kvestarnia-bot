...
- Added a narrow Єгер supply path: any eligible hero can preview the exact bandage price/current gold, then confirm or cancel a tokenized purchase for one bandage or a top-up to 5, 17 or 93 paid bandages per day; `class.ranger` gets a lower price and a replay-safe periodic free bandage claim.
...
- Єгер paid bandage purchase now uses preview/confirm/cancel callbacks backed by the existing daily-action audit boundary, so duplicate confirms replay the same receipt instead of debiting/granting again; bundle buttons top up to the selected daily target and cannot exceed 93 paid bandages for the day.
