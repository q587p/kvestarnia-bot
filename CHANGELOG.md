...
- The Fighting Corner claim card now places the soap reaction before the `Отримано` block and separates granted items with a blank line, so the reward reads as distinct Telegram paragraphs on both first claim and replay.
