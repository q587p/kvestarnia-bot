...
- Newly started turn duels no longer edit an older confirmation or invite into a combat card above fresh Telegram messages. When movement is needed, the callback now delivers the Fighting Corner transition first, the announcement/advice card second and a new combat card last, clears the obsolete confirmation keyboard, and suppresses the generic trailing movement notice. Combat lock passes only the participant's exact current active turn-duel token, including an exact action/view/refresh while Friday Barrel is pending; `/duel`, Quick Duel and unrelated turn-duel routes remain isolated by persistent, Training, party-boss and pending-raid combat without duel state, quest, activity or presence mutations. A resolved historical canonical token never inherits that exemption: rematch and rematch-risk stop before service or presence writes when a newer turn duel, persistent combat, Training, party-boss combat or pending Friday Barrel owns the participant, while the historical result/reference stays untouched and the actual current combat opens through its own card or a separate reply. Quick Duel final acceptance now atomically claims both participants' combat leases before resolution, so either participant's late persistent, Training or party-boss lease returns a non-mutating busy result. A queued exact-current canonical action/Refresh that becomes terminal between middleware and handler still repairs the terminal result. Private active and terminal reopens plus stale owner refreshes reuse the participant's recorded card. Action delivery, remote notifications, timeout updates, combat-lock redirects and Rogue retaliation now share one global protocol: replacements are sent inert, compare-and-set against the expected reference, revalidated and activated only by the winner. Retryable existing edits retain their reference; ambiguous active or terminal activation failures retain the claimed candidate for same-message retry; only definitive missing/uneditable failures release it or permit fallback. Authoritative per-participant serialization prevents older active versions from overwriting newer or terminal cards, and canonical callback sources are not disarmed before a fallible rewrite.
