...
- Made loot v1 a canonical durability boundary: frozen per-enemy stats, supported abilities, reward inputs and per-participant roll commitments now drive restart and settlement without current item, monster-loot, monster-metadata or generic-loot dependencies. Shape-valid item-id/quantity mutations fail closed in active and terminal rows while legitimate pending settlement survives unrelated content drift.
