...
- One shared quest reward presenter now separates `Отримано` from `Здобуто` with a blank line across standard quest cards, including Korchma-round claim/replay. The Fighting Corner card also keeps its soap reaction before the amount, so both fresh and replayed rewards read as distinct Telegram paragraphs.
