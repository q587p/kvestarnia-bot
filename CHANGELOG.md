...
- Added a separate one-time production opening message in the ordinary-fight participant/opponent/tip shape, followed by a compact canonical combat card with visible player/item/monster cooldowns and effect durations. At most one deterministic monster speaks per turn. Multi-target player actions state whether they affect every enemy or ally and whether values are per target; every defeated monster receives an in-turn journal/card line. Player and monster statistics share all eight damage, healing, prevention, control, damage-taken, action, special and guard dimensions behind `📊 Статистика` beside `📜 Журнал`. The complete 25-turn journal retains every turn's HP, mana, remarks, cooldowns and effects. The state budget is `65,536` bytes for the measured full 3×6 journal; Telegram cards/callbacks remain capped at `4,096`/`64`.
