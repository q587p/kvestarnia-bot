...
- Outside combat, complete eligible minutes lazily grant capped `+1 HP / +1 mana`; guarded Character/Cooldown CAS settlement advances the cursor while full, settles through expiry before refeed/freeze and excludes only actual combat-lease time. Every solo lease release now atomically claims the matching lease, advances through its logical release time, preserves the outside remainder and deletes the lease; duplicate releases cannot consume later recovery. Turn-based duel acceptance persists canonical pre-lease recovery to Character exactly once before freezing the remaining contract.
