...
- Review hardening makes delivery acknowledgements revision/reference-CAS safe, treats Telegram `message is not modified` as success, parks permanent send failures and retries only transient/`429` failures. Existing revoke, composer cancellation, terminal retention and redaction recovery continue while fresh chat events are disabled; same-life rejoins re-arm delivery with unique transition keys, and remort bulk expiry now terminalizes every affected Big Barrel chat before clearing memberships. Media, captions and forwarded updates remain with existing handlers, the dev fill stays at the 130-row hard cap, and visible waits preserve exact remaining seconds.
