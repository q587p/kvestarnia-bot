...
- Added the non-production `/dev_group_combat <party-token>` entry and an equivalent leader-only recruiting-card button behind `GROUP_COMBAT_PROOF_ENABLED=false`. Both routes use the same start service; production cannot enable, register, list, show or mutate through either surface even when the flag is set.
