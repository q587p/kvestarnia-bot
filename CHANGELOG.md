...
### Compatibility
- Priest healing/blessing and Rogue adventure gates retain their pre-release behavior; Varenyk self-feed is hidden during an active adventure. Public Varenyk open/preview capture one logical clock and reuse the repository's authoritative active-equipment/resource planning snapshot, so passive regeneration occurs once and displayed mana/maxima/stats match the exact stored rank/cost across the inclusive attunement boundary. Expected guarded Hero/Varenyk lazy-settlement CAS losers retry boundedly to the winner's canonical state without duplicate cursor, recovery or notification effects, while unexpected database errors still surface. Hero renders authoritative post-settlement resources and the ordinary full-HP notice once in the same card, while status-absent and fully settled historical-receipt rows avoid heavyweight Sated fan-out. Training Doppelganger journals now mirror persisted nonzero Sated recovery from the active card without `+0` components or recalculation. Duplicate receipt cards stop calling an expired status active: they show only the remaining recipient wait, or availability once both gates ended. Shynok recovery, ordinary passive regeneration, combat rewards and other stored journals keep their existing contracts. No food/cooking engine, item/economy change, scheduler, rollout flag, location, Prisma model or migration was added.
