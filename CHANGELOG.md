...
### Security
- Completed battle results, journals and statistics are unlisted capability-link artifacts readable by any holder of the opaque terminal token without participant, Character or private-chat authorization. Persistent solo, Training and starter Mimic cards now expose compact versioned Telegram URLs with matching pre-onboarding `/start` consumers. New artifacts freeze their minimum public combatant identity in stored state/result JSON, so rename, title, crest, level or remort changes cannot rewrite old cards; legacy rows use a generic stored-data-only fallback. Reads never join the mutable owner Character, stay non-settling, and do not expose Telegram identity, hidden choices, inventory or other private mutable state.
...
- Forming/active private character cards now show the escaped guild crest and name below the title. Active nearby members show only their escaped crest beside the already-visible Character name; eligible leaders can invite an opted-in nonmember from the same-location card through a revalidated five-row picker.
...
- The custom crest route now accepts exactly one genuine Unicode RGI emoji sequence, preserves visible FE0F presentation selectors, canonicalizes selector-less/FE0F aliases only to a separate reservation key, and rejects explicit FE0E text presentation, lone flag halves, pseudo-ZWJ combinations or non-emoji text decorated with variation selectors. The enabled Spusk card also names the low side passage to the Guild Nest. Unsupported media and links reissue the exact prompt only after current authority/version validation; occupied, textual, multiple-emoji, forged and stale replies recover without intent/profile/gold/cooldown/audit mutation, while saved pre-release photo prompts and view callbacks stay inert.
...
- Made GroupCombat enemy focus match the Big Barrel contract: the living party leader is targeted initially, then the participant with the most actual damage in the previous resolved turn, with stable roster-order ties. Active cards and each newly stored journal turn mark `🎯 ціль ворогів`, while recap actions announce focus changes; legacy snapshots without focus evidence remain unmarked.
...
- Added explicit encounter reservation fields and an additive `20260724233000_left_passage_party_attack` migration. Reservation binds the pending preview or same-life single-enemy terminal survivor, its regenerated primary HP, initiating life, canonical left-passage presence location and party without overloading `periodId`; duplicate create/start, solo/start races, cancellation and undersized expiry converge without rerolling or healing the primary. Production v3 rows that cannot be recovered safely now retain their leases and frozen evidence under an explicit operator-repair marker with a durable reason.
...
- Added real per-participant settlement for final same-life HP/mana, committed consumables, ordinary persistent-fight XP/gold bands, deterministic per-enemy loot and privacy-safe Chronicle activity. A 1×1 fight now uses the same XP formula and character-level gold band as ordinary PvE; larger enemy groups add those enemy budgets before the neutral eligible-participant split. Each enemy also gets one ordinary broad-loot roll and bandage slot, distributed round-robin among eligible manual participants; effective enemy level scales the broad chance, additional enemies add opportunities, and authored/Loot Expansion manatky plus the ordinary rare Iskrokamin replacement remain possible. Timeout auto-guards do not qualify for rewards or activity; each participant lease remains until that participant's exact resource/reward/activity effects and canonical immutable receipt commit.
- Added shared preparation rendering with `✅ Готово` / `⏳ Зачекайте`, an always-available current-leader start action and live gathering discovery/join through exact-location `👀 Хто поруч`. The last ready participant now starts a 1–3-person attack immediately through an atomic roster check; the exact three-minute deadline remains the fallback. The left-passage gathering card states the readiness/deadline rule and early leader start once, then keeps enemy scaling to one short sentence.
- Added a separate one-time production opening message in the ordinary-fight participant/opponent/tip shape, including explicit remort-reinforcement and repeated-Низ-pressure notes when applicable, followed by a compact canonical combat card with visible player/item/monster cooldowns and effect durations. At most one deterministic monster speaks per turn. Multi-target player actions state whether they affect every enemy or ally and whether values are per target; solo ability/item lines name the actor once, use explicit `застосовує вміння` / `використовує манатку`, keep the action icon and omit quoted labels or repeated self-target names. Every defeated monster receives its own paragraph after the complete participant/enemy action exchange. Player and monster statistics share all eight damage, healing, prevention, control, damage-taken, action, special and guard dimensions behind `📊 Статистика` beside `📜 Журнал` on the terminal result only; active cards and stale active callbacks expose neither detail surface. The rolling last-25-turn journal retains HP, mana, remarks, cooldowns and effects. The state budget is `65,536` bytes for the measured full 3×6 window; Telegram cards/callbacks remain capped at `4,096`/`64`.
...
### Fixed
- Aligned GroupCombat landed-hit reactions with the shared monster runtime: counter misses retain their charge, successful counters spend one charge and use authored/default potency, flat reflect spends one landed-hit charge, and counter/reflect/shield-break damage now adds an explicit causal journal line and exact contribution damage. If the final enemy and final participant fall in the same participant-initiated exchange, the already-defeated enemy keeps the canonical victory outcome. Compact recap parsing now rejects non-canonical base64url, masks outside the frozen roster, zero/unknown/truncated groups, hybrid legacy tuples and duplicate/overlapping targets. A single target-side contract derives legal participant/enemy sides from every GroupCombat status source and all compiled frozen monster components, rejecting impossible packed, exact-legacy and verbose pairs such as `guard` on an enemy before expansion; malformed production v3 evidence stays operator-fenced with leases retained. Active and historical effect rows derive truthful beneficial/harmful labels, including participant damage/accuracy/evasion/critical penalties and enemy buffs.
- Restored terminal GroupCombat navigation: `↩️ До результатів` now edits the current journal/statistics card even after that participant's settlement and exit delivery are complete, and a completed party invitation deep link opens the participant's result with `📜 Журнал` / `📊 Статистика` instead of trying to edit the `/start` command as an active card. Active cards and every historical recap now include authored timed monster effects such as bleeding and evasion with their remaining activations; damage-over-time ticks name the actual effect. The compact recap projection groups equal target effects and bit-packs kind/side/target mask/duration while retaining the strict runtime effect evidence, keeping the measured 25-turn 3×6 maximum at `64,658/65,536` bytes. Multi-enemy shared keyboard layout now pairs guard with the first ability when enemy targets already fill their rows, so the remaining target abilities stay paired without changing one-enemy order.
...
- Kept the production GroupCombat opening as a truly separate message that never edits or releases the participant's canonical card reference, so an overlapping first scheduler delivery cannot leave two full first-turn cards. The first authoritative UI-generation owner now sends that opening with the persistent location-aware main reply keyboard and immediately follows it with the canonical card's own non-empty inline combat controls. Every later active-card send/edit carries authoritative inline actions (or observer `🔎 Оновити`), so message order, `/start`, refresh, scheduler retry and Telegram reply-keyboard quirks cannot leave the newest live card inert. A shared ordinary/Training/Party Boss/GroupCombat layout now pairs the primary attack and mode-specific defence, pairs concrete abilities, gives `🎒 Одноразові манатки` its own row and pairs GroupCombat `🏃 Відступити` with `🔎 Оновити` in the final row; ordinary/GroupCombat use `🗡️ Вдарити` / `🛡 Захищатися`, while Party Boss retains distinct `🧱 Захищатися` beside its separate `🛡️ На мене!` ability. Multi-enemy GroupCombat keeps explicit short target names and pairs an odd final target with guard to avoid an unnecessary full-width row.
- Serialized that separate production opening inside the same durable participant claim immediately before the canonical card; command/scheduler overlap therefore cannot leave active combat without a keyboard. Corrected strict exit evidence so repair accepts the normal claimed `menu-delivered` interval before the final CAS; generic combat-lock routing now resumes that exact temporary exit-navigation lease and continues the requested quest/location route instead of repeatedly blocking the whole bot with an owner-mismatch warning.
...
- Preserved rewardless v1/v2 proof parsing, repair and delivery while production v3 strictly validates location, roster/enemy cardinality, frozen pressure/remort sources, backup adjustments, level cap, reward and loot contract, terminal result, plan and receipt. Untrustworthy v3 state fails closed for operator disposition without downgrading or unlocking participants.
- Quarantined operator-required v3 rows before bounded timeout, card delivery, settlement and repair limits, including orphan-lease repair. Initial reads, final reloads and every mutation CAS now fence on the quarantine marker and its version bump, so stale action, timeout, settlement and delivery work cannot mutate or expose the row. Explicit operator inspection alone retains raw active/terminal state, actions and participant receipts. Shape-valid receipt drift after an integrity checkpoint is selected through receipt freshness and fails closed without releasing the remaining lease.
...
- Routed level milestones through the same durable participant transaction and deliver the existing level-up and generic level/item/combat achievement notices as separate Telegram messages only after a fresh manual settlement. Replays and timeout-only settlements do not duplicate or progress them. Terminal cards use the ordinary victory/reward shape; `🧾 Знешкоджено` is recorded separately at each actual enemy defeat and rendered only after the turn's complete action exchange instead of interrupting monster responses or being reconstructed as one terminal list.
- Deferred the unreleased left-passage completion achievement and event. Migration `20260724233000_left_passage_party_attack` keeps its identity but no longer adds achievement-effect columns or indexes; GroupCombat has no effect selector, repair branch, projector, ownership reconciliation or scheduler work.
...
- Removed the nine-ability GroupCombat allowlist and froze all 132 canonical authored monster abilities in the immutable production-v1 catalog. GroupCombat compiles every authored target, damage/modifier, heal, shield, mana-drain, cleanse/removal/reapply, lock/pressure, counter/reflect, damage-over-time, evasion/accuracy, mark, cooldown and conditional follow-up component into typed deterministic execution; persisted effects retain exact source, target, trigger, duration and charges for strict restart rederivation. There is no label-only fallback: unsupported components fail before a production encounter starts, and unknown or malformed persisted semantics fail closed.
- Made the 132-ability guarantee semantic rather than shape-only: every active authored parameter has one typed execution consumer. Mana pressure is charged once only on mana-cost abilities; false-exit pressure feeds the single flee roll without breaking guaranteed attempt seven; class, race and deterministic one-ability locks retain exact evidence; charged marks wait for the first landed hit; and expired-effect reapplication preserves the original authored semantics plus restart-verifiable provenance. Compact backward-compatible recap snapshots retain the complete rolling 25-turn journal while keeping the worst of 42 deterministic 3×6 rosters at `64,658/65,536` bytes, including the authored timed-effect projection.
- Made manual reward eligibility independent of roster execution order. Every action accepted against the complete turn-start snapshot qualifies before start-of-turn effects or earlier actors can end combat; skipped abilities/items consume no mana, inventory or cooldowns, while timeout guards remain ineligible. Chronicle activity now lists only the eligible manual roster.
- Replaced superseded GroupCombat action/refresh cards with a new bottom-most canonical card, removed terminal refresh after settlement, aligned the single-living-enemy action with ordinary combat as `🗡️ Вдарити`, and kept full contribution details in the separate statistics view.
- Made stale left-passage party buttons refresh the current preview without a false changed-occasion alert. `/dev_reset_monster_rest` now clears the consumed-passage completion used by the visible passage rest gate, and Bard performance blockers explain that already-fighting nearby players must receive Inspiration before combat.
- A production victory now gives every participant the same deterministic 13–23-minute clear left-passage window. Its card exposes the return to the Сутерени above an unavailable `🪜 Ярус II` construction notice; during that window the participant cannot enter another left-passage gathering through either an invitation or `👀 Хто поруч`, and `/dev_reset_monster_rest` clears the window for local QA.
- Clarified the reward result: each enemy contributes its ordinary XP/gold budget before the eligible-manual neutral split. A new production loss keeps the one-fifth consolation and adds half the ordinary XP, rounded up, for every enemy already defeated; loss gold and loot remain zero, while older rows without the frozen policy marker replay unchanged. Each enemy now also receives a deterministic ordinary broad-loot roll and bandage slot, so stronger/more numerous enemies can yield authored or Loot Expansion manatky, bandages and the ordinary rare `Іскрокамінь` replacement without rerolling on restart. A winning no-loot card says that no manatka dropped.
...
- Made loot v1 a canonical durability boundary backed by a code-owned immutable monster/ability/effective-rarity catalog and resolver. Restart and settlement rederive enemy evidence, candidates, weights, exact selected items and quantities from frozen inputs without trusting persisted selection ranges or adjacent public checksums and without current item, monster-loot, monster/profile or generic-loot dependencies. Every released Loot Expansion `x025` base/enhancement variant keeps its effective epic selection rarity. The independently parsed relational participant snapshot now anchors immutable class/race/level, maxima, combat/support/base stats, equipment, gear abilities and initial item quantities; only canonical runtime progress and committed item decrements may differ. Coherent actor/evidence/output forgery with recomputed legacy SHA-256 values and shape-valid item-id/quantity mutations fail closed in active and terminal rows while legitimate progress and pending settlement survive restart and unrelated content drift.
...
- Kept the persistent main menu available during active GroupCombat and moved the complete current action set onto every canonical live card as inline controls. Every currently available class, race and equipment ability is visible directly, while canonical cooldown, mana and target validation hides unavailable abilities and restores them when legal. Active publication, flee/terminal menu restoration and later combat ownership share one durable per-participant publication guard plus the owned combat lease; an older worker cannot overwrite newer navigation. Frozen labels from older reply keyboards still reach canonical validation, so an unavailable duplicate commits nothing and produces one fresh bottom-most actor card without a target picker or separate error bubble. Stale GroupCombat reply text delegates instead of replacing a newer combat/navigation surface. One-use items keep their filtered submenu, and a knocked-out participant's canonical card contains only `🔎 Оновити`. Terminal delivery now sends the bottom-most result with `📜 Журнал` and `📊 Статистика` immediately while leaving the already persistent main keyboard untouched; a legacy reply-keyboard result that Telegram cannot edit is deleted and replaced once, and permanent deletion refusal releases navigation instead of fencing quests forever. The redundant completion notice is gone. `🏃 Відступити` remains a personal ordinary escalating flee roll.
- Hid `🎒 Одноразові манатки` whenever no owned supported item is currently useful. The winning canonical-card CAS records the exact inline-control fingerprint already published, so acknowledgement failure or scheduler retry edits that card instead of sending countdown copies. A losing candidate gets bounded deletion attempts and otherwise becomes one compact inert superseded note; replacement compacts the previous canonical message before delete. `/start`, `🔎 Оновити`, handled actions, restart repair and changed controls all converge to a canonical card with non-empty inline controls without removing the persistent main reply keyboard. Completed terminal navigation clears obsolete delivery work, including an unclaimed `menu-delivered` restart row whose legacy Telegram message cannot be edited.
- Bounded every GroupCombat control-surface publication below its durable owner. Active cards renew the exact claim before each Telegram send/edit/delete; exit navigation renews its independent claim and lease immediately before flee-menu or terminal-result publication. A terminal result carries inline controls on its first send; legacy delete-and-replace recovery stays under the same owner through old-card compaction and reference adoption, and only the final token/reference CAS releases the lease. Each request aborts after 13 seconds, ownership loss stops later calls, ordinary failure releases safely, restart after adoption remains recoverable and newer combat UI stays last. Only controls actually attached to a private canonical card advance the legacy fingerprint/generation fields. Validated manual actions commit before bounded turn resolution, so process death or UI contention cannot erase a final win/flee action. Telegram acceptance immediately before abort/network failure remains externally at-least-once.
- Made the single authoritative combat-lease read fail closed when its exact owner is absent, terminal, quarantined, belongs to another participant or uses an unknown kind. The combat lock sends one recovery notice and performs no unrelated combat probes or downstream navigation instead of guessing ownership from secondary state.
- Successful GroupCombat flee now commits exact HP/mana, consumed inventory, timed-status release, canonical exit evidence and that participant's lease release atomically before Telegram delivery. Gameplay exit remains exactly once; notification delivery is documented and implemented as durable at-least-once because Telegram offers no send idempotency key. A live claim retries database acknowledgement for the returned message id without immediately resending, durable acknowledgement prevents resend after card failure, ordinary rejection remains retryable, and stale reclaim bounds the unavoidable duplicate risk in the send/ack crash gap. Menu restoration uses current presence/quest markers for a free player and is durably superseded instead of overwriting a newer combat keyboard. Remaining actors continue with their leases; later settlement cannot overwrite a remorted or otherwise changed escapee, and Chronicle activity contains only eligible manual participants rather than escapees or timeout-only rows.
- Kept knocked-out, non-fled participants leased while living allies continue. Ordinary repair now treats that lease as valid instead of quarantining the production fight, so combat-lock recovery can always reload the canonical observer or battle card.
- Knocked-out participants now receive truthful shared observer copy without a target prompt or auto-guard timer, and manual participants whose neutral split rounds to zero no longer receive the false timeout-only accusation. Join eligibility materializes canonical passive HP/mana recovery before classifying a lease-free character as knocked out, while an active lease prevents mid-combat healing.
- Timeout auto-guards now advance only that participant's authored cooldowns/effects while remaining excluded from committed manual actions, XP, gold, loot and Chronicle activity.
- Made the party action available for a genuinely pending left-passage preview or its exact same-life, single-enemy terminal survivor. Survivor reservation freezes canonical regenerated HP into relational and strict v3 evidence; cancelling the gathering restores consumed solo recovery, while malformed, won, dead, multi-enemy or previous-life rows remain ineligible. The solo action stays available until reservation and is labelled explicitly.
- Generalized the post-GroupCombat rest blocker: it names the previous group fight rather than the left passage while preserving the canonical remaining wait for both direct-invitation and nearby joins.
...
- Consumable Manatka Uses is the next planned `0.4.3` contract, still blocked on its exact-item allowlist activation gate. Guild foundation follows in `0.4.4`; this release implements neither feature and adds no guild, Big Barrel migration, public matchmaking, group chat, parties above three, encounters above six enemies, rare item or new economy.
...
- Corrupted relational rosters above the playable cap now converge through an explicit repair-only invalid representation capped at 13 participants and the existing state-byte budget. Four-row corruption is preserved exactly for zero-reward settlement/replay; larger or otherwise unrepresentable rosters release every owned lease and recoverable frozen status before deterministic row cleanup. Active/won/lost gameplay and plans remain capped at three participants.
- Pending participant settlement rows are now valid only with exactly zero attempts, no receipt and no settlement timestamp. Active invalidation and terminal repair canonicalize positive or negative pending attempts to zero before integrity-checking, while completed canonical receipts, positive attempts, timestamps and replay behavior remain unchanged.
...
- Added update-local single-flight reuse for pending-Friday and Fight overview reads. Global middleware, handler guards, Daily Korchma markers and marker-aware navigation can share the same pre-mutation Promise without a process-wide cache; Friday start/completion and newly created next-problem issues now invalidate only the fact they changed before rebuilding markers, while idempotent paths keep one lookup.
...
- Successful nonterminal persistent-fight turns now reuse their pre-turn problem-quest progress because that path cannot change wins or rewards. Terminal settlement still reloads progress after writes; the representative active-turn query-event budget falls from 48 to 27 reads.
...
### Verification and compatibility
- Added isolated Prisma query-event tests proving Friday budgets of one statement without a character and at most three with a character, grouped Fight/problem budgets of one to five statements, a complete full-source marker reduction from 75 reads to 11, and a fresh nonterminal fight-turn reduction from 48 reads / 51 total statements to 27 / 30. Fixtures with 10,000 irrelevant rows and 100 newer allowlisted rows preserve statement budgets and legacy marker output, including older authoritative current-life, Friday and equipment-attunement facts; valid combat win counting still stops at 93.
...
- Equivalent help aliases now share compact rows: `/hero, /profile`, `/bestiary, /monsters`, and `/equipment, /gear`, while `/equip` remains a separate expanded-control shortcut and every command remains discoverable.
...
- Command aliases now open distinct compact views: `/equip` goes straight to equipment slot controls, `/items` opens Manatky alphabetically, and `/bag` orders item kinds by when their current stack appeared in the bag. Topping up a stack keeps its place; fully depleting and later reacquiring it creates a fresh stack that moves to the top. `/me` renders only identity, title, level, resources and characteristics without action buttons. `/quest` is described as the quest list rather than the physical Quest Table.
- In local development, the persistent `🧰 Адмінка` button is restored beside `📖 Допомога` in the bottom main-keyboard row. Production suppresses it even if requested, and the dev command reference is grouped into general, combat/party, resources/Manatky and quest/wait sections.

### Fixed
- Item detail cards opened from a slot-filtered Manatky/equipment list now keep the existing equip action for upgraded magical gear. Those previews explain the attunement wait or interruption confirmation instead of falsely claiming that the slot is simply ready; the Charkokovalnia return-only route remains unchanged.
- The short `/me` lookup now settles passive and Sated recovery without discarding either one-shot notice. It sends those notices separately after the buttonless six-line card and skips inventory, active-drink, action-gate, achievement and Bard reads that the short surface does not use.
...
- Disabling `BIG_BARREL_BROTHER_RAID_ENABLED` still blocks new player starts, while bounded orphan repair and due resolution continue for already-active sessions. Terminal replays no longer fan out duplicate cards, and terminal PartyBoss history survives leader or nonleader character restart for the remaining roster.
- PartyBoss ally protection is now applied after every participant commits their action, making Priest, Molfar and equipment guards independent of roster order. Molfar counter damage requires positive incoming damage and a surviving protected participant.
- Joined participants at zero HP now enter PartyBoss as frozen knockouts instead of invalid active snapshots. Mixed rosters continue safely, all-zero rosters settle as rewardless losses, and one failed scheduled recruiting start cannot block later starts or active-turn timeout work.
- Legacy terminal PartyBoss JSON now receives its original leader snapshot during the unreleased round-history migration, preserving the leader, result and every journal page after leader restart. Stored rounds reject invalid optional Ward counters, runtime enums and resource/status combinations before presentation.
- The unreleased migration now also converts only the pre-0.3.16 `active` plus numeric zero-HP participant mismatch to `knocked-out` in current state, embedded historical rounds and matching terminal result rows. Legacy mixed raids continue, all-zero raids settle rewardlessly, and malformed values such as string `"0"` remain strict validation failures.
- Big Barrel full-contribution thresholds now use each participant's bounded durable manual-plus-timeout action count as the personal available-round denominator. One manual action no longer earns the full tier after a long timeout run, exact half rounds upward, and an early legitimate knockout is measured only against rounds where that participant could act; 25-turn hot state stays bounded and stored rewards replay exactly once.
- The unreleased JSON normalization migration now inspects object paths only for `json_each` object entries. Scalar strings and other non-object values in participants, round history, participant snapshots and terminal results survive migration unchanged for strict rejection; an active malformed row is cancelled rewardlessly without blocking healthy due work.
...
- Added transactional allowlisted system events for real membership, leadership, Ward, Protocol, start, resolved raid-wide mechanics, participant knockouts and terminal transitions. Chat writes use a separate revision and cannot change combat/session versions, resources, rewards, contribution or achievements.
...
- Raid chat ships as part of the existing `BIG_BARREL_BROTHER_RAID_ENABLED` surface and adds no second environment/config key. Disabling the Big Barrel surface hides chat controls, blocks fresh reads/writes and schedules existing bot-card redaction without deleting retained history immediately.
...
- Review hardening makes delivery acknowledgements revision/reference-CAS safe, treats Telegram `message is not modified` as success, parks permanent send/edit failures and retries only transient/`429` failures. Existing revoke, composer cancellation, terminal retention and redaction recovery continue while the Big Barrel surface is disabled; disabled startup invalidates old compose prompts, same-life rejoins re-arm delivery with unique transition keys, and remort terminalizes only the actor's canonical due Big Barrel membership without sweeping unrelated sessions. Dirty/redaction delivery bypasses clean due rows, replacement references do not re-wake clean work, and remort leader-transfer keys include the winning transition version. Media, captions and forwarded updates remain with existing handlers, the dev fill stays at the 130-row hard cap, and visible waits preserve exact remaining seconds.
...
- Every accepted player post now immediately best-effort sends each other current raid participant a separate HTML-safe notification with the author's bold name and a blockquoted message. A failed recipient delivery cannot roll back the stored post or sender confirmation, and coalesced card delivery remains the durable recovery path. Knocked-out boss-roster participants explicitly keep raid-chat read/write access and the chat button while losing only combat actions.
...
- Fixed Varenyk feeding falsely treating `User.currentAdventureId` and `User.currentRaidId` presence/navigation grouping as active incompatible flows. Cellar, Hunt, quest-table, passage and stale adventure/raid markers no longer block self/other open, preview or commit by themselves; authoritative combat leases still fail closed for actor and recipient, including active Big Barrel combat.

## [0.3.14] - 12026-07-19 - Bard Inspiration and Big Barrel Lament

### Added
- A successful normal Bard performance now grants its frozen initial audience free, non-stacking `✨ Натхнення` for 13 hybrid minutes. Existing performance grades map to `+1/+2/+3/+5` percentage points on canonical player hit rolls only; stronger Inspiration replaces and resets the timer, while equal or weaker Inspiration neither replaces nor refreshes it. Tips, applause, house payout, damage, critical hits and non-roll effects do not change the bonus.
...
### Availability and compatibility
- Inspiration ships as part of normal Bard performances without a class-specific rollout flag. Lament remains scoped to the existing `BIG_BARREL_BROTHER_RAID_ENABLED` Big Barrel surface. The Bard’s Tavern raid-prep tip again mentions buffs, debuffs and stools, while the detailed Inspiration/Lament explanation stays on the Big Barrel intro instead of leaking into Fight or Duel cards.
...
- Review hardening preserves existing raid achievement events when Lament completes a round, makes shared music history life-bound, consumes Inspiration on fatal committed responses, shows frozen Inspiration on `/hero`, rejects generic Lament writes, and keeps the shared timed-status lifecycle replay-safe across every durable combat surface. Active timed buffs now share one Telegram presentation shape with bold name and duration; Inspiration bolds only its numeric modifier and explains its outside/combat countdown, while `/hero` groups several effects under one `Стани:` heading without repeated row labels. Big Barrel folds Lament activation and its reduction into one action sentence, removes the redundant active tick from action history, and groups round-authoritative Inspiration, `Ситий`, Lament, buffs/debuffs and ability/item cooldowns under the journal's `Кулдауни та ефекти:` block. The current Bard's live same-location performance now appears with its remaining reaction window on `/online`, Shynok, and the bar instead of another start button; the lookup is life-bound and read-only. Bard reaction keyboards hide tip amounts above the listener's current gold snapshot while commit-time validation still rejects stale callbacks after later spending. A solo Bard's Big Barrel recruiting card now explains that Lament appears after combat starts, the same class-specific hint stays hidden from non-Bard participants, and repository coverage proves the one-participant action resolves normally without a prior Barrel performance.
- Big Barrel recruiting no longer accepts a character whose same-period legacy solo Barrel raid is still pending, including the due-but-unclaimed boundary. The start transaction rechecks that conflict; once the deadline is due, a permanently incompatible roster enters a canonical terminal state, releases leader/membership guards, notifies participants truthfully, and leaves the scheduler queue instead of starving newer raids. Manual and automatic closure now share participant-specific terminal-card delivery: missing or permanently uneditable preparation cards receive one persisted replacement, replays reuse it, terminal keyboards expose only refresh, and stale join/leave/cancel/readiness/Ward/Protocol/deep-link actions preserve the incompatibility reason instead of reporting ordinary expiry. Terminal notification reads, Telegram calls, and reference writes are best-effort and isolated per participant, so one failure cannot block later participants or later scheduler work; no automatic retry is promised after the terminal row leaves the due queue. Transient combat/loss-cooldown conflicts remain retryable, while eligible elapsed gatherings still share the leader-button and automatic recovery path. Every production route back to the bar now performs the same read-only live-performance lookup, so an active Bard performance shows its remaining reaction window without exposing a duplicate start action. The isolated local-bot manager also treats Windows source paths case-insensitively, preventing `run` and `refresh` from owning duplicate polling processes solely because one command used `D:\...` and another `d:\...`.
- Updated `📖 Перекази` and matching lore references so every class entry reflects shipped combat and side mechanics; the Bard now explicitly covers performance, voluntary tips, payment-independent Inspiration and Big Barrel Lament. The machine-readable Вареник-мант seed ID now matches the runtime `class-varenyk-mancer` lore ID, with a consistency test for every class seed entry. The player news keeps its required in-world closing paragraph, while manual QA and other operational gates stay out of `Ще не відчинено:` copy. Manual Telegram QA remains pending; automated domain, repository, service, presenter and callback coverage owns the implementation evidence.

## [0.3.13] - 12026-07-18 - Location-Safe Quest and Combat Fixes

### Fixed
- Bard Performance no longer inherits the generic Shynok callback location assignment. Starting, responding to, refreshing and completing a performance now preserve the character's current normalized location; existing Shynok-only house behavior still applies when the performance genuinely starts there. Live cards now label the 93-minute next-performance wait as applying to the current location, matching the existing per-location cooldown contract.
- Adventure quest monster handoff no longer writes the first Niz layer into presence. The Prisma daily-action claim now carries normalized `User.lastSeenLocationId` into the completed Adventure result; the bot persists it as fight `originLocationId`, reuses it for redraw/result presence, and returns front, yard or Quest Table encounters to their actual origin after victory or flee. One shared policy now prevents Adventure problem, problem-help and approach callbacks in `active-fight`, `combat-blocked` or `already-completed` states from writing presence, regardless of timing or where an existing claim was observed. Explicit navigation, selected problem/help callbacks and newly completed non-fight results retain their Quest Table movement. Genuinely missing or invalid locations retain the Quest Table fallback, and ordinary Niz combat keeps its existing route.
...
- The Fighting Corner rule card now italicizes both first-rule quotes and the nearby-challenge route, while bold labels separate the second and third rules for faster Telegram scanning.
- Shared reward presenters now separate XP/gold amounts from actual item grants with a blank line across standard quests, the Barrel tutorial, fights, hunts, Tavern and Big Barrel Brother raids, and newly claimed duel-tournament prizes. Granted inventory rows consistently use `Здобуто` (or the aggregate `Здобуто загалом`), while previews and purchases keep their non-grant wording. The Fighting Corner card also keeps its soap reaction before the amount, so both fresh and replayed rewards read as distinct Telegram paragraphs.
- Nyz passage searches can now rarely replace an already-won bandage item slot with one `Іскрокамінь`, reusing the ordinary-fight bounded `4–6%` replacement policy instead of adding a second item roll. Safe and risky search results remain token-seeded, persist the selected item before presentation and replay without duplicate grants; the result card separates gold from the `Здобуто` item row. Only the winning repository resolution enters the standard `item.received` achievement path, and callback, scheduler or due-search guard completion delivers any unlock after the result card while replays and concurrent losers remain silent.
- Incoming open and targeted duel cards no longer label their non-mutating preview as `Accept`. Invite recipients get details, decline and refresh, while the author's own status card gets only cancel and refresh, including self-opened Quick and turn-duel links. Ordinary `/duel` and pending duel routes record only a neutral activity heartbeat until the final transition, preserving the current location, raid and adventure; participant-wide blocked final Quick acceptance deliberately performs no presence write. Quick Details and deep-link previews compute due regeneration and warnings for both participants without persisting Character resources, recovery anchors or notifications. Explicit place navigation remains unchanged, and the existing detailed confirmation remains the only surface with the actual `Так, прийняти` action; only a fresh final active/resolved transition moves the player. Its challenger introduction is separated from the mode explanation as its own Telegram paragraph. The voluntary-duel invitation flavor now uses `🗂️ Форма 13-Д`, leaving `Форма 13-А` exclusively to the Bureaucramancer ability and raid filing.
- Newly started turn duels no longer edit an older confirmation or invite into a combat card above fresh Telegram messages. When movement is needed, the callback now delivers the Fighting Corner transition first, the announcement/advice card second and a new combat card last, clears the obsolete confirmation keyboard, and suppresses the generic trailing movement notice. Combat lock passes only the participant's exact current active turn-duel token, including an exact action/view/refresh while Friday Barrel is pending; an active canonical classification whose independent active lookup already returns resolved/null still reaches terminal repair without exposing its source to another combat/Barrel edit. `/duel`, Quick Duel and unrelated turn-duel routes remain isolated by persistent, Training, party-boss and pending-raid combat. A resolved historical token never inherits the exemption: observed combat blocks rematch before service, and a temporary actor combat lease now atomically fences resource synchronization, recovery suppression and targeted challenge creation against newer turn-duel, persistent, Training or party-boss leases acquired after prechecks. Late busy results preserve resources, challenges, quest/activity state, presence and the historical result/reference. The neutral rematch heartbeat is deferred until the final Friday decision and atomic fence; late pending Friday writes none, while an allowed outside-combat rematch writes exactly one. Quick Duel final acceptance separately claims both participants' combat leases atomically. Private active and terminal reopens plus stale owner refreshes reuse the participant's recorded card. Action delivery, remote notifications, timeout updates, combat-lock redirects and Rogue retaliation now share one global protocol: replacements are sent inert, compare-and-set against the expected reference, revalidated and activated only by the winner. Retryable existing edits retain their reference; ambiguous active or terminal activation failures retain the claimed candidate for same-message retry; only definitive missing/uneditable failures release it or permit fallback. Authoritative per-participant serialization prevents older active versions from overwriting newer or terminal cards, and canonical callback sources are not disarmed before a fallible rewrite.
...
### Compatibility
- No schema, migration, dependency, new action, achievement, lore promise, production flag or dev helper was added. The existing non-production `/dev_reset_passage_search` command accepts an `iskrokamin` fixture argument for deterministic natural-loot QA. Existing Shynok performances, ordinary Niz fights, Training, Quick Duel and turn-duel mechanics retain their prior contracts outside the corrected state/copy paths.
...
- Reused `CharacterCooldown` without a schema migration for one versioned current recipient activation/receipt, actor-recipient pair waits and a short server-owned preview proof. Preview binds exact stat/applied rank, cost, effective stats, attuned equipment row/slot/version identity, Shynok recovery windows, target lives, expiry and the observed target activation; confirmation can commit only that plan, never silently upgrade after later regeneration or overwrite a target changed since preview. Activation and pair-wait prechecks run before settlement, while plan/affordability drift or any late CAS loser rolls the whole transaction back, so even a minute-late stale confirmation cannot consume the winner's pending recovery. Current-life duplicates replay the durable current receipt before mutable action gates and display wait/availability from the validated matching pair row; a shortened, expired or cleared row is authoritative without letting a newer mismatched pair rewrite the receipt.
- Outside combat, complete eligible minutes lazily grant the committed rank's capped HP/mana portion; guarded Character/Cooldown CAS settlement advances the cursor while full, retires the final incomplete fraction exactly at expiry and excludes only actual combat-lease time. Hero reads under a matching lease preserve the frozen remainder. Solo, turn-duel and Big Barrel leases persist the exact logical freeze/start time and paused remaining duration from the settled cursor, so an incomplete outside minute remains separate and cannot remove an initial combat turn. Wall-clock waiting during a stored combat lease consumes nothing; every fresh supported surviving pulse grants the same rank-scaled portion and consumes exactly one complete frozen minute. Terminal release rebases cursor and expiry together so the durable `expiresAt - cursorAt` exactly equals the combat state's frozen `expiresAt - cursorAt`, without adding the outside remainder twice. If solo or duel state is missing or malformed, cleanup reconstructs only the matching durable activation that was active at lease start, preserving its complete no-state frozen budget without inventing combat recovery or resource changes. Zero duration snaps to terminal expiry. Duplicate/replayed pulses and concurrent losing cleanup cannot shorten again, consume later recovery or touch a newer activation. Direct solo and pending-passage first/restart Fight paths now share the same atomic Sated freeze, pre-lease recovery and Character CAS as Training. Later combat damage, spend, healing and pulses remain state-local. The public turn-duel acceptance preflight uses one inclusive-`readyAt` equipment view read-only for passive resource values, warnings, confirmation, natural maxima, stats, set effects and granted actions; only the winning leased repository transaction atomically persists passive/Sated resources and anchors, rolls DEX/LUCK initiative and owns the final snapshot, so a concurrent loser or replay cannot write or reroll. Big Barrel start keeps the same canonical pre-lease-only boundary before raid state owns later resources.
- Stored persistent PvE, multi-enemy PvE and Doppelganger states grant at most one pulse after the hostile response to the owner's committed action; turn-based duels pulse after both queued actions resolve, and Big Barrel pulses after boss retaliation. A participant reduced to `0 HP` is not revived. Persistent PvE preserves the canonical restricted final response from an enemy alive at the exchange start even when defeated by that action; mutual `0 HP` against the final enemy remains a hero victory, while hero `0 HP` with another enemy alive remains a loss. Quick duels receive no synthetic pulse.
- Active `😋 Ситий` does not stack: a permitted fresh feeding replaces it with one new activation, rank and 13-minute timer. The same caster must finish or shorten their actor-recipient wait; another Varenyk-mancer has an independent pair wait and may replace it immediately. Recipient remort clears old-life state and actor remort clears that actor's pair waits without cancelling another recipient's committed status.

### Compatibility
- Completed Fight, Mimic-fight and Training presence markers no longer impersonate an active Varenyk-blocking flow after their authoritative combat lease ended; terminal Fight/Training presentation also clears the marker. Real active leases and noncombat adventures still block, while Priest/Rogue gates remain unchanged.
- Priest healing/blessing and Rogue adventure gates retain their pre-release behavior; Varenyk self-feed is hidden during an active adventure. A successful public Varenyk preview returns actor/recipient resources, natural/effective maxima, full stats, Priest blessing, exact attuned rows, rank and cost from the same transaction that persists those planning snapshots, so interleaving equipment changes cannot mix preliminary and durable views; inclusive `readyAt` may cross the real internal rank/cost boundary. Expected guarded Hero/Varenyk lazy-settlement CAS losers retry boundedly, then reload a version-validated current-life Character/status/self-pair snapshot without fabricating recovery or throwing a generic player error. Hero resolves the viewer's self-feed wait from that viewer's current-life pair row even after another caster replaces the status; optimistic fallback sandwiches Character, status and self-pair observations, while strong fallback reads all three together. A missing status row still validates Hero's preliminary fingerprint, and three unstable optimistic pairs fall back to one serializable read-only snapshot; unexpected database errors still surface. Character resources/anchors, cursor and recovery notification remain exact-once. Hero renders authoritative post-settlement resources and the ordinary full-HP notice once in the same card, while status-absent and fully settled historical-receipt rows remain bounded and avoid heavyweight Sated fan-out. Fresh Sated preview/results now match the established Priest support rhythm with semantic blank-line groups, bold player names/values, natural `Одразу до …` / `Далі …` effect sentences, the compact `щохвилини поза боєм або кожен хід в бою (це забирає хвилину дії)` explanation and no visible rank. Each target-list recipient now has one compact status/effect/repeat-wait line, and replacement buttons say `оновити стан`. Combat recovery still settles after the completed hostile response/exchange, and full-HP copy does not claim partially filled mana is full. Fight/Training journal entries and turn-duel/Big Barrel round summaries own their historical Sated snapshot; one shared combat-effect formatter places the same compact `N ходів (+H HP / +M мани)` row under journal effects. Training journal effects now sit under the same heading, identical rows for distinct same-named Big Barrel participants remain distinct, and a real resolved-duel service round-trip retains both stored recovery and the post-round Sated snapshot. Shared recovery follows the hostile response with plain names/values and italic `ситість`, omitting `+0`. Hero and every supported combat surface still show the same exact hidden-rank effect. Active Shynok drink rows bold durations and numeric modifiers. Lazy recovery discovered by Hero is sent once as a separate message rather than embedded in the card. Shynok recovery, ordinary passive regeneration, combat rewards and other stored journals keep their existing contracts. No food/cooking engine, item/economy change, scheduler, rollout flag, location, Prisma model or migration was added.

- The active outside-combat Sated status keeps its duration and recovery explanation on one compact em-dash line.
...
- Structural call-count removal is the release proof. Production latency impact remains unmeasured until the documented post-deploy sampling window is complete.
...
- Added 13 deterministic failure-copy variants for grownup cellar `Домовитись із мишею` attempts, without changing cooldowns, odds, rewards or the paid seal route.
