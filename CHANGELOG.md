...
- Outside combat, complete eligible minutes lazily grant the committed rank's capped HP/mana portion; guarded Character/Cooldown CAS settlement advances the cursor while full, retires the final incomplete fraction exactly at expiry and excludes only actual combat-lease time. Hero reads under a matching lease preserve the frozen remainder. Solo, turn-duel and Big Barrel leases persist the exact logical freeze/start time and paused remaining duration; every fresh combat pulse grants the same rank-scaled portion and consumes exactly one complete frozen minute without subtracting wall-clock delay between actions. Terminal release atomically rebases the post-pulse remainder onto the matching activation, preserves the outside sub-minute cursor and guarded-deletes the lease. Duplicate/replayed pulses and concurrent losing cleanup cannot shorten again, consume later recovery or touch a newer activation. Direct solo and pending-passage first/restart Fight paths now share the same atomic Sated freeze, pre-lease recovery and Character CAS as Training. Later combat damage, spend, healing and pulses remain state-local. The public turn-duel acceptance preflight uses one inclusive-`readyAt` equipment view read-only for passive resource values, warnings, confirmation, natural maxima, stats, set effects and granted actions; only the winning leased repository transaction atomically persists passive/Sated resources and anchors, rolls DEX/LUCK initiative and owns the final snapshot, so a concurrent loser or replay cannot write or reroll. Big Barrel start keeps the same canonical pre-lease-only boundary before raid state owns later resources.
