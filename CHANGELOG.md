...
- Big Barrel recruiting no longer accepts a character whose same-period legacy solo Barrel raid is still pending, including the due-but-unclaimed boundary. The start transaction rechecks that conflict, and the leader's start button now uses the same expired-recruiting recovery allowance as the automatic scheduler instead of showing a misleading stale-deadline rejection. The isolated local-bot manager also treats Windows source paths case-insensitively, preventing `run` and `refresh` from owning duplicate polling processes solely because one command used `D:\...` and another `d:\...`.
- Updated `📖 Перекази` and matching lore references so every class entry reflects shipped combat and side mechanics; the Bard now explicitly covers performance, voluntary tips, payment-independent Inspiration and Big Barrel Lament. The player news now keeps its required in-world closing paragraph, while manual QA and other operational gates stay out of `Ще не відчинено:` copy. Manual Telegram QA remains pending; automated domain, repository, service, presenter and callback coverage owns the implementation evidence.
