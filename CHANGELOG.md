...
- Query-event fixtures record `30` statements for start, `19` for a queued action including replacement lookup and its durable delivery revision, `30` total for two concurrent duplicate last-action attempts, and `1` for the lean due-id scan. Active state stays below `13,000` serialized characters with at most five recap rounds; Telegram cards stay under the platform limit.
