...
- Incoming open and targeted duel cards no longer label their non-mutating preview as `Accept`. The first card now offers details, decline and refresh; the existing detailed confirmation remains the only surface with the actual `Так, прийняти` action. Details, warnings, gates, busy/missing/expired/replayed states and decline preserve location, raid and adventure presence; only a fresh final active/resolved transition moves the player.
