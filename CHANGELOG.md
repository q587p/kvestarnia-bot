...
- Outside combat, complete eligible minutes lazily grant capped `+1 HP / +1 mana`; guarded Character/Cooldown CAS settlement advances the cursor while full, retires the final incomplete fraction exactly at expiry and excludes only actual combat-lease time. Hero reads under a matching lease preserve the frozen remainder. Solo, turn-duel and Big Barrel leases persist the exact logical freeze/start time; every fresh combat pulse shortens the stored combat expiry by one minute, and terminal release atomically writes that shortened expiry back to the matching activation while advancing the cursor once and guarded-deleting the lease. Duplicate/replayed pulses and concurrent losing cleanup cannot shorten again, consume later recovery or touch a newer activation. Direct solo and pending-passage first/restart Fight paths now share the same atomic Sated freeze, pre-lease recovery and Character CAS as Training. Later combat damage, spend, healing and pulses remain state-local. The public turn-duel acceptance preflight uses one inclusive-`readyAt` equipment view read-only for passive resource values, warnings, confirmation, natural maxima, stats, set effects and granted actions; only the winning leased repository transaction atomically persists passive/Sated resources and anchors, rolls DEX/LUCK initiative and owns the final snapshot, so a concurrent loser or replay cannot write or reroll. Big Barrel start keeps the same canonical pre-lease-only boundary before raid state owns later resources.
