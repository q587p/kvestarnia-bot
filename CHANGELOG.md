...
- A production victory now gives every participant the same deterministic 13–23-minute clear left-passage window. Its card exposes the return to the Сутерени above an unavailable `🪜 Ярус II` construction notice; it starts no new location or combat, and `/dev_reset_monster_rest` clears the window for local QA.
...
- Replaced the persistent main menu with a compact six-action battle keyboard for active GroupCombat, moving exact enemy, ability and one-use-item choices into on-demand submenus. Active cards no longer carry the action grid; terminal cards retain only `📜 Журнал` and `📊 Статистика`, then restore the location-aware main menu. `🏃 Відступити` now rolls the ordinary escalating flee chance separately for each participant: success removes only that actor, restores their main menu and forfeits their encounter rewards, while failure leaves them exposed in combat instead of silently becoming guard.
