...
- Made loot v1 a canonical durability boundary backed by a code-owned immutable monster/ability/loot catalog and resolver. Restart and settlement rederive enemy evidence, candidates, weights, exact selected items and quantities from frozen inputs without trusting persisted selection ranges or adjacent public checksums and without current item, monster-loot, monster/profile or generic-loot dependencies. Coherent evidence/output forgery with recomputed legacy SHA-256 values and shape-valid item-id/quantity mutations fail closed in active and terminal rows while legitimate pending settlement survives unrelated content drift.
