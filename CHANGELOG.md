...
- Kept the production GroupCombat opening as a truly separate message that never edits or releases the participant's canonical card reference, so an overlapping first scheduler delivery cannot leave two full first-turn cards. The first authoritative keyboard-generation owner now attaches the current battle reply keyboard to both that opening and the canonical card, so either visible order remains actionable and a stale starter cannot publish a late keyboard-free intro. Active-card and already-delivered compact-reference edits omit an empty inline markup when controls are unchanged, preserving the acknowledged persistent reply keyboard without another participant send; only changed controls or an explicit fresh-card request republish it. The compact reply layout fills two buttons per row, including `📝 Правка на полях` beside `🛡️ Захиститися`.
...
- Replaced superseded GroupCombat action/refresh cards with a new bottom-most canonical card, removed terminal refresh after settlement, simplified the single-living-enemy action to `⚔️ Атакувати`, and kept full contribution details in the separate statistics view.
...
- Replaced the persistent main menu with a compact battle keyboard for active GroupCombat. Every currently available class, race and equipment ability is visible directly, while canonical cooldown, mana and target validation hides unavailable abilities and restores them when legal. Active publication, flee/terminal menu restoration and later combat ownership now share one dedicated durable per-participant publication guard plus the owned combat lease, with stale-claim recovery; an older worker cannot overwrite newer navigation. Acknowledged keyboard fingerprints prevent routine party-wide republishing, and the current reply keyboard is attached to the one fresh canonical card instead of a separate readiness announcement. Frozen labels from older keyboards still reach canonical validation, so an immediate unavailable duplicate commits nothing and produces one fresh bottom-most actor card without a target picker or separate error bubble. Stale GroupCombat reply text with no matching active session delegates to authoritative combat/navigation handling instead of replacing a newer solo, duel, raid or GroupCombat keyboard with a generic menu. One-use items keep their filtered submenu, and a knocked-out participant receives an explicit observer card with only `🔎 Оновити`. Active cards no longer carry the action grid; terminal delivery restores the location-aware main menu first, then atomically adopts a fresh bottom-most result card with only `📜 Журнал` and `📊 Статистика` as that participant's canonical reference. `🏃 Відступити` rolls the ordinary escalating flee chance separately for each participant: success removes only that actor, restores their main menu and forfeits their encounter rewards, while failure leaves them exposed in combat instead of silently becoming guard. Queued copy describes that personal attempt rather than retreat by the whole party.
- Hid `🎒 Разові` whenever no owned supported item is currently useful, matching the ordinary-fight and raid rule instead of opening an empty turn-specific menu. The winning canonical-card CAS now records a reply keyboard that Telegram already accepted, so an acknowledgement failure or scheduler retry edits that adopted card rather than sending another countdown card every five seconds. A sent candidate that loses the durable card claim gets three bounded deletion attempts; permanent Telegram refusal reduces it to one compact inert superseded note instead of leaving a second full battle card. Every successful active or terminal replacement also reduces the previous canonical message to that compact note before attempting deletion, so a rejected delete cannot leave the old full combat card or result beside the new one. If fresh terminal-result publication fails, the previous result is restored instead of disappearing. A deliberately fresh actor card (`/start`, `🔎 Оновити` or a handled reply action) republishes the current keyboard even when its fingerprint is unchanged, so recovery cannot produce a newest card with missing controls. `🔎 Оновити` first persists a participant-specific pending keyboard request; a busy or restarted publisher therefore converges through the bounded delivery queue, and that queue cannot be cleared until a real reply-keyboard fingerprint is acknowledged. Completed terminal navigation now clears obsolete pending-delivery work even when a legacy row's older card revision was never advanced, preventing finished fights from consuming every five-second scheduler pass.
- Bounded every GroupCombat reply-keyboard publication below its durable owner. Active cards renew the exact claim before each Telegram send/edit/delete; exit navigation renews its independent exact claim and lease after resolving current markers and immediately before the main-menu send. After menu acknowledgement, terminal publication retains that same claim and lease through old-card compaction, fresh result send, exact canonical-reference adoption and losing-candidate cleanup; only the final token/reference CAS releases the lease. Each request aborts after 13 seconds, lost ownership sends nothing, ordinary failure releases safely, restart after adoption remains recoverable, and stale takeover or newer combat UI stays last. Only a keyboard actually attached to a private card advances its fingerprint/generation; no-keyboard redirects cannot suppress the next private controls, while terminal result cards remain keyboard-free. Validated manual actions now commit durably before a separate bounded turn-resolution transaction, so process death or UI contention cannot erase a final win/flee action: duplicate or timeout work adopts it exactly once without replaying consumables, rewards, receipts or escape effects. Telegram acceptance immediately before abort/network failure remains an explicit external at-least-once ambiguity because Bot API provides no request idempotency key.
