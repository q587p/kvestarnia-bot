...
- `c006`, `c013` and Fancy Cheese now affect exactly the first eligible enemy response in canonical order and commit only for a positive incremental delta after earlier protection. `c006` never spends for zero prevented damage; evade preserves the chosen enemy action/cooldown while suppressing that owner's damage and harmful on-hit riders. Later multi-enemy responses remain normal, the journal stores truthful selected-target damage/prevention for restart replay, and untargeted or already-fully-protected owners keep the item without use evidence.
