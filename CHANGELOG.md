...
- Newly started turn duels no longer edit an older confirmation or invite into a combat card above fresh Telegram messages. When movement is needed, the callback now delivers the Fighting Corner transition first, the announcement/advice card second and a new combat card last, clears the obsolete confirmation keyboard, and suppresses the generic trailing movement notice. Duel-owned commands, callbacks and deep links pass through combat lock to their canonical handlers; private reopens and stale owner refreshes reuse the participant's recorded card. Action delivery, remote notifications, timeout updates, combat-lock redirects and Rogue retaliation now share one global protocol: replacements are sent inert, compare-and-set against the expected reference, revalidated and activated only by the winner. Retryable Telegram edit errors retain the existing reference, definitive missing-message errors may recover through a fallback, and failed activation releases ownership so duplicate or concurrent delivery cannot leave a second actionable card or publish stale controls after resolution.
