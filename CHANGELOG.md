...
- Nyz passage searches can now rarely replace an already-won bandage item slot with one `Іскрокамінь`, reusing the ordinary-fight bounded `4–6%` replacement policy instead of adding a second item roll. Safe and risky search results remain token-seeded, persist the selected item before presentation and replay without duplicate grants; the result card separates gold from the `Здобуто` item row. Only the winning repository resolution enters the standard `item.received` achievement path, and callback/scheduler completions deliver any unlock once while replays and concurrent losers remain silent.
- Incoming open and targeted duel cards no longer label their non-mutating preview as `Accept`. Invite recipients get details, decline and refresh, while the author's own status card gets only cancel and refresh; stale author-side Details callbacks redraw that neutral owner card instead of showing self-duel copy. Opening `/duel`, creating an open/nearby challenge or rematch, refreshing, viewing details and declining now preserve the current location, raid and adventure presence. Explicit place navigation remains unchanged, and the existing detailed confirmation remains the only surface with the actual `Так, прийняти` action; only a fresh final active/resolved transition moves the player.
- Newly started turn duels no longer edit an older confirmation or invite into a combat card above fresh Telegram messages. When movement is needed, the callback now delivers the Fighting Corner transition first, the announcement/advice card second and a new combat card last, clears the obsolete confirmation keyboard, and suppresses the generic trailing movement notice. Duplicate or concurrent final acceptance leaves the obsolete confirmation inert and may refresh only the already-recorded canonical combat card without replacing its message reference.
