...
- Kept the production GroupCombat opening as a truly separate message that never edits or releases the participant's canonical card reference, so an overlapping first scheduler delivery cannot leave two full first-turn cards. Active-card and already-delivered compact-reference edits now omit an empty inline markup when controls are unchanged, preserving the acknowledged persistent reply keyboard without another participant send; only changed controls or an explicit fresh-card request republish it. The compact reply layout fills two buttons per row, including `📝 Правка на полях` beside `🛡️ Захиститися`.
- Serialized that separate production opening inside the same durable participant claim immediately before the keyboard-bearing canonical card, so command/scheduler overlap cannot leave the plain intro last and make Telegram hide the battle keyboard. Corrected strict exit evidence so repair accepts the normal claimed `menu-delivered` interval before the final CAS; generic combat-lock routing now resumes that exact temporary exit-navigation lease and continues the requested quest/location route instead of repeatedly blocking the whole bot with an owner-mismatch warning.
...
- Made the party action available only for a genuinely pending left-passage preview. A consumed/recovered preview cannot masquerade as a new reservation, and the solo action is labelled explicitly.
