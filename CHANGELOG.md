...
- Outside combat, complete eligible minutes lazily grant capped `+1 HP / +1 mana`; guarded Character/Cooldown CAS settlement advances the cursor while full, retires the final incomplete fraction exactly at expiry and excludes only actual combat-lease time. Hero reads under a matching lease preserve the frozen remainder; every solo lease release atomically claims the matching lease, advances through its logical release time, preserves the outside remainder and deletes the lease, so duplicate releases cannot consume later recovery. Turn-based duel acceptance settles against each Character's natural maxima, persists canonical pre-lease recovery exactly once, then rebuilds the temporary balanced duel snapshot with the existing ratio semantics.
