...
- Kept the persistent main menu available during active GroupCombat and moved the complete current action set onto every canonical live card as inline controls. Every currently available class, race and equipment ability is visible directly, while canonical cooldown, mana and target validation hides unavailable abilities and restores them when legal. Active publication, flee/terminal menu restoration and later combat ownership share one durable per-participant publication guard plus the owned combat lease; an older worker cannot overwrite newer navigation. Frozen labels from older reply keyboards still reach canonical validation, so an unavailable duplicate commits nothing and produces one fresh bottom-most actor card without a target picker or separate error bubble. Stale GroupCombat reply text delegates instead of replacing a newer combat/navigation surface. One-use items keep their filtered submenu, and a knocked-out participant's canonical card contains only `🔎 Оновити`. Terminal delivery restores the location-aware main menu first, then atomically adopts a fresh bottom-most result card with `📜 Журнал` and `📊 Статистика`. `🏃 Відступити` remains a personal ordinary escalating flee roll.
- Hid `🎒 Одноразові манатки` whenever no owned supported item is currently useful. The winning canonical-card CAS records the exact inline-control fingerprint already published, so acknowledgement failure or scheduler retry edits that card instead of sending countdown copies. A losing candidate gets bounded deletion attempts and otherwise becomes one compact inert superseded note; replacement compacts the previous canonical message before delete, and failed terminal publication restores the prior result. `/start`, `🔎 Оновити`, handled actions, restart repair and changed controls all converge to a canonical card with non-empty inline controls without removing the persistent main reply keyboard. Completed terminal navigation clears obsolete delivery work.
- Bounded every GroupCombat control-surface publication below its durable owner. Active cards renew the exact claim before each Telegram send/edit/delete; exit navigation renews its independent claim and lease immediately before the main-menu send. After menu acknowledgement, terminal publication retains ownership through old-card compaction, fresh result adoption and cleanup; only the final token/reference CAS releases the lease. Each request aborts after 13 seconds, ownership loss stops later calls, ordinary failure releases safely, restart after adoption remains recoverable and newer combat UI stays last. Only controls actually attached to a private canonical card advance the legacy fingerprint/generation fields; terminal result cards do not replace the persistent reply keyboard. Validated manual actions commit before bounded turn resolution, so process death or UI contention cannot erase a final win/flee action. Telegram acceptance immediately before abort/network failure remains externally at-least-once.
