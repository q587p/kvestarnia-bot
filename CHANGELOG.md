...
- Newly started turn duels no longer edit an older confirmation or invite into a combat card above fresh Telegram messages. When movement is needed, the callback now delivers the Fighting Corner transition first, the announcement/advice card second and a new combat card last, clears the obsolete confirmation keyboard, and suppresses the generic trailing movement notice. Private `/start` reopens and stale owner refreshes reuse the participant's recorded canonical card. An atomic reference claim records a necessary fallback immediately and disarms concurrent losing cards, so duplicate delivery cannot leave a second actionable combat card or replace the canonical reference.
