...
- A production victory now gives every participant the same deterministic 13–23-minute clear left-passage window. Its card exposes an unavailable `🪜 Ярус II` construction notice above the return to the Сутерени; it starts no new location or combat, and `/dev_reset_monster_rest` clears the window for local QA.
