...
- Added real per-participant settlement for final same-life HP/mana, committed consumables, neutral XP/gold, at most one encounter-wide common bandage, privacy-safe activity and the rewardless first-completion achievement. Each manual participant now commits a durable session/character/type achievement effect with the settlement receipt; bounded idempotent projection retries after restart. Timeout auto-guards create no effect and do not qualify for rewards; each participant lease remains until that participant's canonical receipt/effect transaction commits.
...
- Quarantined operator-required v3 rows before bounded timeout, card delivery, settlement, achievement and repair limits, including orphan-lease repair. They cannot starve healthy work or mutate through stale card delivery, remain unavailable as ordinary combat, emit one diagnostic when quarantined and retain active/terminal raw evidence for explicit operator inspection.
