...
- Review hardening preserves existing raid achievement events when Lament completes a round, makes shared music history life-bound, consumes Inspiration on fatal committed responses, shows frozen Inspiration on `/hero`, rejects generic Lament writes, and keeps the shared timed-status lifecycle replay-safe across every durable combat surface.
