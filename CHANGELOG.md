...
- Outside combat, complete eligible minutes lazily grant capped `+1 HP / +1 mana`; guarded Character/Cooldown CAS settlement advances the cursor while full, retires the final incomplete fraction exactly at expiry and excludes only actual combat-lease time. Hero reads under a matching lease preserve the frozen remainder. Solo, turn-duel and Big Barrel stored state now owns that original remainder for the lease lifetime; terminal, timeout, malformed/orphan and participant-remort release paths atomically advance the matching cooldown once and guarded-delete the lease, so duplicate cleanup cannot consume later recovery or touch a newer activation. Turn-based duel acceptance settles against each Character's natural effective maxima from stored base, level growth and currently attuned equipment, never rewrites base max columns, persists canonical pre-lease recovery exactly once, then rebuilds the temporary balanced duel snapshot with the existing ratio semantics.
