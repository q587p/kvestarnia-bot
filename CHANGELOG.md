...
- Reused `CharacterCooldown` without a schema migration for one versioned current recipient activation/receipt, actor-recipient pair waits and a short server-owned preview proof. Preview binds exact stat/applied rank, cost, effective stats, attuned equipment row/slot/version identity, Shynok recovery windows, target lives, expiry and the observed target activation; confirmation can commit only that plan, never silently upgrade after later regeneration or overwrite a target changed since preview. Current-life duplicates replay the durable current receipt before mutable action gates. A shortened or cleared actor-recipient pair row is authoritative for that caster's next feeding.
