...
- Bandage reservation recovery now avoids false reserved-item blocks: duplicate restore-to-full races replay the canonical live confirmation card, ordinary one-bandage use plus restore-to-full can replace each other's own pending previews, and combat bandage use can replace the player's own pending item-use preview without replaying the previous combat turn as if it happened again.
