...
- Quarantined operator-required v3 rows before bounded timeout, card delivery, settlement, achievement and repair limits, including orphan-lease repair. Initial reads, final reloads and every mutation CAS now fence on the quarantine marker and its version bump, so stale action, timeout, settlement and delivery work cannot mutate or expose the row. Explicit operator inspection alone retains raw active/terminal state, actions, participant effects and receipts. Canonical achievement effects are selected before the bounded projection limit, so malformed older rows cannot starve healthy work.
