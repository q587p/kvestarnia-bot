...
- Big Barrel create/live/live-membership routes now leave every transcript-bearing recruiting card and reopen to the durable raid-chat scheduler. `/raid` and Tavern callbacks never reuse an arbitrary current message for private chat; versioned delivery claims and reference CAS keep one tracked card, retire losing sends, and prevent stale pre-gate snapshots from replacing newer revisions.
