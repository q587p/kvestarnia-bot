...
- Outside combat, complete eligible minutes lazily grant capped `+1 HP / +1 mana`; guarded Character/Cooldown CAS settlement advances the cursor while full, retires the final incomplete fraction exactly at expiry and excludes only actual combat-lease time. Hero reads under a matching lease preserve the frozen remainder. Solo, turn-duel and Big Barrel starts persist exact pre-lease Sated recovery and full-resource anchors to `Character`, then store the exact logical freeze/start time; later combat damage, spend, healing and pulses remain combat-local. Terminal, timeout, early malformed cleanup at the observed clock, orphan and participant-remort release paths atomically advance the matching cooldown once and guarded-delete the lease, so duplicate or concurrent losing cleanup cannot consume later recovery or touch a newer activation. The public turn-duel acceptance path uses the same inclusive-`readyAt` active-equipment view for resource synchronization, warnings, confirmation, natural maxima, stats, set/grant effects and initiative; only the winning transactional claim rolls initiative and builds the final ratio-balanced snapshot without rewriting base max columns.
