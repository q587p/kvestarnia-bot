...
- Big Barrel Brother recruiting cards now keep `🔎 Оновити` beside the ready/waiting toggle, hide `🧹 Скасувати збір` once at least two participants have joined, show `📣 Картка запрошення` and `🔗 Запросити на рейд` on one row, and keep `🛢️ Почати рейд` at the bottom of the card.
