...
- Updated `📖 Перекази` and matching lore references so every class entry reflects shipped combat and side mechanics; the Bard now explicitly covers performance, voluntary tips, payment-independent Inspiration and Big Barrel Lament. Manual Telegram QA remains pending; automated domain, repository, service, presenter and callback coverage owns the implementation evidence.
