...
- The custom crest route now accepts exactly one genuine normalized emoji, canonicalizes presentation aliases to one reservation key, and rejects lone flag halves or non-emoji text decorated with variation selectors. Unsupported media and links reissue the exact prompt only after current authority/version validation; occupied, textual, multiple-emoji, forged and stale replies recover without intent/profile/gold/cooldown/audit mutation, while saved pre-release photo prompts and view callbacks stay inert.
