...
- Nyz passage searches can now rarely replace an already-won bandage item slot with one `Іскрокамінь`, reusing the ordinary-fight bounded `4–6%` replacement policy instead of adding a second item roll. Safe and risky search results remain token-seeded, persist the selected item before presentation and replay without duplicate grants; the result card separates gold from the `Здобуто` item row.
- Incoming open and targeted duel cards no longer label their non-mutating preview as `Accept`. The first card now offers details, decline and refresh; the existing detailed confirmation remains the only surface with the actual `Так, прийняти` action, preserving resource warnings and exact-once duel resolution.
- Newly started turn duels no longer edit an older confirmation or invite into a combat card above fresh Telegram messages. When movement is needed, the callback now delivers the Fighting Corner transition first, the announcement/advice card second and a new combat card last, clears the obsolete confirmation keyboard, and suppresses the generic trailing movement notice; ordinary active-card refreshes still edit in place.
