...
- Added one forward-only schema migration for append-only PartyBoss round history. No production flag, dependency, quest, reward or new achievement definition was added. Lore needs no update because the change fixes existing combat support targeting and lifecycle safety rather than world canon.
- Final repository verification passed 314 unit files / 3923 tests, 23 integration files / 543 tests (including a cache-disabled repeat), and all 56 PartyBoss reducer/simulation tests. Target deployment, migration application, flags, backup/restore, Telegram QA and observation remain unverified.
