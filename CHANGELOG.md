...
- Bounded live GroupCombat UI publication below its durable lease: the exact owner renews atomically before each Telegram send/edit/delete and every call aborts after 13 seconds, so stale takeover, terminal/flee navigation or a newer combat prevents all later old-worker calls. Only a keyboard actually attached to a private card advances its fingerprint/generation; no-keyboard redirects cannot suppress the next private controls. Terminal wins, successful flee and timeout wait through a live UI fence without spending mutation retries or returning a false changed-turn result. Telegram acceptance immediately before abort/network failure remains an explicit external at-least-once ambiguity because Bot API provides no request idempotency key.
