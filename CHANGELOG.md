...
- Review hardening makes delivery acknowledgements revision/reference-CAS safe, treats Telegram `message is not modified` as success, parks permanent send/edit failures and retries only transient/`429` failures. Existing revoke, composer cancellation, terminal retention and redaction recovery continue while fresh chat events are disabled; disabled startup invalidates old compose prompts, same-life rejoins re-arm delivery with unique transition keys, and remort terminalizes only the actor's canonical due Big Barrel membership without sweeping unrelated sessions. Dirty/redaction delivery bypasses clean due rows, replacement references do not re-wake clean work, and remort leader-transfer keys include the winning transition version. Media, captions and forwarded updates remain with existing handlers, the dev fill stays at the 130-row hard cap, and visible waits preserve exact remaining seconds.
