...
- Newly created Big Barrel gatherings now assign their first recruiting-card publication exclusively to the durable raid-chat scheduler. The creation route still sends the separate approach notice but no longer races the scheduler into producing two identical gathering cards; a deterministic interleaving regression asserts one total card.
