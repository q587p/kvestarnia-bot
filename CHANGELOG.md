...
- Replaced the persistent main menu with a compact battle keyboard for active GroupCombat. Every currently available class, race and equipment ability is visible directly; only an exact target picker opens when an ability has several legal targets, while one-use items keep their filtered submenu. A knocked-out participant receives an explicit observer card with only `🔎 Оновити`. Active cards no longer carry the action grid; terminal cards retain only `📜 Журнал` and `📊 Статистика`, then restore the location-aware main menu. `🏃 Відступити` now rolls the ordinary escalating flee chance separately for each participant: success removes only that actor, restores their main menu and forfeits their encounter rewards, while failure leaves them exposed in combat instead of silently becoming guard.
