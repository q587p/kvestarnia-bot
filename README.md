...
- `/tavern` і `/raid` відкривають малий solo-рейд «П’ятничний рейд на Бочку Пінного Міражу» з винагородою раз на локальний день; у залі корчми є `🍻 Всім пива`, що працює тільки після рейду й списує золото лише після явного вибору якості.
...
9. У залі корчми натисніть `🍻 Всім пива`: до рейду корчмар має відмовити, після рейду має показати вибір `🍻 Якісне — 100` або `🍺 Просте — 10`; золото списується тільки після вибору якості.
