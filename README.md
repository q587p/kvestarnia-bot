...
Квестарня - україномовна текстова Telegram RPG. Репозиторій містить TypeScript/Node.js foundation для Telegram-бота на grammY, Prisma/SQLite local baseline, Zod-validated content і перші Phase 1 зрізи: ідемпотентний `/start` onboarding, малий корчемний рейд, пригоду з міміком-шаурмою, combat probe, persistent манатки й repeatable підвальне доручення.
...
- Коли денна шаурма й fight probe уже витрачені, `/quest` веде до repeatable активності «Підвальна справа» з маленькою винагородою і коротким SQLite cooldown.
...
9. Перейдіть до `📋 Стіл зі справами`, відкрийте `/adventure` або `/quest`.
10. Оберіть одну дію проти `Міміка-шаурми`: тицьнути, попросити чек або відступити.
11. Відкрийте `/fight` або `/hunt`.
12. Оберіть одну дію в «Сутичці з Міміком-шаурмою»: вдарити, збити з пантелику чеком або відступити красиво.
13. Коли денні шаурма й сутичка витрачені, відкрийте `/quest` або кнопку `🗺️ Квест` усередині корчми: має з’явитися «Підвальна справа».
14. Оберіть підвальну дію й одразу повторіть її: cooldown має не дублювати XP/золото.
15. Перевірте `/hero`: XP, золото, рівень, HP, мана й головна характеристика класу мають показати новий прогрес.
16. Перевірте `/inventory`, `/items` або `/bag`: перші манатки мають з’явитися в торбі.
17. Натисніть той самий рейд, пригоду або сутичку ще раз і переконайтесь, що XP, золото й кількість предметів не дублюються.
18. Перевірте `/version` і `/news`, щоб побачити поточну версію, останню новину й архів.
19. Щоб почати героя з початку, виконайте `/restart` і підтвердьте видалення.
20. Для локальних dev-перевірок також доступний `/dev_reset_me`.
...
`Підвальна справа` зараз є першим repeatable fallback-ом, а не повним quest hub: `/quest` переходить до неї після витраченої денної шаурми й сутички, cooldown зберігається в SQLite, а Redis не потрібен.
