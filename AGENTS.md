...
18d. An active combat must always converge to a visible authoritative reply keyboard. Every private message that can become the newest combat UI must carry the current legal combat keyboard or be followed, under the same durable publication ownership, by a keyboard-bearing canonical card. A living actor receives the currently legal actions; a knocked-out or observing participant receives at least `🔎 Оновити`. Intro, refresh, scheduler, retry, error and concurrent-delivery paths must never leave an active combat lease without a battle keyboard. Cover initial delivery, stale/restarted delivery and observer-only delivery with exact `reply_markup` assertions.
