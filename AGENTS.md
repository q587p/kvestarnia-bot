...
12. In visible docs/changelog/news/player dates, use the Holocene calendar: `12026`, not `2026`. This includes player-facing period labels, board dates, tournament keys, leaderboard periods, reward periods, and other visible ledger IDs; storage keys, callback data, migration names, machine timestamps, and technical IDs may remain Gregorian when the code contract needs them, but must be converted before display. Release/news/changelog date headings use Kyiv time (`Europe/Kyiv`) and the fixed Holocene `1YYYY-MM-DD` format, for example `12026-06-20`. Before calling a release PR ready, compare the current Kyiv date with the latest technical release/changelog heading and refresh it if the last implementation commit landed on a newer Kyiv day. Do not advance an existing `news.md` heading merely because technical fixes, review follow-ups, CI reruns or documentation commits happened later; change that player-facing date only when the release promise/news entry itself changes or is actually republished. Do not rewrite machine timestamps, migrations or historical release/news dates.
...
- Every implementation or PR-follow-up commit on a later Kyiv day than the latest technical release/changelog heading must refresh that technical date before the branch is called ready. Preserve the existing `news.md` date for technical fixes, review follow-ups, CI reruns and documentation-only changes; update it only for a changed player-facing release promise or actual republication.
