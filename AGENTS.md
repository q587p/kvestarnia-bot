...
Будуємо україномовну текстову Telegram RPG з емоджі та легкими ілюстраціями: просту на вході, смішну в тоні, але глибоку в прогресії. Натхнення: настільні RPG, Munchkin, книжкова серія MythAdventures Роберта Аспріна, YouTube-серіал Epic NPC Man від VLDL, класичні MMORPG, гумор Террі Пратчетта, українські меми та фольклор без кальки з російського контенту.
...
7. У видимих датах для документації, changelog/news і гравецьких текстів використовувати Holocene calendar: `12026`, а не `2026`. Машинні timestamp-и, назви міграцій і технічні id не переписувати.
8. Заголовки версій у `CHANGELOG.md` і `news.md` мають містити номер, дату й короткий опис зміни, а не лише номер версії з датою.
9. Назви PR для версійних gameplay/runtime змін мають починатися з номера версії й короткого опису з `CHANGELOG.md`, наприклад `0.0.4 — First Mimic Shawarma Adventure`.
10. Docs-only / presentation зміни не є номерним релізом: не bump-ати `package.json`, не оновлювати `CHANGELOG.md`/`news.md`, не створювати git tag або GitHub Release, якщо користувач прямо не попросив.
...
12. Після відкриття або оновлення PR перевіряти mergeability/конфлікти проти base branch. Якщо PR має конфлікти, одразу підтягнути base branch і виправити їх або явно пояснити, що блокує resolution.
13. Дотримуватись канонічного неймінґу: `Квестарня` для гравців, `kvestarnia` для технічних ідентифікаторів. Не використовувати `Questarnia`, `Квестарнія`, `Kvestarnya`.
14. Не вставляти секрети, токени, приватні chat_id або реальні ключі в код/доки.
15. Не ламати існуючі міграції. Нові зміни схеми — через нову міграцію.
16. Усі ігрові розрахунки мають бути детермінованими та тестованими: бойова логіка не повинна залежати від Telegram API.
17. Повідомлення Telegram тримати короткими: 1 екран на мобільному, кнопки для дій, деталі — за запитом.
18. Не робити pay-to-win механіки. Монетизація може давати косметику, комфорт або підтримку сервера, але не безчесну бойову перевагу.
19. Після зміни логіки запускати тести або пояснити, чому не вдалося. Для docs-only змін прийнятно писати `Not run — docs-only change`.
