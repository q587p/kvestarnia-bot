...
21. Не робити pay-to-win механіки. Монетизація може давати косметику, комфорт або підтримку сервера, але не безчесну бойову перевагу.
22. Після зміни логіки запускати тести або пояснити, чому не вдалося. Для docs-only змін прийнятно писати `Not run — docs-only change`.
