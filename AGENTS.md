...
23. When a task introduces a new environment variable, update both `.env.example` and the existing untracked root `.env` in the same implementation pass. Preserve all secrets and unrelated local values; never stage or commit `.env`. Keep deploy-safe defaults in `.env.example`, but put the explicit non-production value required to exercise the requested manual flow in local `.env`. For the isolated local bot, remember that the running snapshot has its own copied `.env`: when the user asks Codex to make the feature available for testing, run `refresh-local-bot.cmd` and verify `status-local-bot.cmd`, the exact snapshot SHA and the relevant runtime `.env` keys. Otherwise, state the exact refresh command at handoff instead of implying the checkout `.env` changed the running bot.
23a. When the user asks to prepare a gameplay change for local/manual testing, update the existing untracked root `.env` with every non-secret value needed by that exact QA flow, including already-existing feature flags; do not merely list the values in docs or the PR body. Preserve secrets and unrelated local values, never stage `.env`, keep production/deploy defaults unchanged, and report whether the isolated runtime still needs an explicit refresh.
