...
- If an active PR already exists for the current work, add follow-ups to that same branch and PR unless the user explicitly asks for a separate branch or a separate PR. Before opening any new PR, check whether the current task belongs to an already-open PR and update that PR instead when it does.
