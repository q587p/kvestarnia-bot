...
- Keep each new/current `news.md` release entry in the established `0.3.4`-and-earlier shape: one short lead paragraph, then `У грі вже:` with a Markdown bullet list of visible changes, followed only by an optional short closing paragraph or one short `Ще не відчинено:` line. Do not publish a paragraph-only release entry. Preserve this shape with a repository test against the newest entry.
