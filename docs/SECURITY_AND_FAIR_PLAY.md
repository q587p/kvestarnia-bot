...
`0.2.17` Big Barrel Brother reuses the `0.2.15`/`0.2.16` party source of truth behind `BIG_BARREL_BROTHER_RAID_ENABLED`; disabling the flag blocks new Big creation but must not make existing Big sessions unrecoverable. Big sessions are identified by the frozen party origin and stored `rulesVersion = big-barrel-brother-v1`; action callbacks remain token/turn/action hints only, while server state owns actor, resources, cooldowns, contribution and terminal result. Runtime has no hidden round cap, enrage timer or automatic loss by turn number: round 13 is only a balance/QA horizon. Victory settlement uses the existing `tavern.friday-barrel-raid` daily-action key for the frozen Barrel period as the exactly-once success/reward guard, then applies stored XP/gold/current Barrel item grants only for meaningful matched participants. Failure writes no Barrel success, gold or items. Terminal replay, callback retry and process restart must converge on the stored result, release `party-boss` leases/live party keys, and never let one character collect legacy plus Big Barrel success in the same period.
