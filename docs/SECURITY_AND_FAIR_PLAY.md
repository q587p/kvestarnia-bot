...
`0.1.24` Shynok economy mutations are token-scoped and replay-safe. Self-drink and round confirms spend gold only inside guarded transactions and replay completed tokens. Round confirms use the stored recipient snapshot, not a fresh presence list, and recipient offers accept/decline at most once. Manatka sale confirm rereads live inventory, equipment and reservations inside the transaction, recomputes the sale fingerprint/value/payout from current content and returns `stale-selection` without mutation on drift; completed sale replay cannot grant gold or consume items again. Telegram delivery of round/private cards is best-effort and never the source of truth.
