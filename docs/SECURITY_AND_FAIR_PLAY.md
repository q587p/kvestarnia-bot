...
`0.2.13` postal delivery extends Safe Gifting without becoming a market or mailbox. Known recipients come only from completed item-transfer history in either direction; selector and cards must not reveal current location or online status. Postal drafts use opaque `v1:post:` tokens and store package lines server-side. Confirm accepts only 1-5 distinct eligible `itemId` stack types, 1-93 units each, and turns the draft into one live `postal:<sender>` reservation that reserves every package line for sale/chest/barter/use/gift/remort paths. Recipient accept rechecks sender/receiver life, active combat, ownership, equipment, transfer tags/fingerprints, quantities, competing reservations and sender fee gold inside one transaction; if any line or the fee fails, no item moves and no fee is charged. The delivery fee is a gold sink charged from the sender at recipient acceptance, not a player-to-player gold transfer. Terminal callbacks replay stored package state and best-effort notices are not the source of truth.
