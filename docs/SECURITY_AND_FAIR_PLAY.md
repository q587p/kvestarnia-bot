...
The final `0.1.20` hardening extends the same replay/audit contract to cooldown-backed cellar results through `character_cooldowns.result_json`. Replays must use the stored cooldown result/audit rather than rerolling. HP rollback compensates only the committed loss and caps to the canonical max, so unrelated later resource changes are not overwritten. If persistent-fight handoff returns an existing unrelated active fight instead of a newly started adventure fight, the quest claim is rolled back and the active fight is shown.
