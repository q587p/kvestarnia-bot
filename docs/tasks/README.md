...
- [0.3.7-warrior-raid-taunt.md](0.3.7-warrior-raid-taunt.md) — active release candidate for the narrow Warrior Big Barrel Brother raid taunt slice: a living joined Warrior can spend their queued raid action to redirect personal and broad boss attacks into themselves for exactly three boss responses, with a five-turn cooldown and no generic threat system. A manual pass found stale recruiting-card delivery; one boss-first canonical gate now covers actor and every stored participant card across preparation/view/deep-link races with manual or scheduled start, while latest-head targeted Telegram recheck, merge and deploy remain unproven.
- [0.3.x-varenyk-mancer-sated-support.md](0.3.x-varenyk-mancer-sated-support.md) — planning draft and candidate `0.3.8` after Warrior Raid Taunt: level 3+ Varenyk-mancers can feed themselves or an active same-location recipient to create short schema-free `😋 Ситий` HP/mana sustain across lazy out-of-combat minutes and stored combat turns, behind an off-by-default rollout flag.
