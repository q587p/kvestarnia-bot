...
- [0.3.7-warrior-raid-taunt.md](0.3.7-warrior-raid-taunt.md) — active release candidate for the narrow Warrior Big Barrel Brother raid taunt slice: a living joined Warrior can spend their queued raid action to redirect personal and broad boss attacks into themselves for exactly three boss responses, with a five-turn cooldown and no generic threat system. A manual pass found stale leader-card delivery; serialized canonical refresh/fallback fixes followed, so the latest-head targeted Telegram recheck, merge and deploy remain unproven.
