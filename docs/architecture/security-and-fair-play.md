...
`0.3.12` Varenyk-mancer feeding reuses `CharacterCooldown` without a schema change. The short actor-owned preview stores the exact affordable applied rank/cost, effective stat and attuned-equipment snapshot, Shynok recovery windows, target/remort identities and expiry; rank stays internal in player copy. Confirmation settles canonical resources but may commit only that stored plan; changed equipment/attunement/Shynok identity is stale, lost exact affordability is insufficient mana, and later regeneration cannot silently upgrade the rank. The committed rank deterministically owns both the immediate HP amount and later minute/own-turn HP/mana portion (`1/1`, `2/1`, `2/2`, `3/2`, `3/3` for ranks 1-5), while player copy exposes only exact effects. The recipient row is claimed and finalized with guarded JSON/timestamp CAS in the same transaction as mana spend, immediate HP-only recovery and receipt creation; mana begins only with a later eligible minute/turn pulse. Sated never stacks: each wait belongs to the exact actor-recipient pair, so another Varenyk-mancer may replace an active status immediately, while the same caster may replace it only after that pair wait genuinely ends, is shortened to expiry or is explicitly cleared by isolated QA tooling. Every replacement uses a fresh server-owned preview and may select a new internal affordable rank without exposing rank in player copy. Existing payloads settle through expiry before refeed/combat freeze. Durable combat state freezes the remaining duration from the settled cursor rather than subtracting an incomplete outside minute, preserves that sub-minute progress separately, pauses wall-clock duration, consumes exactly one complete status minute per fresh pulse and rebases only the matching activation on lease release; duplicate/replay/losing CAS paths cannot consume duration twice and roll back without notification or achievement.
...
`0.2.16` party-vs-one-boss is still a dev/flag-gated proof, not Big Barrel Brother or the production raid route. It reuses `party_sessions` / `party_participants`; no second party abstraction is authoritative. Starting a proof freezes the joined roster into `party_boss_sessions.state_json`, inserts one `party-boss` `ActiveCombatLease` per participant and stores one shared boss target. Participant action callbacks carry only opaque party token, turn hint and compact action key; server state owns actor, turn, eligibility, HP/mana, cooldowns and result. `party_boss_actions` is unique by `(sessionId, turn, actorCharacterId)`, so duplicate/stale action buttons cannot double-spend mana, cooldown, RNG, contribution or boss HP; knocked-out participants replay canonical state instead of creating useful action rows. Missing actions resolve only through deterministic timeout defend fallback. Shared/spectator cards must not expose private HP, mana, selected actions, Telegram ids or hidden rewards. Runtime proof completion has no hidden round cap: it stores result JSON once, releases all `party-boss` leases and clears live party keys only when the boss reaches 0 HP or all active participants are knocked out. Remort during an active proof cancels that proof and clears the same live keys/leases without rewards. This proof grants no XP, gold, items, manatky, achievements, Barrel success, serious raid reward or economy faucet.

`0.2.17` Big Barrel Brother reuses the `0.2.15`/`0.2.16` party source of truth behind `BIG_BARREL_BROTHER_RAID_ENABLED`; disabling the flag blocks new Big creation but must not make existing Big sessions unrecoverable. Big sessions are identified by the frozen party origin and stored `rulesVersion = big-barrel-brother-v1`; action callbacks remain token/turn/action hints only, while server state owns actor, resources, cooldowns, contribution and terminal result. Runtime has no hidden round cap, enrage timer or automatic loss by turn number: round 13 is only a balance/QA horizon. Big start rejects the whole joined roster with a generic ineligible result if any participant fails the remort-aware gate (non-remorted level `8+`, or remorted level `3+`), is in another active combat lease, already completed the frozen Barrel period, is still on the 3-minute `tavern.big-barrel-brother.loss-retry.cooldown`, or no longer matches the stored remort life; shared cards do not disclose the private reason. Active participant battle cards may show the raid party's current HP/mana and the boss target marker because those resources are part of the shared raid state; they must still not expose Telegram ids, hidden eligibility reasons, exact future reward formulas or current selected actions before resolution. The battle journal opens only after terminal state with stored action outcomes, target rows and focus-switch notes. `/start party_<token>` deep links are still opaque-token routes; after start or terminal they may replay the active boss card or stored result before recruiting fallback, but must not expose DB ids, Telegram ids, hidden eligibility reasons or exact future reward formulas. Victory settlement uses the existing `tavern.friday-barrel-raid` daily-action key for the frozen Barrel period as the exactly-once success/reward guard, then reapplies the level/remort gate, remort-life and existing-success checks before stored XP/gold/current Barrel item grants for meaningful matched participants. Big retaliation target selection is server-owned: ordinary hits start on the party leader, then follow previous-round top damage among living participants, while every fourth turn hits all living participants until a future threat/taunt system replaces this temporary rule. Production timeout callbacks and the active-turn scheduler may fill missing actions only when `turnExpiresAt <= now`; early force-timeout is a dev-helper-only path. Failure writes no Barrel success, gold or items, but meaningful non-AFK participants may receive small replay-safe attempt XP and then cannot create or join another Big Barrel Brother raid until the short loss retry cooldown expires; timeout-only auto-defend is not meaningful participation. Terminal replay, callback retry and process restart must converge on the stored result, release `party-boss` leases/live party keys, and never let one character collect legacy plus Big Barrel success in the same period.

`0.3.2` Kharakternyk ward signs reuse the same party-session source of truth without adding public ids or a new schema table. Placement/support callbacks carry only the opaque party token and server-owned actor identity; the repository verifies recruiting Big Barrel status, joined membership, current remort life, class/level placement eligibility and mana before writing participant snapshot metadata. Placement first wins a session compare-and-swap guard before mana spend, so concurrent eligible placers cannot commit two signs or charge the losing placer. Duplicate placement/support callbacks replay the existing sign/support and cannot spend mana twice. Boss start recomputes the effective sign from the final joined roster, so stale left/remorted placers or supporters may remain in snapshot history but do not affect combat. The active boss state owns the frozen mitigation, remaining support charges and carried/broken status; refresh, journal replay and process restart must not recalculate or duplicate already-resolved sign activations.
...
`Банка підтримки Квестарні` є добровільною підтримкою, а не payment-to-gameplay integration. У `0.1.1` є тільки link plumbing: optional `SUPPORT_JAR_URL`, `/support`, secondary homepage block and `/start support_thanks`. Канонічний backlog: [SUPPORT_JAR_BACKLOG.md](../backlog/support-jar-backlog.md). Майбутній read-only live status через Monobank API описаний окремо: [SUPPORT_JAR_LIVE_STATUS.md](../operations/support-jar-live-status.md).
