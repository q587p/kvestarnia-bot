...
`0.3.2` Kharakternyk ward signs reuse the same party-session source of truth without adding public ids or a new schema table. Placement/support callbacks carry only the opaque party token and server-owned actor identity; the repository verifies recruiting Big Barrel status, joined membership, current remort life, class/level placement eligibility and mana before writing participant snapshot metadata. Duplicate placement/support callbacks replay the existing sign/support and cannot spend mana twice. Boss start recomputes the effective sign from the final joined roster, so stale left/remorted placers or supporters may remain in snapshot history but do not affect combat. The active boss state owns the frozen mitigation and broken status; refresh, journal replay and process restart must not recalculate or retrigger the same sign.
