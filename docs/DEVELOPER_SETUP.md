...
- `/dev_add_iskrokamin [число]` — додає Іскрокамені для локальної перевірки Чароковальні; без числа додає один.
- `/dev_set_item_plus itemId=<item.id> level=0..5` — переводить один екземпляр цієї манатки у відповідний `+N` itemId для перевірки Чароковальні.
- `/dev_set_upgrade_pity itemId=<item.id> target=1..5 failures=0..5` — виставляє жаль-лічильник підсилення.
- `/dev_complete_upgrade_order` — робить активні замовлення Чароковальні готовими до спроби.
- `/dev_cancel_upgrade_order` — скасовує активні замовлення Чароковальні.
