...
Manual Telegram QA status for the implementation pass: partial local smoke found and fixed hidden blocked gear-action buttons after equipping a grant manatka, then found and fixed active-fight gear swaps and active fight overview cards not refreshing the gear-action buttons; full manual pass still pending.
