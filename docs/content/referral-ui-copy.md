...
Усі динамічні імена проходять referral-specific sanitization і фінальне HTML escaping. Готовий share draft навмисно показує власне поточне імʼя запрошувача, активний косметичний титул і живу ґільдію, якщо вони є. У решту текстів і журналів не потрапляють Telegram username/ID, guild custom emoji identifiers, destination chat або raw token; токен дозволений лише всередині контрольованого deep/share URL, який бот показує власникові поклику.
...
Окремий екран є готовим до пересилання повідомленням: показує один із 13 різних коротких українських текстів, жирне безпечне імʼя запрошувача, активний титул і живу ґільдію за наявности, а також саме стабільне referral-посилання. Кнопка `🎲 Перегенерувати текст` переходить до наступного варіянта, але номер варіянта й службове пояснення в повідомленні не показуються.
...
«<b>ІМʼЯ ПЕРСОНАЖА</b>» лишає тобі поклик до Квестарні. Створи пригодника — корчма вже сперечається, який кухоль вважати вітальним.
Титул: <i>«АКТИВНИЙ ТИТУЛ»</i>
Ґільдія: 🐉 <b>НАЗВА ҐІЛЬДІЇ</b>

🔗 <a href="https://t.me/BOT_USERNAME?start=ref1_TOKEN">https://t.me/BOT_USERNAME?start=ref1_TOKEN</a>
```

Це перший із 13 канонічних варіянтів; титул і ґільдійні рядки зникають, якщо відповідної identity немає. Кнопка `📨 Поділитися запрошенням` використовує Telegram Share URL і передає те саме стабільне referral-посилання. Гравець може просто переслати готову bot-картку або сам обрати чат/групу/канал і змінити текст. Бот не зберігає destination.
