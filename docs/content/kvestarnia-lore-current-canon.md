...
- **Козак-характерник** (`race.kharakternyk`) — legacy / не для нового onboarding. Deprecated fallback для старих персонажів. Нові характерники тепер обираються як клас.

## Класи

- **Воїн** (`class.warrior`) — primary stat `strength`. Простий план: стояти рівно й переконливо махати залізом.
...
- **Бюрокромант** (`class.bureaucramancer`) — primary stat `intelligence`. Знерухомлює ворогів формами, печатками й дуже серйозним виглядом.
