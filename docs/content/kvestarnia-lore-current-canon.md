...
- **Воїн** (`class.warrior`) — primary stat `strength`. Простий план: стояти рівно й переконливо махати залізом; у рейді проти Старшого Брата Бочки може гукнути `🛡️ На мене!` й прийняти гуркіт на себе.
- **Маг** (`class.mage`) — primary stat `intelligence`. Каже складні слова, після яких у кімнаті стає гарячіше.
- **Бард** (`class.bard`) — primary stat `charisma`. Перемагає харизмою, куплетами й небезпечною впевненістю.
- **Злодій** (`class.rogue`) — primary stat `dexterity`. Зникає швидше, ніж рахунок після походу в таверну.
- **Жрець** (`class.priest`) — primary stat `charisma`. Лікує союзників і суворо дивиться на нежить.
- **Вареник-мант** (`class.varenyk-mancer`) — primary stat `intelligence`. Керує тістом і від 3 рівня може нагодувати себе або активного пригодника поруч: HP підбадьорюється одразу, а короткий `😋 Ситий` потім підтримує HP і ману щохвилини поза боєм або після завершення бойового обміну; кожна бойова порція забирає хвилину ситости.
- **Бюрокромант** (`class.bureaucramancer`) — primary stat `intelligence`. Знерухомлює ворогів формами, печатками й дуже серйозним виглядом; перед рейдом на Старшого Брата Бочки може відкрити вузький Протокол 13-З для персональних претензій підписантів.
- **Єгер** (`class.ranger`) — primary stat `dexterity`. Знає стежки, пастки й де ховається ваша остання стріла.
- **Козак-характерник** (`class.kharakternyk`) — primary stat `luck`. Дивиться на проблему так, що проблема сама шукає собі іншу пригоду; біля Старшого Брата Бочки може поставити рейдовий знак, який ватага підпирає до першого великого гуркоту.
