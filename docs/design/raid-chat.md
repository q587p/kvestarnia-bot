...
6. Reject empty input, forward/media/caption input and Telegram entities `url`, `text_link`, `email`, `phone_number`, `mention` and `text_mention`. Command updates were already passed to the command router rather than treated as submitted chat.
