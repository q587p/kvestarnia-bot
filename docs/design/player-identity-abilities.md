...
| Бюрокромант | `📄 Форма 13-Б` (`skill.form-thirteen-b`): all enemies plus response reduction, social/control, 4 мани, 3 cooldown. | Class-specific adventure offers; paperwork/authority method flavor. In Big Barrel Brother recruiting, level 3+ Bureaucramancers can file narrow `Протокол 13-Б` so joined signers block their first personal boss slap. | Forms, permits and queue manipulation as social techniques; keep callbacks server-owned. Protocol 13-Б is a raid-prep exception, not a generic permit framework. |
