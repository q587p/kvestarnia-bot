...
| Вареник-мант | `🥟 Кипляча начинка` (`skill.boiling-filling`): all enemies plus small self/ally fallback heal, spell, 4 мани, 2 cooldown. | `0.3.12` `🍽️ Нагодувати`: level 3+ self/active exact-location support with an internal deterministic affordable rank, an exact server-owned preview plan, immediate HP only and short `😋 Ситий` HP/mana sustain outside combat and in durable turns. The committed rank scales each minute/own-turn HP/mana portion (`1/1`, `2/1`, `2/2`, `3/2`, `3/3`), combat pauses wall-clock duration, an incomplete outside minute does not remove an initial combat turn and each fresh combat pulse spends exactly one complete status minute; player copy shows exact effects while hiding rank. `Ситий` never stacks: another Varenyk-mancer may replace it immediately, while the same caster must wait until the exact actor-recipient pair wait ends, is shortened to expiry or is explicitly cleared by isolated QA tooling. Replacement uses a fresh preview and may choose a new internal affordable rank. | Keep food magic scoped; no broad cooking, ingredients or crafting engine. |
| Бюрокромант | `📄 Форма 13-А` (`skill.form-thirteen-b`): all enemies plus response reduction, social/control, 4 мани, 3 cooldown. | Class-specific adventure offers; paperwork/authority method flavor. In Big Barrel Brother recruiting, level 3+ Bureaucramancers can file narrow `Протокол 13-З` so joined signers block their first personal boss slap. | Forms, permits and queue manipulation as social techniques; keep callbacks server-owned. Protocol 13-З is a raid-prep exception, not a generic permit framework. |
