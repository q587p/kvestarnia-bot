...
| 0.4.3 consumable manatka uses | no; implementation branch | no migration required | no target evidence; `CONSUMABLE_MANATKA_USES_ENABLED` defaults off and gates only 17 nonmedical rows | yes; static + 4,306 unit + 694 integration tests, focused suites, docs and combat simulator pass (integration rerun with cache disabled after Windows cache-lock EPERM) | one-account ordinary/out-of-combat and three-account Party Boss/GroupCombat matrix pending | `CONSUMABLE_MANATKA_USES_ENABLED`; deploy rollback; release operator unassigned | deferred |
