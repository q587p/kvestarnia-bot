...
| 0.3.16 closeout safeguards/report/docs | pending merge | `20260721113000_party_boss_round_history`; target deploy unknown; legacy leader, exact active-zero-HP normalization and scalar-safe reconstruction covered locally | no new production flag; Big Barrel target value unknown/default off | `db:validate`; static checks; 316 unit files / 3956 tests; 23 integration files / 552 tests, repeated without cache; 56 PartyBoss reducer tests + 5 targeted simulation corridors | target matrix pending | existing Big Barrel flag + deploy rollback; release operator unassigned | deferred |
