...
| GroupCombat SQLite delivery hotfix | no; implementation branch | no migration required | no new flag | focused actor-first/serial-delivery, partial-`P1008`, retry-window and repository integration tests pass | three-account last-ready plus two-turn regression pending on exact hotfix head | deploy rollback; release operator unassigned | deferred |
