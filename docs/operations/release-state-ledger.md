...
| 0.4.2 left-passage party attack | no; draft branch only | no; branch includes unchanged additive `20260724233000_left_passage_party_attack` with durable v3 operator marker/reason and participant achievement-effect receipts; this follow-up adds no migration; rollback requires flag-off, healthy drain, explicit quarantine disposition and pre-migration restore | no target evidence; repository default off | focused 13 files / 351 plus 4 files / 52; full 4,092 unit + 643 integration, docs 416/249, Prisma validation/generation and simulator passed; action/timeout/settlement/delivery quarantine barriers, ordinary-winner re-read fence, 13 wrong-key starvation, canonical effect repair, ambiguous receipt quarantine and v1/v2 compatibility covered | pending on final exact head | `LEFT_PASSAGE_PARTY_ATTACK_ENABLED`; release operator unassigned | deferred |
