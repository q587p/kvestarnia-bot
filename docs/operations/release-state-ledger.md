...
## Current repository baseline (repository evidence, 12026-08-26)
...
| 0.4.8 guild weekly goal, Glory Book and GroupCombat ability parity | no; active PR `#197` on `codex/0.4.8-guild-weekly-goal` | target deployment unknown; repository adds additive `20260824090000_guild_weekly_goal` with paired rollback and populated-row apply→rollback→restore smoke; frozen achievement provenance has no live-period FK | no target evidence; `GUILD_WEEKLY_GOAL_ENABLED` defaults off and guards weekly/Glory reads+writes | local static/docs/build/lint/type gates plus `4,805` unit and `789` integration tests pass, including survivor-safe atomic `/dev_delete_account`, immutable notification state/reprojection, public nonmember Book access and separate shared-presenter invite-card coverage; replacement exact-head CI pending | updated four-account journal/Glory/ties/pagination/restart/notices/flag/reset/invite-card matrix pending | `GUILD_WEEKLY_GOAL_ENABLED`; release operator unassigned | deferred |
