...
| 0.4.8 guild weekly goal and GroupCombat ability parity | no; active on `codex/0.4.8-guild-weekly-goal` | target deployment unknown; repository adds additive `20260824090000_guild_weekly_goal` | no target evidence; `GUILD_WEEKLY_GOAL_ENABLED` defaults off | local static/build plus `4,772` unit and `775` integration tests pass; owning PR/CI pending | four-account matrix pending | `GUILD_WEEKLY_GOAL_ENABLED`; release operator unassigned | deferred |
