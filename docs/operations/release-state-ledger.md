...
| 0.4.3 consumable manatka uses | no; implementation branch | no migration required | no target evidence; `CONSUMABLE_MANATKA_USES_ENABLED` defaults off and gates only 17 nonmedical rows | yes; static + 4,352 unit + 709 integration tests, focused 637 unit + 238 integration combat suites, docs and combat simulator pass | one-account ordinary/out-of-combat and three-account Party Boss/GroupCombat matrix pending, including cooldown `1` and two-enemy one-response checks | `CONSUMABLE_MANATKA_USES_ENABLED`; deploy rollback; release operator unassigned | deferred |
