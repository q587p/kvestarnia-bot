...
| 0.4.8 guild weekly goal, Glory Book and GroupCombat ability parity | no; active PR `#197` on `codex/0.4.8-guild-weekly-goal` | target deployment unknown; repository adds additive `20260824090000_guild_weekly_goal` with paired rollback and populated-row apply→rollback→restore smoke | no target evidence; `GUILD_WEEKLY_GOAL_ENABLED` defaults off and guards weekly/Glory reads+writes | local static/docs/build/lint/type gates plus `4,797` unit and `788` integration tests pass, including atomic weekly-aware `/dev_delete_account` cleanup; replacement exact-head CI pending | updated four-account journal/Glory/ties/pagination/restart/notices/flag/reset matrix pending | `GUILD_WEEKLY_GOAL_ENABLED`; release operator unassigned | deferred |
