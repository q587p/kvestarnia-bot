...
| 0.4.5 guild foundation, Guild Nest and emoji crests | no; PR `#190` implementation branch | target deployment unknown; repository includes unchanged additive `20260802230000_guild_foundation` plus additive `20260806120000_guild_custom_crests`, each with tested rollback/restore isolation; the initial-8/absolute-13 capacity boundary adds no schema field or migration | no target evidence; repository default off | current-main integration and prior Nest/directory/routing checks passed; final presence/main-menu/copy correction 144/144 focused, first-life/remort founding boundary 71/71 focused, guild repository 25/25, full no-cache unit 4614/4614 and integration 745/745 pass locally after static gates; exact-head CI evidence is maintained in the PR body | exact-head three-account Nest/directory/create/invite/roles/remort/restart/party/privacy plus catalog/custom-emoji and invitation-card matrix pending, including level-6 rejection, level-7 first-life access and level-3 remort access | `GUILD_FOUNDATION_ENABLED`; release operator unassigned | deferred |
