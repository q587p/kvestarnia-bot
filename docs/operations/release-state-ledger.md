...
| 0.4.2 left-passage party attack | no; draft branch only | no; branch includes additive `20260724233000_left_passage_party_attack` with durable v3 operator marker/reason and participant achievement-effect receipts; rollback requires flag-off, healthy drain, explicit quarantine disposition and pre-migration restore | no target evidence; repository default off | yes; focused 13 files / 214 tests, full 4,092 unit + 633 integration, docs 416/249, Prisma validation/generation and simulator passed; active/terminal quarantine, 13-row starvation, repeat-tick silence, effect-stage rollback and before/after-unlock restart replay covered | pending on final exact head | `LEFT_PASSAGE_PARTY_ATTACK_ENABLED`; release operator unassigned | deferred |
