...
| GroupCombat SQLite delivery hotfix | no; PR `#192` | no migration required | no new flag | focused actor-first/session-serialized-tail, exact-revision CAS, scheduler-overlap, restart recovery, partial-`P1008`, retry-window and repository integration tests pass | three-account last-ready plus two-turn latency/regression run pending on exact hotfix head | deploy rollback; release operator unassigned | deferred |
