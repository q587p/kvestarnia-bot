...
| 0.4.3 consumable manatka uses | no; implementation branch | no migration required | no target evidence; all 20 exact mappings have no catalog-specific rollout flag, while existing combat-surface entry flags retain their scope | yes; static + 4,363 unit + 709 integration tests, focused consumable 225 unit + 192 integration tests and recruitment presenter/callback 166 unit tests, docs and combat simulator pass | one-account ordinary/out-of-combat and lone-owner `c002`, three-account Party Boss/GroupCombat matrix, cooldown `1`, two-enemy one-response, left-passage nearby invitation and full `3/3` checks pending | deploy rollback; release operator unassigned | deferred |
