...
| 0.4.6 referral foundation | no; current `0.4.6` PR branch | not deployed; repository adds `20260819090000_referral_foundation` with paired rollback | no target evidence; both flags default off | focused policy, first-touch, consent, Character-arrival, level-jump, exact-once payout, restart/remort-gap, outbox retry, privacy, Chronicle and production-guard tests pass on the branch; full CI pending | six-account private/share/decline/competing-inviter/payout/restart/remort/flag matrix pending | `REFERRAL_FOUNDATION_ENABLED` + `REFERRAL_REWARD_PAYOUTS_ENABLED`; release operator unassigned | deferred |
