...
| 0.4.2 left-passage party attack | no; draft branch only | no; branch includes additive `20260724233000_left_passage_party_attack` with durable v3 operator-repair state | no target evidence; repository default off | yes; focused 264 unit/service/bot + 66 repository integration, full `npm run check` with 4,091 unit + 627 integration, docs 416/249, Prisma validation/generation and 24-case GroupCombat simulator; transactional stage failures and v3 lease retention covered | pending on final exact head | `LEFT_PASSAGE_PARTY_ATTACK_ENABLED`; release operator unassigned | deferred |
