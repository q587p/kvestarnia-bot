...
і scheduler не реєструються. Для локальної перевірки створіть ватагу через
`/dev_party`: вона вміщує 2–3 пригодників, показує точний час старту через три
хвилини й після дедлайну автоматично починає сутичку з поточним складом.
Приватна картка ватажка дозволяє почати раніше кнопкою
...
- `/dev_reset_korchma_round [random|hit|miss|forced-pity]` — скидає поточний київський день Корчмарського обходу й у non-production обирає звичайну, гарантовано влучну, гарантовано невдалу або примусову pity-гілку для повторюваної QA; у production команда не реєструється, не показується й не мутує стан.
