...
7. On a remort `2+` character, start a solo Yeger fight and verify the intro plus active card show `Відплата за минулі пригоди`, not `Натиск Низу`; rewards and Yeger board progress should look unchanged after victory.
8. Compare ordinary one-enemy and two-enemy threat fights: a base-life character should still need three eligible wins before `Натиск Низу`, remort `1` should need two, and remort `2+` should need one; solo remort-pressure cards may show `Відлуння минулих пригод` or `Відплата за минулі пригоди` from `р2`, while two-enemy threat cards keep the existing `Натиск Низу` pressure language.
