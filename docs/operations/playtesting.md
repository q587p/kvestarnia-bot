...
Manual Telegram QA status for the implementation pass: not run. Use two joined accounts and one outsider in the isolated non-production bot with `BIG_BARREL_BROTHER_RAID_ENABLED=true`; raid chat has no separate config key. Create a gathering once through `/raid` and once through the explicit Barrel action: each must show one separate approach notice and exactly one recruiting card. Verify that each accepted player post reaches the other account immediately as a separate blockquote notification without opening chat, then knock one participant out and confirm that only combat actions disappear while raid-chat read/write remains available. Follow the complete evidence matrix in [`docs/qa/0.3.15-raid-chat-qa.md`](../qa/0.3.15-raid-chat-qa.md).
...
8. Очікування: немає stale resource warning; перед вирішенням видно explicit confirmation, потім один збережений результат без XP, золота, манаток, ставок або втрат. Якщо обидва учасники вже прийняли `Перше правило Бійцівського кутка` в поточному житті, кожен окремо отримує лише ідемпотентний квестовий крок.
