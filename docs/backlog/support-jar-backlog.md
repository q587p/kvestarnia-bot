...
Live read-only status may be considered only as a separate PR after checking official Monobank API docs, token scopes, rate limits, caching, privacy and logging boundaries. Canonical future plan: [SUPPORT_JAR_LIVE_STATUS.md](../operations/support-jar-live-status.md).
