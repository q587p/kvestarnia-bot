...
The private inviter dashboard shows this exact per-stage table before the link is shared, but no separate aggregate totals line. Policy/economic checks still verify `1 × item.dense-bandage`, `6 × item.field-kit`, `276 × item.iskrokamin` and `1830` gold. This owner-approved disclosure is referral-dashboard-only; eligible invitees continue directly into ordinary onboarding, while lore and `news.md` remain spoiler-light.
