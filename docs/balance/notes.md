...
`0.3.0` Charkokovalnia item upgrades add a bounded resource sink for selected equipment manatky from the Korchma yard elf-mage surface. Access starts at level `5+`, or remorted level `3+`, then consumes one `Польова аптечка` for a bounded dynamic XP unlock inside Charkokovalnia; the quest-table route stays spoiler-light before the player enters that surface. Authored upgrade variants use concrete ids from `base.plus-1` through `base.plus-5`; generated Loot Expansion `-plus-N` ids remain possible drops but their weights are now a very rare tail of the generated loot pool. Costs spend gold or mana plus `Іскрокамінь`, chance uses capped luck/pity/donor bonuses, previews stay qualitative before commit, and stale direct attempts reject before spend when the stack, level or pity snapshot has changed. The Iskrokamin ladder is Kvestarnia-spaced rather than linear: `+1: 5`, `+2: 13`, `+3: 23`, `+4: 42`, `+5: 93`. Modifiers use the strongest applicable value: uncommon `x1.05`, rare `x1.13`, epic `x1.23`, set pieces `x1.42`, and legendary items `x1.93`; same-template/same-set/same-slot donors discount `42%`/`23%`/`13%`, give at least one visible Iskrokamin discount when applicable, and cannot reduce Iskrokamin below half the modified cost. Positive post-fight bandage reward slots can rarely be replaced by Iskrokamin instead of granting both: base chance is `4%`, Luck caps it at `6%`, and replacement quantity is `ceil(bandageQuantity / 2)`. Successful upgrades move exactly one stack unit to the next concrete id and align equipped rows; failed attempts spend and increment pity once. `+1..+3` are weak magic with the original small stat scaling and 13-minute post-equip attunement. `+4..+5`, Mantok set pieces and ability-grant manatky are strong magic with larger stat jumps, higher Iskrokamin costs and 42-minute post-equip attunement. Pending attunement withholds stats, set bonuses and equipment actions so equipping stronger gear cannot spike combat power before the visible timer resolves. Item rarity now supports `legendary` above `epic`, but broad legendary loot remains disabled unless explicitly authored. This is not an item-instance rewrite, market, paid power path, broad crafting economy or combat-action change.

`0.3.2` Kharakternyk ward signs are a bounded Big Barrel Brother prep sink, not a new universal shield. Placement costs deterministic `8..11` mana from a base `13` discounted by effective Intelligence plus Luck; support costs deterministic `5..8` mana from the same stats and has no free class exception. Final support count is frozen from the joined/remort-valid starting roster and caps at `7`. Mitigation stays `min(95, 25 + 10 * supportCount)`, applies only to Big Barrel broad retaliations, and spends one frozen support charge per activation; an unsupported sign still triggers once.
...
- LUCK може підняти rarity максимум на один крок у поточній бойовій таблиці й теж має малий cap, щоб rare/epic не ставали обовʼязковими; broad legendary drops лишаються вимкненими;
