...
1. Напиши `/start`.
2. Створи персонажа.
3. Дійди до корчми або знайди tavern hub.
4. Відкрий `/quest`.
...
12. Перевір, що terminal state веде назад до quest table / tavern / new fight.
...
3. Переконайся, що `13th win` видає рівно одну fixed reward.
4. Повтори callback.
...
7. Перевір, що копірайт говорить про `Нагорода за справу`, а не про per-fight loot.

### Expected reward for `0.0.21+`
...
Це очікувана поведінка для поточного slice `0.0.21+`, а не універсальне правило для майбутніх квестів.
...
2. Перевір outside-tavern gate, якщо він є.
