...
- `0.2.17` Big Barrel Brother Raid MVP: shipped a feature-flagged level 8+ Barrel route on the existing party foundation and party-boss runtime, with Big-specific rules/version/copy/tuning, no hidden runtime round cap, deterministic 13-round balance probes, exact-once victory settlement through the existing Barrel success key, and no failure gold/items/success.
