...
- `0.2.17` Senior Barrel Brother Raid MVP: shipped a feature-flagged level 8+ Barrel route on the existing party foundation and party-boss runtime, with Senior-specific rules/version/copy/tuning, no hidden runtime round cap, deterministic 13-round balance probes, exact-once victory settlement through the existing Barrel success key, and no failure gold/items/success.
...
- Fuller production raid mechanics after the Senior MVP hardens, with richer contribution-aware rare/serious manatky rewards only through capped replay-safe rules.
