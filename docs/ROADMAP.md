...
- `0.1.5` opens the first Phase 2 prep surface: `🥊 Бійцівський куток` and `/spar` show a deterministic, rewardless training card against the `Сумлінний Допельґанґер`, with no real duel invites, PvP state, rewards or HP/inventory mutation.
