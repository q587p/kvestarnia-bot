...
- Safe gifting shipped first in `0.2.0`; `0.2.1` shipped only the backward-compatible two-enemy foundation. Architecture stabilization is queued next as a behavior-preserving `0.2.2` task before threat escalation. Broader trading, item tags/equipment and party/raid runtime stay in later `0.2.x+` slices.
- The next implementation prompt should be chosen from the current `0.2.x` task docs, with threat escalation or item tags remaining evidence-gated follow-ups after architecture stabilization.
...
- `0.2.2` Architecture Stabilization: modular bot registrars, extracted cross-cutting middleware, explicit composition root/runtime lifecycle and architecture boundary tests without gameplay, schema, callback or copy changes.
