...
- `0.1.20` follow-up hardening: active outcome bodies are method-and-grade-specific instead of shared label-substitution templates, with the last intent-wide grammar-sensitive templates replaced by grammar-neutral method beats or explicit scene text; result cards show the chosen method after the scene title and do not repeat the full button label inside the outcome paragraph; personalized method hints preserve scene risk warnings for injury and fight handoff; daily and cooldown claims store the exact committed HP audit with the service-supplied effective max; `character_cooldowns.resultJson` stores cellar result audit data for duplicate safety while the ordinary on-cooldown card stays valid; concurrent different-key claims apply both HP losses instead of overwriting; rollback uses the original claim identity, compensates only the committed HP delta and never reduces later HP; Adventure fight handoffs persist and pass the actual eligible encounter id, and any non-new handoff state rolls the quest claim back rather than consuming it.
