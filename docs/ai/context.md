...
- The inviter dashboard exposes only consented current Character name/level, historical stage marks and aggregates. It excludes Telegram identity, presence, quests, inventory, balance, remort and guild identity. `REFERRAL_FOUNDATION_ENABLED=false` blocks entry/capture/acceptance while accepted relations still record stages; `REFERRAL_REWARD_PAYOUTS_ENABLED` controls delivery. First accepted Character arrival emits one normal `referral.arrived` Chronicle fact in `all`/`adv` under the ordinary Character-arrival dedupe key; progress/payouts emit no referral-specific public rows and public referrals never join or reward guilds.
