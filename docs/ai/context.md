...
- Current status (authoritative over the two historical implementation-summary bullets below): repository package `0.4.4` merged through PR `#191` and was observed in production at `67bd02cdd1e9c73e9f8fbe8ebdcffb7ac279af4c`. The active work is an unnumbered GroupCombat SQLite delivery hotfix for production `P1008`/expired `P2028` contention and stale first-turn/later-turn cards: deliver the initiating player first, serialize the remaining roster, isolate per-participant failure and defer recently attempted recovery rows for 13 seconds. Combat state, balance, rewards, schema, migrations, flags and the planned `0.4.5` scope remain unchanged; exact-hotfix Telegram QA and deployment remain pending.
