...
- A fresh feed applies capped immediate `2 + rank HP` and `1 mana`, then one recipient-global `CharacterCooldown`: `😋 Ситий` lasts 13 minutes and the recipient wait lasts 93 minutes. The server-owned preview binds the exact applied rank/cost, stat/equipment/Shynok snapshot, target/lives and expiry; confirmation never silently changes that plan.
- Outside combat, complete eligible minutes lazily grant capped `+1 HP/+1 mana`; the cursor advances while full and excludes only actual combat-lease time. Durable combat states pulse at most once after the owner's committed action/spend and before the following hostile response; quick duels do not pulse.
