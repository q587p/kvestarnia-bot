...
- `0.1.21` combat action foundation: combat state now supports basic attack, basic defend, class skill and flee through a server-authoritative action/availability contract; class skill cooldowns are ability-keyed while legacy `cooldowns.skill` states remain readable; current class actions become available after one subsequent own committed action. Unavailable skill attempts because of missing mana or cooldown are no-ops: no mana spend, no turn advance, no cooldown tick, no monster response and no RNG advancement. `🛡 Захищатися` appears in persistent solo fights, training doppelganger fights and turn-based duels; PvE defend reduces current-round incoming damage and can rarely counter with fatigue on repeated defense, while hidden-choice duels apply deterministic same-round incoming damage reduction at reveal. New solo/training turns store `turnExpiresAt` in `CombatState` for about `23` seconds; battle-surface restore/callback paths lazily commit a canonical basic auto-attack after the deadline, while safe side-surface redirects after the deadline commit a missed-turn skip where the hero does not attack but the monster still acts. Persistent fight state stores an origin location: ordinary `Низ` fights keep post-fight `Новий бій`, while Adventure Choice and Yeger handoff fights return to the surface where they started instead of always pointing to `Низ`. Persistent monster fights also freeze a Kyiv-local context snapshot plus deterministic bark state in `CombatState`; the content roster now has 93 monsters, each with a context profile and five authored barks, and these affect fight texture only, not XP/gold/loot/Yeger matching or monster eligibility. No race/signature/monster/item ability catalog, schema migration, reward/economy change or broad combat coefficient rewrite ships in this slice.

- `0.1.21` combat UI follow-up: newly started persistent monster fights send a separate static intro card and then one active button card that updates each turn. The active card shows HP/mana, turn number and the visible `23` second auto-action rule; stored monster barks render as Telegram `<blockquote>` speech; generic `Наступний крок` prompts were removed from fight/hunt result templates.
