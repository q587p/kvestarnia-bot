...
- GroupCombat live cards show one `🗡️ Вдарити`, one defend action and one button per canonically available class/race/gear ability. Attack and explicit single-enemy or ally/self abilities commit directly when exactly one canonical target remains and open a target-only picker for two or more; auto-target abilities commit directly. Enemy focus begins on the living party leader, then follows the highest actual participant damage from the previous resolved turn with roster-order ties; active cards and each newly stored journal turn mark `🎯 ціль ворогів`, recap actions announce changes, and legacy snapshots without focus evidence remain unmarked. New/relationally rebuilt state carries additive `enemyFocusVersion: 1`; an active unmarked v2/v3 state ignores old cumulative threat, presents and targets the living leader once, then persists exact resolved-turn damage, with zero/tied damage falling back by roster order. `↩️ До дій` is read-only, reply pickers are temporary, and final target callbacks retain canonical stale/duplicate/timeout validation. Party Boss keeps its established distinct layout. Item rows remain usefulness-gated; old reply labels remain read aliases. Durable message-reference/control publication, bounded losing-candidate cleanup, refresh adoption and terminal delivery contracts remain unchanged. The underlying proof remains available only behind `GROUP_COMBAT_PROOF_ENABLED` outside production.
  ...
  Universal terminal statistics record eight truthful dimensions—damage, healing, prevention, control, damage taken, actions, special actions and defensive turns—only where canonical evidence exists. Viewer-specific rows remain private and every Statistics callback is read-only.
