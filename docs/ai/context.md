...
- Varenyk-mancer level 3+ `🍽️ Нагодувати` works on self or an active exact-normalized-location recipient from existing locations. Attunement-aware INT/CHA/level determine stat rank; after canonical passive mana settlement, the highest affordable rank uses costs `8/12/16/20/23`. Public open/preview capture one `now` and reuse one repository-owned resource/equipment planning snapshot, so regeneration occurs once and displayed resources/maxima/stats agree with the stored preview across inclusive `readyAt`.
