...
- Current repository version: `0.4.4` — PR #190 corrects a default-off durable guild shell with User-level membership/leadership, a 13-minute replaceable preview, exact-once 93-gold forming charter, seven-day founder cooldown, target-bound 93-hour invitations, accepted nominee transfer, capped roles/roster and bounded name release. Only the second distinct User activates `guild.created`; remort and safe Character recreation preserve membership without inferred succession. `/guild_party` is a five-row picker for an already-existing real gameplay recruiting `PartySession`; ordinary invite/join/canonical-card flow and nonmember routes remain authoritative, with `joinSource=guild` as attribution only. Roster/audit expose no exact location, online/activity time, Telegram identity or invite token. `GUILD_FOUNDATION_ENABLED` defaults false; disabled mode keeps safe reads/exits and `/dev_guild_gold` remains non-production. No bank, shared economy, buffs, XP, weekly goal, boss, chat, trade, war, territory or PvP is implied. The completed `0.4.3` exhaustive twenty-consumable catalog remains active without `CONSUMABLE_MANATKA_USES_ENABLED`; existing combat-surface flags keep their original scope.
