...
- Current repository version: `0.4.4` — PR #190 corrects a default-off durable guild shell with persistent-menu and active-character entry, requirements→crest→separate private name→optional description→preview/confirm creation, private share deep links, explicit accept/decline plus exact-once inviter notices, and button-first profile/member actions backed by stable membership IDs. The charter costs 587 gold exactly once; User-level membership/leadership, seven-day founder cooldown and forming window, target-bound 93-hour invitations, capped roles/roster and bounded name release remain authoritative. Existing rewardless create/join achievements remain authoritative, while `📖 Перекази` explains the shipped shell and its no-bank/ordinary-vatanga boundary. Only the second distinct User activates `guild.created`; every relevant operation terminalizes its own overdue forming charter independently of the bounded 23-row cleanup backlog, while invite creation validates the target User's specific membership and creation validates the exact reservation owner behind unrelated backlog. Remort and safe Character recreation preserve membership without inferred succession. The hub expires all guild-owned due invites, uses one stable five-row member/invitation stream, scopes cancel controls to current authority and keeps duplicate names independently selectable. `/guild_party` is a five-row picker for an already-existing real gameplay recruiting `PartySession`; ordinary invite/join/canonical-card flow and nonmember routes remain authoritative, with `joinSource=guild` as attribution only. Disabled legacy guild-party callbacks stop before audience resolution, Telegram delivery, canonical publication or audit. `/restart` ignores expired generic recruiting residue but protects live ordinary, automatic-start and combat contracts. Roster/audit expose no exact location, online/activity time, Telegram identity or invite token. `GUILD_FOUNDATION_ENABLED` defaults false; disabled mode keeps safe reads/exits and `/dev_guild_gold` remains non-production. No bank, shared economy, buffs, XP, weekly goal, boss, chat, trade, war, territory or PvP is implied. The completed `0.4.3` exhaustive twenty-consumable catalog remains active without `CONSUMABLE_MANATKA_USES_ENABLED`; existing combat-surface flags keep their original scope.
...
- `0.4.2` is the first production-capable GroupCombat consumer, but its entry
  ...
  `0.4.4` adds independent User-level guild identity behind `GUILD_FOUNDATION_ENABLED`; it uses ordinary `PartySession`, does not own party/combat state and implies no bank/trade/war/weekly-goal scope. Manual three-account Telegram QA, merge, migration deployment and target enablement remain unproven; an audited abandoned-leader operator runbook is a production-enable blocker. A dedicated combat reply keyboard that temporarily replaces main navigation is deferred in `docs/backlog/dedicated-combat-reply-keyboard.md`; keep the shipped persistent-main plus inline-control fallback until durable cross-mode publication/recovery is proven.
...
- `docs/tasks/0.4.4-guild-foundation.md` and `docs/qa/0.4.4-guild-foundation-qa.md` — current repository release contract and pending three-account Telegram QA.
- `docs/tasks/0.4.3-consumable-manatka-uses.md` — previous exhaustive consumable catalog contract; merged through PR `#188`.
