...
- `/duel` and every duel callback emit only a neutral user-activity heartbeat until a fresh final transition applies the single authoritative Fighting Corner payload. Final turn-duel acceptance keeps the ordered transition, intro/advice and new combat card. Each participant has one atomically claimed canonical private card: stale owner refreshes and repeated deep links reuse it, concurrent fallback losers are disarmed, and group links remain spectator-safe. The voluntary-duel flavor uses `Форма 13-Д`; `Форма 13-А` remains the Bureaucramancer combat/raid form.
