...
- Current repository version: `0.4.2` — the exact hard `deep-left` pending encounter can be reserved by a 1–3-person `PartySession` and resolved against 1–6 enemies through production-capable `group-combat.v3` behind default-off `LEFT_PASSAGE_PARTY_ATTACK_ENABLED`; solo attack remains available, and the canonical preview presence marker does not self-block party reservation.
  ...
  Runtime servicing remains active when new entry is disabled. Each enemy adds its ordinary-PvE XP/gold budget before the manual-only neutral split; loss XP is one fifth and loss gold is zero. Every enemy also gets one deterministic ordinary broad-loot roll plus one bandage slot, round-robin among eligible manual participants: enemy count adds opportunities, effective level scales broad-loot multiplier `0.75..1.5`, authored/Loot Expansion manatky are eligible and the ordinary positive-slot `4–6%` replacement may grant Іскрокамінь. Cosmetic titles remain presentation-only. `lootVersion: 1` uses a code-owned immutable monster/ability/loot catalog and resolver: canonical enemy evidence, candidates, weights, selected items and quantities are rederived from frozen inputs, while persisted selection ranges or adjacent public checksums are never authority. Restart and pending settlement consult no current item, monster-loot, monster/profile or generic-loot catalogs; coherent evidence/output mutations and forged plans fail closed. `skill.strict-blessing` and `gear.asclepius-instruction` retain ally heal/protection scopes and send their authored direct damage to the first living enemy in frozen order. Every valid manual action accepted for the turn remains eligible even when an earlier actor or start-of-turn effect ends combat; skipped actions spend nothing and timeout guards remain ineligible. Active combat swaps the main reply keyboard for six compact battle actions and on-demand target/ability/item menus. Each participant's flee uses the ordinary escalating chance and a persisted personal attempt count; success atomically persists exact resources/inventory/status release and exit evidence and releases only that lease before Telegram delivery. A durable pending/claimed/menu-delivered/completed state retries failed menus after later turns, avoids resending an acknowledged menu after card failure, and clears the inert old card; failure leaves the actor exposed. Remaining actors validate and continue without the escapee lease; later settlement cannot overwrite the escapee's newer state/remort life, and shared discovery applies only to the unchanged frozen life. Chronicle ids/count include only eligible manual participants, excluding escapees and timeout-only rows. Terminal delivery restores the location-aware main menu before the fresh Journal/Statistics card. A win persists one shared deterministic 13–23-minute left-passage clear window and unavailable Ярус II construction notice for matching frozen lives. One-transaction per-player effective resources/consumables, Chronicle activity, monotonic levels, receipts and participant-owned leases converge after restart/retry. Fresh manual settlements may deliver the existing generic level/item/combat achievement notices; replays and timeout-only rows do not. The new completion achievement is deferred: 0.4.2 emits no left-passage-specific achievement event and settlement has no achievement-effect fields or projection work.
...
- The underlying `0.4.1` restart-safe 2–3 player versus 2–3 enemy proof remains available only behind `GROUP_COMBAT_PROOF_ENABLED` outside production.
  ...
  journal. Current production query observations/budgets are `34/34` manual start, `33/33`
  due start, `23/23` queue, `25/35` single resolve, `35` concurrent-pair aggregate, `1/1` due/delivery/pending-settlement scans, `6/6` idle repair and `26/38` participant-settlement events (the cap covers up to twelve aggregated per-enemy item grants);
