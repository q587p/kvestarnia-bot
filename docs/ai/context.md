...
- `0.1.20` follow-up hardening: active outcome bodies are method-and-grade-specific instead of shared label-substitution templates; personalized method hints preserve scene risk warnings for injury and fight handoff; daily and cooldown claims store the exact committed HP audit with the service-supplied effective max; `character_cooldowns.resultJson` stores cellar result replay data; concurrent different-key claims apply both HP losses instead of overwriting; rollback compensates the committed HP delta instead of restoring an old absolute value; Adventure fight handoffs persist and pass the actual eligible encounter id, and an unrelated already-active fight causes quest rollback rather than consuming the adventure claim.
