...
- Active implementation target: `0.4.7 — Universal Battle Statistics, Richer Korchma Rounds, and Manatka Dismantling`, on `codex/0.4.7-universal-battle-stats-dismantling` / PR #196 from merged `0.4.6` main. The release owns truthful read-only terminal statistics across every shipped combat surface; every authoritative solo, Training and Mimic terminal producer exposes exactly one typed `ba1` Telegram capability URL while active cards expose none, and frozen stored public identity keeps historical cards independent of a mutable Character. It also owns deterministic richer Daily Korchma rewards with a LUCK-bounded `13..23%` identity branch, unchanged canonical `tavern_event` candidate weights and six-miss pity, plus atomic one-unit Forge dismantling; manual Telegram QA, merge and deployment remain separate evidence.
...
- `0.4.2` is the first production-capable GroupCombat consumer, but its entry
  ...
  `0.4.4` is merged. `0.4.5` plus PR #193 add independent User-level guild identity and replay-safe activation Chronicles behind `GUILD_FOUNDATION_ENABLED`; guilds reuse ordinary parties and own no combat state. `0.4.6` is merged at PR #195 and adds independent User-level Referral Foundation behind two default-off flags; it owns no guild/party/combat state. Guild and referral manual Telegram QA, migration deployment and target enablement remain unproven. Draft PR #196 implements `0.4.7`: universal read-only terminal battle statistics, richer Daily Korchma rewards whose LUCK affects only the bounded identity-branch chance rather than canonical candidate weights, and atomic Forge dismantling. It is unmerged, undeployed and pending Manual Telegram QA. Guild weekly work shifts to `0.4.8`; the accepted forward sequence continues through Guild Cosmetic Progression at `0.4.15`.
...
- `docs/tasks/0.4.7-universal-battle-stats-korchma-rewards-dismantling.md` — current release contract and pending Manual Telegram QA; `docs/tasks/0.4.6-referral-foundation.md`, `docs/design/referral-system.md` and `docs/qa/0.4.6-referral-foundation-qa.md` retain the merged referral contract and separate rollout evidence.
