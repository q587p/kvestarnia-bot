...
- Outside combat, complete eligible minutes lazily grant capped `+1 HP/+1 mana`; the cursor advances while full and excludes only actual combat-lease time. Solo lease release claims/advances/deletes atomically and preserves the outside remainder; duel acceptance persists canonical pre-lease recovery before freezing. Durable combat states pulse at most once after the owner's committed action/spend and before the following hostile response; quick duels do not pulse.
- Persistent PvE keeps the canonical restricted final response from an enemy alive at attack/class-skill exchange start even when that exchange defeats it. Mutual `0 HP` against the final enemy is a hero win; hero `0 HP` with another enemy alive is a loss. Hero renders authoritative post-settlement resources, preserves the ordinary full-HP notice once and uses an indexed fast path for finished historical rows.
