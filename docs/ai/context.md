...
- `/duel`, duel deep links and every duel callback emit only a neutral user-activity heartbeat until a fresh final transition applies the single authoritative Fighting Corner payload; duel-owned routes reach their canonical handlers through combat lock. Final turn-duel acceptance keeps the ordered transition, intro/advice and new combat card. Each participant has one global CAS-owned canonical private card across actions, remote notifications, timeout, combat-lock and Rogue delivery: replacements are inert until the winner revalidates and activates them, retryable edits retain the old reference, failed activation releases it, stale owner refreshes and repeated deep links reuse it, concurrent losers stay inert, and group links remain spectator-safe. The voluntary-duel flavor uses `Форма 13-Д`; `Форма 13-А` remains the Bureaucramancer combat/raid form.
