...
- Current repository version: `0.4.4` — a default-off durable guild social shell stores normalized unique names, emoji crests, bounded descriptions, user-level membership, roles, invitations and privacy-safe audit/funnel rows. Previewed creation spends 587 gold exactly once; serializable transactions, unique constraints and bounded conflict retry converge competing creates/accepts, duplicate confirms, stale role callbacks and deterministic leadership succession. Membership survives remort/repository restart, while guild rows have no ownership or cascade path to `PartySession`, `GroupCombatSession` or current-life combat leases. `/guild_party` delegates to the ordinary party service, preserves nonmember join routes, privately notifies only eligible current characters and does not resend on a replayed live session. Roster/audit surfaces expose no exact location, online timestamp, Telegram identity or invite token. `GUILD_FOUNDATION_ENABLED` defaults false; `/dev_guild_gold` is non-production only. No bank, shared economy, buffs, XP, weekly goal, boss, chat, trade, war, territory or PvP is implied. The completed `0.4.3` exhaustive twenty-consumable catalog remains active without `CONSUMABLE_MANATKA_USES_ENABLED`; existing combat-surface flags keep their original scope.
...
- `0.4.2` is the first production-capable GroupCombat consumer, but its entry
  ...
  `0.4.4` adds independent User-level guild identity behind `GUILD_FOUNDATION_ENABLED`; it uses ordinary `PartySession`, does not own party/combat state and implies no bank/trade/war/weekly-goal scope. Manual three-account Telegram QA, merge, deployment and target enablement remain unproven. A dedicated combat reply keyboard that temporarily replaces main navigation is deferred in `docs/backlog/dedicated-combat-reply-keyboard.md`; keep the shipped persistent-main plus inline-control fallback until durable cross-mode publication/recovery is proven.
