...
- A fresh feed applies capped immediate `2 + rank HP` and `1 mana`, then one recipient-global `CharacterCooldown`: `😋 Ситий` lasts 13 minutes and the recipient wait lasts 93 minutes. A server-owned preview row binds token/target/lives.
- Outside combat, complete eligible minutes lazily grant capped `+1 HP/+1 mana`; the cursor advances while full and discards combat-lease time. Durable combat states pulse at most once per own committed turn/round identity; quick duels do not pulse.
- Sated never has parallel stacks. After a genuinely ended/shortened/cleared wait, a fresh feed replaces rank and duration. Recipient remort clears old-life state; actor remort does not cancel another recipient's activation.
