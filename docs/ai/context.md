...
- Active implementation target: `0.4.8 — Guild Weekly Goal`, on `codex/0.4.8-guild-weekly-goal` from merged `0.4.7` PR #196. One default-off Monday `00:00 Europe/Kyiv` goal counts thirteen successful ordinary left-passage party expeditions through immutable session/contributor receipts. Historical active membership at terminal `completedAt` selects one guild; at least two manual guildmates qualify, support counts equally, progress caps at thirteen and completion grants no XP, gold, item, buff or reward multiplier. The private guild hub shows current Holocene ISO-week progress; completion emits one privacy-safe guild Chronicle fact and rewardless contributor achievement. Settlement/replay/repair converge from receipts; `/dev_guild_weekly [finish]` is non-production only. `GUILD_WEEKLY_GOAL_ENABLED=false` blocks new weekly eligibility/writes/UI without blocking ordinary parties or deleting evidence. Automated validation, owning PR and CI are gates; Manual Telegram QA, merge, migration deployment, target flag and production availability remain separate evidence.
