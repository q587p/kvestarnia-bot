...
- Active implementation target: `0.4.7 — Universal Battle Statistics, Richer Korchma Rounds, and Manatka Dismantling`, on `codex/0.4.7-universal-battle-stats-dismantling` / PR #196 from merged `0.4.6` main. The release owns truthful terminal statistics across every shipped combat surface, deterministic richer Daily Korchma rewards with six-miss pity, and atomic one-unit Forge dismantling; manual Telegram QA, merge and deployment remain separate evidence.
...
- `0.4.2` is the first production-capable GroupCombat consumer, but its entry
  ...
  `0.4.4` is merged. `0.4.5` plus PR #193 add independent User-level guild identity and replay-safe activation Chronicles behind `GUILD_FOUNDATION_ENABLED`; guilds reuse ordinary parties and own no combat state. `0.4.6` adds independent User-level Referral Foundation behind two default-off flags; it owns no guild/party/combat state. Guild and referral manual Telegram QA, migration deployment and target enablement remain unproven. `0.4.7` owns universal terminal battle statistics, richer Daily Korchma rewards and forge dismantling. Guild weekly work shifts to `0.4.8`; the accepted forward sequence continues through Guild Cosmetic Progression at `0.4.15`.
