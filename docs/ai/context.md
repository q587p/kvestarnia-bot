...
- Outside combat, complete eligible minutes lazily grant capped `+1 HP/+1 mana`; the cursor advances while full, retires the terminal fraction at exact expiry and excludes only actual combat-lease time. Solo/Training, turn-duel and Big Barrel starts persist canonical pre-lease recovery/anchors, exact lease times and frozen remainders before combat-local resources take over; guarded release and Hero reads preserve that boundary through missing/malformed state. Public turn-duel acceptance uses one inclusive-`readyAt` equipment view for sync, warnings, confirmation, natural maxima, stats, set/granted actions and winner-only initiative, never rewrites max columns, then ratio-balances. Durable states pulse after the owner's committed action/spend and before the following hostile response; quick duels do not pulse.
