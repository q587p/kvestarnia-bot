...
- Bard Performance callbacks preserve the current normalized location. Quest-spawned persistent fights store and reuse their origin location, while every `already-completed` Adventure method callback preserves current presence regardless of whether it observed the claim during preflight or atomic claim. Quest monsters scale to the current level before canonical once-only remort derivation and show quest remort pressure only in the opening message.
