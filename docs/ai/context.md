...
- Latest `0.1.20` visible-method pass: race/class/signature variants use distinct scene-action labels and affordance ids so the resolver can actually surface them; the resolver also preserves at least one risky method when the scene supports risk, and duplicated legacy starter/cellar actions route through explicit canonical alias maps.
