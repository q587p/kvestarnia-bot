...
- Current status (authoritative over the two historical implementation-summary bullets below): repository package `0.4.4` merged through PR `#191` and was observed in production at `67bd02cdd1e9c73e9f8fbe8ebdcffb7ac279af4c`. PR `#192` is the unnumbered GroupCombat SQLite delivery hotfix for production `P1008`/expired `P2028` contention, stale cards and serial-delivery latency: active callbacks return after the initiating player's card, while active/scheduler/terminal operations share one per-session queue, drain participants serially and finalize only their frozen revision through repository CAS; failures stay isolated and recent recovery rows wait 13 seconds. A terminal `nyz_left_attack_*` replay after completed exit delivery publishes a fresh canonical result through an exact reference/revision CAS instead of returning silently. Combat state, balance, rewards, schema, migrations, flags and `0.4.5` remain unchanged; exact-hotfix Telegram QA and deployment remain pending.
- Active implementation target: `0.4.4` Bugfix & Polish. GroupCombat uses one Attack, commits a sole canonical target directly, opens target-only pickers for two or more targets, and names the actually damaged enemy in single-target ability recaps when several enemies remain. Enemy focus follows the Big-Barrel contract on active cards and each new journal turn: living leader first, then the highest actual damage from the previous resolved turn with roster-order ties; additive `enemyFocusVersion: 1` evidence keeps persisted unmarked v2/v3 cumulative threat from masquerading as previous-turn damage, and legacy journal snapshots without focus evidence remain unmarked. Left-passage `nyz_left_attack_*` links require the canonical origin kind and exact left-passage location, relocate only eligible joiners atomically, and recheck canonical GroupCombat before rendering preparation; inventory marks unequipped requirement-gated equipment `🔎 🔒` from one shared in-memory evaluator. No migration, ґільдія runtime or new flag; manual Telegram QA/deployment remain pending. PR `#188` merged `0.4.3`; PR `#189` shipped isolated-runtime supervision and bounded rotating logs.
- Current repository version: `0.4.3` — an exhaustive exact-id catalog activates all twenty current consumables: the three existing medical supplies, two authored cellar items and generated `c001`–`c015`. `c001/c003/c009` heal exactly 7/8/9 HP in and out of combat; the remaining typed immediate effects cover bounded self/pair/party healing, useful once-resolved HP/mana outcomes, conditional/both-resource restoration, one canonical GroupCombat cleanse, one-turn cooldown reduction, fixed 13 damage, 42% response reduction and response evasion. Paired `c002` is useful when either its owner or a living ally is injured; a lone owner receives one capped 8-HP heal, never both halves. Full/inapplicable resources or targets consume nothing; `c005` is useful only when a cooldown remains positive after the ordinary action tick. `c006`/`c013`/Fancy Cheese commit only on the first canonical response where they add a positive actual delta after earlier protection; `c006` never records zero prevention, while evade requires real HP loss or a harmful owner-targeted on-hit consequence that would apply. A miss, static potential component or already-neutralized compiled rider is not eligible; an authored rider that really still applies at zero remaining damage is. Evade keeps the enemy action/cooldown but suppresses only that owner's selected consequence, and later/unrelated responses stay normal. A shared-round item made useless or left without an eligible owner response is journaled without decrement/cooldown/achievement evidence; Solo instead keeps the original active turn/session. Solo guard/evade attempts use a narrow deterministic RNG keyed by the immutable session snapshot, turn and item, so unchanged no-op duplicate/restart callbacks cannot reroll; eligible commits retain the transactional active-turn CAS. Selected response damage/prevention persists for restart replay. The cellar quest bottle remains protected until the current-life `keep` ending; retained stacks are not permanently remort-locked. `item-use-v5` freezes the typed random branch, exact starting quantity and life evidence; stack drift stales, completed receipts replay before the current quest gate, and every combat commit rechecks that gate inside its transaction. Exact source tags and `effect_id` never infer behavior. All twenty exact mappings are available without a catalog-specific rollout flag; existing combat-surface flags retain only their own entry scope. Take-away purchases remain `0.4.9`. This builds on `0.4.2`: the exact hard `deep-left` pending encounter can be reserved by a 1–3-person `PartySession` and resolved against 1–6 enemies through production-capable `group-combat.v3` behind `LEFT_PASSAGE_PARTY_ATTACK_ENABLED`; solo attack remains available, and the canonical preview presence marker does not self-block party reservation.
...
- `0.4.2` is the first production-capable GroupCombat consumer, but its entry
  ...
  Independent guild identity follows in `0.4.5`; do not turn `PartyBossSession` into N×M state or imply guild bank/trade/war scope. A dedicated combat reply keyboard that temporarily replaces main navigation remains deferred beyond the `0.4.4` target picker polish.
...
- `docs/tasks/0.4.4-bugfix-polish.md` — active release contract; `docs/tasks/0.4.3-consumable-manatka-uses.md` is the latest merged contract. PR `#188` is merge evidence, not deployment or manual-QA evidence.
