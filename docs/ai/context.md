...
- `0.1.21` combat action foundation: combat state now supports basic attack, basic defend, class skill and flee through a server-authoritative action/availability contract; class skill cooldowns are ability-keyed while legacy `cooldowns.skill` states remain readable; current class actions become available after one subsequent own committed action. Unavailable skill attempts because of missing mana or cooldown are no-ops: no mana spend, no turn advance, no cooldown tick, no monster response and no RNG advancement. `🛡 Захищатися` appears in persistent solo fights, training doppelganger fights and turn-based duels; PvE defend reduces current-round incoming damage and can rarely counter with fatigue on repeated defense, while hidden-choice duels apply deterministic same-round incoming damage reduction at reveal. New solo/training turns store `turnExpiresAt` in `CombatState` for about `23` seconds; active cards store their Telegram message reference, and an in-process scheduler best-effort commits the canonical basic auto-attack and edits the active card after the deadline. Battle-surface restore/callback paths keep the same auto-attack as a restart/delivery/race fallback, while safe side-surface redirects after the deadline commit a missed-turn skip if they win the overdue-turn race first. Persistent fight state stores an origin location: ordinary `Низ` fights keep post-fight `Новий бій`, while Adventure Choice and Yeger handoff fights return to the surface where they started instead of always pointing to `Низ`. Persistent monster fights also freeze a Kyiv-local context snapshot plus deterministic bark state in `CombatState`; the content roster now has 93 monsters, each with a context profile and five authored barks, and these affect fight texture only, not XP/gold/loot/Yeger matching or monster eligibility. No race/signature/monster/item ability catalog, schema migration, reward/economy change or broad combat coefficient rewrite ships in this slice.
