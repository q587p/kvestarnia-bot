...
- Active implementation target: `0.4.4` Bugfix & Polish. GroupCombat uses one Attack plus target-only explicit pickers; left-passage `nyz_left_attack_*` links relocate only eligible joiners atomically; inventory marks unequipped requirement-gated equipment `🔎 🔒` from one shared in-memory evaluator. No migration, ґільдія runtime or new flag; manual Telegram QA/deployment remain pending. PR `#188` merged `0.4.3`; PR `#189` shipped isolated-runtime supervision and bounded rotating logs.
