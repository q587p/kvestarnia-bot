...
- Active implementation target: `0.4.5` Guild Foundation on PR `#190`. It retains the default-off 587-gold transactional charter, User-level membership through remort, private target-bound invitations, exact-once lifecycle/audit behavior, button-first profile/member management, ordinary `PartySession` reuse and strict privacy/party isolation. This release adds `🪺 Гніздо ґільдій` under `Спуск до Низу` as a same-presence public discovery surface with an active-only five-row directory; public Nest actions require current `PRESENCE_LOCATION_KORCHMA_DEEP`, while private `/guild`, invite/deep-link recovery and character-card shortcuts remain usable anywhere under existing rules. New persistent main keyboards omit the global `🏰 Ґільдії` row and retain the historical three-row composition with nearby/help together; cached legacy text remains a guarded compatibility route. The 13 catalog crests are transactionally exclusive to forming/active guilds; filtered creation/profile pickers chunk only visible choices into non-empty rows of at most five while retaining original catalog indices in callbacks. A direct custom Telegram-photo alternative uses a restart-safe User-bound draft, requires authoritative size metadata within 5 MiB, stores only reusable media references/metadata, renders `🖼️` in text and never widens the directory DTO with raw media ids. A custom-crest leader can preserve the exact stored image while editing only the description. One replaceable creation intent associates only its latest consumed custom draft; custom/custom and custom/catalog/custom replacement detach old prompts so stale photos are inert. Wrong-media replies revalidate the live User-bound draft before another ForceReply. All guild routes, exact fallback commands, callbacks, valid private start links and the exact custom-photo ForceReply pass canonical PassageSearch, pending-raid and combat ownership guards before presence, draft or guild work; unrelated photos/replies stay outside guild routing. Current player news keeps each release heading behind a blank Markdown boundary.
