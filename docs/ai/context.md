...
- `0.1.22` battle-card follow-up: persistent PvE overdue turns now commit `defend` / `timeout-auto-defend` instead of basic auto-attack, while legacy timeout-auto-attack JSON remains readable. Successful persistent turns append durable `turnLog` entries in combat state JSON and active/terminal fight keyboards can open `📜 Журнал бою` with first/previous/next/last paging and `↩️ До бою`; unavailable skill no-ops do not create log rows. Active fight cards no longer show `Хід записано` or `Остання дія`, skill cooldown rows name the exact skill, monster ability summaries include damage/effect/no-direct-damage consequences, and monster context cues are covered against `????` mojibake. The three Nyz passage buttons now enter separate presence locations (`Лівий прохід`, `Прямий прохід`, `Правий прохід`), so `👀 Хто поруч`, nearby duel validation and post-fight `Новий бій` preserve the selected passage instead of merging everyone into `Сутерени Корчми`.
