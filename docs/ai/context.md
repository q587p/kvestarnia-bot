...
- GroupCombat `🎒 Разові` appears only while canonical validation finds a useful owned supported item. The winning message-reference CAS records the attached inline-control fingerprint/generation atomically; acknowledgement failure or scheduler retry edits that adopted card instead of sending five-second countdown copies. A losing candidate gets three bounded delete attempts and otherwise becomes one compact inert superseded note. Active and terminal replacement compact the previous canonical message before best-effort delete, and a failed fresh terminal send restores the prior result. `🔎 Оновити` durably marks that participant's control publication pending before the immediate attempt, so a busy/dead publisher is adopted by the bounded delivery queue; active delivery cannot finalize while that fingerprint is pending. Completed exits retire terminal delivery work even when a legacy card revision is older. The underlying `0.4.1` restart-safe 2–3 player versus 2–3 enemy proof remains available only behind `GROUP_COMBAT_PROOF_ENABLED` outside production.
  ...
  due start, `25/25` queue, `39/42` single resolve, `50` concurrent-pair aggregate, `41` production start, `1/1` due/delivery/pending-settlement scans, `6/6` idle repair and `29–31/38` participant-settlement events (the cap covers up to twelve aggregated per-enemy item grants);
