...
- `0.1.19` Nyz passage balance polish: ordinary/problem fight passage balance keeps the center route as baseline, changes the right/easy route to prefer available monsters `3-5` levels below the character with safe fallback, gives easy XP as a downward-rounded `0.5x-0.75x` character-level roll with a small bounded LUCK bias, gives left/hard XP as a `1.25x-1.5x` character-level roll above center with the same bounded LUCK bias, keeps existing drop/loot item/rarity profile modifiers, and makes fight gold a variable `0..character level` roll rather than a stable passage payout with `0` gold boosting item drop chance to `93%` before interpolating back to the current chance at max gold; stored completed fight rewards replay from session/daily-action data and are not recomputed.
