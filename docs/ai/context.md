...
- Current merged package: `0.4.5` through PR `#190` at `a3945b1f`; merged unnumbered PR `#192` at `2afe359a` retains the SQLite GroupCombat delivery correction. Guild Foundation deployment, production enablement and exact-head Telegram QA remain pending; the active narrow hotfix restores its missing public activation Chronicle event.
- Guild Foundation retains the default-off 587-gold transactional charter, User-level membership through remort, private target-bound invitations, exact-once lifecycle/audit behavior, button-first profile/member management, ordinary `PartySession` reuse and strict privacy/party isolation. Guild delivery preserves `joinSource=guild`; an existing left-passage session uses its canonical `nyz_left_attack_*` link and origin-bound relocation on the first callback instead of requiring a retry. Activation by the second distinct User records one deduplicated `guild.created` public activity fact in both `👥 Пригодники` and `⭐ Важливе`, while private forming charters remain undisclosed; its authoritative `activatedAt`/`activatedByInviteId` timestamp survives founder Character deletion and replay, though founder achievement delivery still requires that Character. This release adds `🪺 Гніздо ґільдій` under `Спуск до Низу` as a same-presence public discovery surface with an active-only five-row directory; the enabled Spusk card names its low side passage and places the Nest left of descent in one row, while public Nest actions require current `PRESENCE_LOCATION_KORCHMA_DEEP` and private `/guild`, invite/deep-link recovery and character-card shortcuts remain usable anywhere under existing rules. New persistent main keyboards omit the global `🏰 Ґільдії` row and retain the historical three-row composition with nearby/help together; cached legacy text remains a guarded compatibility route. The 13 catalog crests and one-emoji custom alternatives are transactionally exclusive to forming/active guilds; filtered creation/profile pickers chunk only visible catalog choices into non-empty rows of at most five while retaining original catalog indices in callbacks. The guided custom path accepts one genuine Unicode RGI emoji sequence, preserves its visible FE0F presentation selectors, canonicalizes selector-less/FE0F aliases only to a separate reservation key, rejects explicit FE0E text presentation, fake variation-decorated text, pseudo-ZWJ sequences, photos/files/links and reissues an exact prompt only after current authority/version validation; legacy photo prompts are inert. `GUILD_INITIAL_MEMBER_CAPACITY=8` remains the current admission/UI boundary and `GUILD_MAX_MEMBER_CAPACITY=13` records the future absolute maximum without an expansion action, persisted field or migration. A personal invitation renders a forwardable card with 13 distinct Ukrainian texts, its actionable private deep link, stable link/token, separated instruction paragraphs and bold real response-button labels; nonmember and ordinary-member visitors get distinct explanations of the inviter-only leader/officer requirement plus `🏰 До ґільдії` recovery. Forming/active private hero cards show escaped crest/name; active nearby members show crest only, and a leader may invite an opted-in same-location nonmember through both `/online` and the persistent `👀 Хто поруч` route using a revalidated five-row picker. Disabled presence construction omits the guild relation entirely; enabled reads select only the active-guild crest, and no private opt-in token reaches Telegram markup. Single-page member management omits redundant `1/1` text and controls while real overflow retains stable pagination. Acceptance/founder/join achievement results and every non-ready guild-party picker state retain `🏰 До ґільдії`. All guild routes, exact fallback commands, callbacks, valid private start links and exact custom-emoji ForceReply pass canonical PassageSearch, pending-raid and combat ownership guards before presence or guild work; unrelated replies stay outside guild routing. Public recruitment/applications, treasury, chat, structures, diplomacy and territory remain separately scoped future backlog. Current player news names storage, glory and buildings only in one genuine unavailable-boundary line.
...
- The completed `0.4.3` exhaustive twenty-consumable catalog remains active without `CONSUMABLE_MANATKA_USES_ENABLED`; existing combat-surface flags retain their original scope and take-away purchases remain `0.4.10`.
...
- `0.4.2` is the first production-capable GroupCombat consumer, but its entry
  ...
  `0.4.4` is the merged Group Targeting, Left Invitations and Equipment Locks release. `0.4.5` adds independent User-level guild identity, exclusive catalog/custom-emoji crests and the same-presence Nest directory behind `GUILD_FOUNDATION_ENABLED`; it uses ordinary `PartySession`, does not own party/combat state and implies no bank/trade/war/weekly-goal scope. The additive custom-crest migration and rollback preserve existing foundation history and unrelated User/Character/party/combat rows. Manual three-account Telegram QA, merge, migration deployment and target enablement remain unproven; an audited abandoned-leader operator runbook is a production-enable blocker. A dedicated combat reply keyboard remains deferred in `docs/backlog/dedicated-combat-reply-keyboard.md`.
...
- `docs/tasks/0.4.5-guild-foundation.md` and `docs/qa/0.4.5-guild-foundation-qa.md` — current PR release contract and pending exact-head three-account Telegram QA.
- `docs/tasks/0.4.4-bugfix-polish.md` — latest merged release contract; deployment and manual-QA evidence remain separate.
