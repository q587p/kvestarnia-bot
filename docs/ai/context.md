...
- `0.1.19` Nyz passage balance polish: ordinary/problem fight passage balance keeps the center route as baseline, changes the right/easy route to a stronger `-5` effective monster level adjustment with safe clamping, lower easy XP/gold/drop/loot power and downward rounding for easy reward fractions, and keeps the left/hard route higher-risk with XP/gold above center while trimming the earlier XP/drop overpayment; stored completed fight rewards replay from session/daily-action data and are not recomputed.
