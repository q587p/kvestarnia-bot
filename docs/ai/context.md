...
- Current repository version: `0.4.2` — the exact hard `deep-left` pending encounter can be reserved by a 1–3-person `PartySession` and resolved against 1–6 enemies through production-capable `group-combat.v3` behind default-off `LEFT_PASSAGE_PARTY_ATTACK_ENABLED`; solo attack remains available, and the canonical preview presence marker does not self-block party reservation.
  ...
  Active GroupCombat UI uses one durable participant combat-lease publication claim, shared with flee/terminal navigation fencing and later combat ownership. The exact token is atomically renewed before every Telegram send/edit/delete and each call aborts after 13 seconds below the 23-second stale boundary; ownership loss stops later calls, while Bot API acceptance immediately before abort/network failure remains externally at-least-once. A live UI fence does not spend gameplay mutation retries or discard terminal/flee/timeout actions, and dead claims recover from elapsed wall time after restart. Acknowledged keyboard fingerprints/generations avoid routine unchanged party-wide republishes and advance only when the keyboard was actually attached to a private canonical card; supergroup/no-keyboard delivery cannot poison them. Changed legal abilities, cooldown/mana recovery and knockout controls travel on one fresh canonical card without a readiness placeholder. Frozen stale ability labels still reach current validation without committing unavailable actions, while reply text with no matching GroupCombat delegates instead of overwriting newer combat/navigation. The ordinary persistent-fight `🎒 Одноразові манатки` callback bypasses the generic combat lock and opens its non-consuming filtered fight submenu. Manual Telegram QA, production enablement, merge and deployment remain pending.
- The underlying `0.4.1` restart-safe 2–3 player versus 2–3 enemy proof remains available only behind `GROUP_COMBAT_PROOF_ENABLED` outside production.
  ...
  due start, `25/25` queue, `27/35` single resolve, `38` concurrent-pair aggregate, `41` production start, `1/1` due/delivery/pending-settlement scans, `6/6` idle repair and `30/38` participant-settlement events (the cap covers up to twelve aggregated per-enemy item grants);
