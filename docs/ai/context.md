...
- Current repository version: `0.4.3` — an exhaustive exact-id catalog activates all twenty current consumables: the three existing medical supplies, two authored cellar items and generated `c001`–`c015`. `c001/c003/c009` heal exactly 7/8/9 HP in and out of combat; the remaining typed immediate effects cover bounded self/pair/party healing, useful once-resolved HP/mana outcomes, conditional/both-resource restoration, one canonical GroupCombat cleanse, one-turn cooldown reduction, fixed 13 damage, 42% response reduction and response evasion. Full/inapplicable resources or targets consume nothing; `c005` is useful only when a cooldown remains positive after the ordinary action tick, and `c006`/`c013`/Fancy Cheese affect exactly the first eligible canonical enemy response while later responses stay normal. A shared-round item made useless or left without a response targeting its owner is journaled without decrement/cooldown/achievement evidence; selected response damage/prevention persists for restart replay. The cellar quest bottle remains protected until the current-life `keep` ending; retained stacks are not permanently remort-locked. `item-use-v5` freezes the typed random branch, exact starting quantity and life evidence; stack drift stales, completed receipts replay before current flag/quest gates, and every combat commit rechecks those gates inside its transaction. Exact source tags and `effect_id` never infer behavior. `CONSUMABLE_MANATKA_USES_ENABLED` defaults off and gates only the seventeen nonmedical activations; medical items remain live. Take-away purchases remain `0.4.9`. This builds on `0.4.2`: the exact hard `deep-left` pending encounter can be reserved by a 1–3-person `PartySession` and resolved against 1–6 enemies through production-capable `group-combat.v3` behind default-off `LEFT_PASSAGE_PARTY_ATTACK_ENABLED`; solo attack remains available, and the canonical preview presence marker does not self-block party reservation.
...
- `0.4.2` is the first production-capable GroupCombat consumer, but its entry
  ...
  `0.4.3` activates the exhaustive twenty-id typed immediate consumable catalog through `item-use-v5`; the default-off flag gates only seventeen nonmedical rows. GroupCombat item menus page five useful rows at a time. Turn duels remain item-free, take-away purchases stay in `0.4.9`, and manual Telegram QA/merge/deployment remain unproven.
