...
- Current repository version: `0.4.2` — the exact hard `deep-left` pending encounter can be reserved by a 1–3-person `PartySession` and resolved against 1–6 enemies through production-capable `group-combat.v3` behind default-off `LEFT_PASSAGE_PARTY_ATTACK_ENABLED`; solo attack remains available, and the canonical preview presence marker does not self-block party reservation.
  Recruitment lasts three minutes, is discoverable through exact-location `👀 Хто поруч`, shares readiness controls with Big Barrel, starts immediately when all current 1–3 participants are ready, may start early by the current leader and otherwise auto-starts with the authoritative current-life same-location roster at expiry.
  The exact preview is primary; one enemy per participant is the base, while each strong frozen participant may add one reservation-stable backup from canonical remort, solo Низ pressure or a three-level advantage, capped at six. A fresh production start sends the deterministic roster/levels/tip plus remort/pressure note separately before the clean canonical combat card; replay/restart does not repeat it. Live card text and exact-target buttons use collision-safe ordinary-fight short monster names, while intros, defeat notices, journals and statistics keep full names. The immutable production-v1 catalog freezes all 132 canonical authored monster abilities; GroupCombat compiles their typed target, damage, modifier, support, control, counter, damage-over-time, cooldown and conditional trigger recipes without a second short allowlist or label-only fallback, and a typed map assigns every semantically active authored parameter to exactly one execution consumer. Effective ability mana cost includes base plus active pressure exactly once, while basic actions/items never pay it. Class/race locks and one deterministic frozen-ability lock persist exact evidence; false-exit pressure changes the single pre-roll flee chance but attempt seven remains guaranteed; charged marks consume only on the first landed hit. Reapplied expired effects preserve their original authored semantic source plus separate enemy/ability/turn provenance and are strictly rederived on restart; unsupported or malformed semantics fail before start or on load. Enemies alive at exchange start keep one final basic response after participant defeat, never a posthumous special. Production has no artificial turn-limit loss; the deterministic journal rolls over the last 25 turns in a compact legacy-readable snapshot shape, journal/statistics buttons appear only on the terminal result, and ability/item recap lines name the actor once with explicit `застосовує вміння` / `використовує манатку` wording and unquoted iconic labels.
  Runtime servicing remains active when new entry is disabled. Each enemy adds its ordinary-PvE XP/gold budget before the manual-only neutral split; loss XP is one fifth and loss gold is zero. Every enemy also gets one deterministic ordinary broad-loot roll plus one bandage slot, round-robin among eligible manual participants: enemy count adds opportunities, effective level scales broad-loot multiplier `0.75..1.5`, authored/Loot Expansion manatky are eligible and the ordinary positive-slot `4–6%` replacement may grant Іскрокамінь. Cosmetic titles remain presentation-only. `lootVersion: 1` uses a code-owned immutable monster/ability/effective-rarity catalog and resolver: every `x025` variant is released-effective epic, and canonical enemy evidence, candidates, weights, selected items and quantities are rederived from frozen inputs while persisted selection ranges or adjacent public checksums are never authority. The strictly parsed relational participant snapshot independently anchors immutable identity/display/title/life/order, class/race/level, maxima, attack/defense/support/stats, equipment, gear abilities and initial item quantities; runtime state may change HP/mana, cooldowns, threat, flee/fumbles and canonically committed item decrements only. Restart and pending settlement consult no current item, monster-loot, monster/profile or generic-loot catalogs; coherent evidence/output/actor mutations and forged plans fail closed. `skill.strict-blessing` and `gear.asclepius-instruction` retain ally heal/protection scopes and send their authored direct damage to the first living enemy in frozen order. Every valid manual action accepted for the turn remains eligible even when an earlier actor or start-of-turn effect ends combat; skipped actions spend nothing and timeout guards remain ineligible, while timeout auto-guards still advance that actor's cooldowns/effects. Active combat swaps the main reply keyboard for direct class/race/equipment ability buttons that pass canonical current-turn cooldown, mana and target validation, immediate single-target commits, target-only pickers when several legal targets exist, a filtered item submenu and an observer-only refresh state for knocked-out participants. Every valid or unavailable reply action redraws the acting participant's canonical live card as the newest message without a separate stale-action bubble. Each participant's flee uses the ordinary escalating chance and a persisted personal attempt count; success atomically persists exact resources/inventory/status release and exit evidence and releases only that lease before Telegram delivery. Gameplay exit is exactly once; notification is durable at-least-once because Telegram send has no idempotency key. Live claims retry message-id acknowledgement without immediate resend, durable acknowledgement prevents resend, and the send/ack crash gap has bounded duplicate risk on stale reclaim. Free-player menus use current presence/quest markers; a newer combat durably supersedes the old delivery instead of losing its battle keyboard. Failure leaves the actor exposed. Remaining actors, including knocked-out non-fled lease owners, validate and continue without false repair quarantine; later settlement cannot overwrite the escapee's newer state/remort life, and shared discovery applies only to the unchanged frozen life. Chronicle ids/count include only eligible manual participants, excluding escapees and timeout-only rows. Terminal delivery restores the location-aware main menu first, then atomically adopts the fresh bottom-most Journal/Statistics result as the canonical participant card. A win persists one shared deterministic 13–23-minute left-passage clear window and unavailable Ярус II construction notice for matching frozen lives; until it ends, both direct invitations and `👀 Хто поруч` reject another join with the canonical remaining wait and generic previous-group-fight wording. One-transaction per-player effective resources/consumables, Chronicle activity, monotonic levels, receipts and participant-owned leases converge after restart/retry. Fresh manual settlements may deliver the existing generic level/item/combat achievement notices; replays and timeout-only rows do not. The new completion achievement is deferred: 0.4.2 emits no left-passage-specific achievement event and settlement has no achievement-effect fields or projection work.
  Active GroupCombat UI uses one durable participant combat-lease publication claim, shared with flee/terminal navigation fencing and later combat ownership. Active-card tokens renew atomically before every Telegram send/edit/delete; the independent exit-navigation token and exact lease renew after current markers resolve and immediately before its main-menu send. After menu acknowledgement, terminal delivery retains the same claim and lease through old-card compaction, fresh-card send, canonical adoption and cleanup, then releases the lease only in the exact final CAS. Each call aborts after 13 seconds below the 23-second stale boundary; ownership loss stops later calls, ordinary exit rejection returns to pending, and Bot API acceptance immediately before abort/network failure remains externally at-least-once. Validated manual actions commit in a separate transaction before bounded turn resolution, so process death or a live UI fence cannot erase a final win/flee action; duplicate or timeout work adopts the complete action set exactly once after restart. Generic combat-lock routing reads one authoritative `activeCombatLease` and loads only its exact `referenceId + characterId` owner; a missing, terminal, quarantined, wrong-participant or unknown owner fails closed without unrelated probes or downstream navigation, while an exact temporary exit-navigation owner resumes its healthy participant/session and then continues the requested route without mismatch spam; solo overview reuses the valid lease. Strict repair accepts an exactly claimed `menu-delivered` row during the normal Telegram-ack/final-CAS interval. Callback telemetry records actor-visible `interactiveMs` separately from `postPresentationMs` and full `totalMs`. The separate production intro is serialized inside the same durable participant claim immediately before its keyboard-bearing canonical card, never releases the canonical reference and cannot arrive after an acknowledged battle keyboard. Acknowledged keyboard fingerprints/generations avoid routine unchanged party-wide republishes and advance only when the keyboard was actually attached to a private canonical card; unchanged active/compact edits omit inline reply markup so the persistent keyboard stays visible without another send. Supergroup/no-keyboard delivery cannot poison fingerprints, and terminal result cards carry no reply keyboard. Changed legal abilities, cooldown/mana recovery and knockout controls travel on one fresh canonical card without a readiness placeholder. An explicitly fresh actor card from `/start`, `🔎 Оновити` or a handled reply action republishes the current keyboard even when the fingerprint is unchanged. Frozen stale ability labels still reach current validation without committing unavailable actions, while reply text with no matching GroupCombat delegates instead of overwriting newer combat/navigation. The ordinary persistent-fight `🎒 Одноразові манатки` callback bypasses the generic combat lock and opens its non-consuming filtered fight submenu. Manual Telegram QA, production enablement, merge and deployment remain pending.
- GroupCombat `🎒 Разові` appears only while canonical validation finds a useful owned supported item. When a private candidate send attaches a reply keyboard, the winning message-reference CAS records its fingerprint/generation atomically; a later acknowledgement failure or scheduler retry edits that adopted card instead of sending five-second countdown copies. A candidate that loses the durable card claim gets three bounded delete attempts and otherwise becomes only a compact inert superseded note. Active and terminal replacement compact the previous canonical message before its best-effort delete, so delete refusal cannot preserve a second full battle card or terminal result; a failed fresh terminal send restores the prior result. `🔎 Оновити` durably marks that participant's reply keyboard pending before the immediate attempt, so a busy/dead publisher is adopted by the bounded delivery queue; active delivery cannot be finalized while that fingerprint is still pending. Completed participant exits retire terminal delivery work even when a legacy card revision is older, so finished sessions leave the five-second queue. The underlying `0.4.1` restart-safe 2–3 player versus 2–3 enemy proof remains available only behind `GROUP_COMBAT_PROOF_ENABLED` outside production.
