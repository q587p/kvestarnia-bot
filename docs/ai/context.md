...
- The completed `0.4.3` exhaustive twenty-consumable catalog remains active without `CONSUMABLE_MANATKA_USES_ENABLED`; existing combat-surface flags retain their original scope and take-away purchases remain `0.4.11`.
...
- `0.4.2` is the first production-capable GroupCombat consumer, but its entry
  ...
  `0.4.3` activates the exhaustive twenty-id typed immediate consumable catalog through `item-use-v5` without a catalog-specific rollout flag. GroupCombat item menus page five useful rows at a time. Turn duels remain item-free and take-away purchases stay in `0.4.11`.
  `0.4.4` is merged. `0.4.5` plus PR #193 add independent User-level guild identity and replay-safe activation Chronicles behind `GUILD_FOUNDATION_ENABLED`; guilds reuse ordinary parties and own no combat state. `0.4.6` adds independent User-level Referral Foundation behind two default-off flags; it owns no guild/party/combat state. Guild and referral manual Telegram QA, migration deployment and target enablement remain unproven. Guild weekly work shifts to `0.4.7`; the accepted forward sequence continues through Guild Cosmetic Progression at `0.4.14`.
