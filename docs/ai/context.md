...
- Current package version in this repository snapshot: `0.1.20`.
...
- `0.1.20` authored quest resolutions: level 3+ Adventure Choice no longer uses active `safe/flair/risky` method buttons; selected problems render 5-7 stable authored scene-action methods, with race/class/signature/title shaping checks, selected affordances and outcomes without exposing mechanic-source labels in buttons, and with qualitative hints and no exact future reward/odds copy. Adventure Choice uses compact stable problem and method callback keys under Telegram's 64-byte limit, while service resolution stores canonical ids. Hint rendering suppresses duplicate reliability wording and keeps chance phrases sentence-cased; generated race/class/title families use scene-native анкета/кухоль/портрет/підручник/форма/іспит/титул methods instead of one universal generated template, and outcome copy avoids singular/plural agreement traps. Race/class/signature influence is bound to concrete scene affordances rather than blind noun substitution, while visible labels/buttons name the scene action; the deterministic visible-method resolver is authoritative for both button rendering and completion, so hidden authored method ids stale out before mutation unless they are explicit legacy starter/cellar actions. Starter mimic-shawarma and level 2-3 cellar mouse use the same resolution contract while preserving level gates, item grants, idempotency and legacy starter/cellar callback replay; the cellar paid bribe stays reachable without dropping race/class/signature slots. Starter shawarma and starter combat each grant `75%` of the level 1-to-2 XP gap rounded up on the remort-adjusted curve, so doing both reaches level 2, and the quest archive shows completed starter shawarma beside the completed starter combat probe. Old Adventure Choice `v1` safe/flair/risky callbacks show stale-refresh/current offer rather than mapping to new methods, and unknown current-version method ids stale out before claim/cost/cooldown/reward. Quest checks use the effective stat snapshot including equipped manatky/item effects, are deterministic and bounded, produce `strong-success`, `success`, `mixed-success` or `complication`, and store method/grade/consequence/check/cost/reward/items in `daily_actions.resultJson` with `spent_gold` for guarded atomic paid methods. Universal filler methods are not active content; every active general problem/generated family supplies enough scene-native affordances. Level 3+ authored rewards apply small stored XP/gold variance and a low LUCK-influenced manatka chance after resolution; paid Adventure methods grant no gold reward and instead get a slightly higher manatka chance; replay never rerolls. Cellar mouse rewards stay deliberately tiny and paid mouse methods cannot become a farm. Player-facing result copy avoids internal method-source labels and uses a separate `Винагорода за справу` block. Complications are varied: full/reduced/XP-only/paid/cosmetic outcomes, method-owned minor/serious HP injury with stored before/lost/after/max audit clamped to `1 HP`, and scene-specific existing persistent-fight handoff with rollback if combat cannot start. Korchma hall prose still mentions `Бійцівський куток` and `спуск до Низу`, but the hall hides both action buttons before level 3, and stale direct corner/duel-board callbacks return a short level-gate card instead of moving presence into the corner. `🎒 Манчкін-скупник` follows a Kyiv local `21:00-07:00` night schedule: day paragraph/button stays at the korchma front door, night paragraph/button moves to `Спуск до Низу`; level barter now requires at least `587` gold value from eligible manatky before wallet gold can fill the rest. Training and duel result screens use explicit `↩️ Повернутися до кутка` return buttons for Fighting Corner navigation. Local `/dev_help` lists enabled dev commands, `/dev_raid_stop` sends the ordinary separate level-up celebration when its raid reward crosses a level threshold, and `/dev_add_level` accepts an optional positive amount parameter.
- `0.1.20` follow-up hardening: active outcome bodies are method-and-grade-specific instead of shared label-substitution templates, with intent-wide active fallback replaced by complete authored/family outcome beats; result cards show the chosen method after the scene title, do not repeat the full button label inside the outcome paragraph, and render current HP from the returned post-claim character summary; personalized method hints preserve scene risk warnings for injury and fight handoff; daily and cooldown claims store the exact committed HP audit with the service-supplied effective max; `character_cooldowns.resultJson` stores cellar result audit data for duplicate safety while the ordinary on-cooldown card stays valid; concurrent different-key claims apply both HP losses instead of overwriting; rollback uses the original claim identity and guarded retries, compensating only the committed XP/gold/item/HP deltas while preserving later changes and never reducing later HP; Adventure fight handoffs persist and pass the actual eligible encounter id, any non-new handoff state rolls the quest claim back rather than consuming it, and a newly started handoff stamps canonical solo-fight presence.
- Latest `0.1.20` rollback/content pass: active method outcomes fail fast if complete four-grade outcome beats are missing, tests reject sliced-label focus markers and old global template sentences, daily-action audit stores exact `appliedItemGrants` after caps, rollback receives the current effective HP max from the service summary instead of claim-time max, and handoff rollback branches keep canonical persistent/training/rest presence instead of stamping quest-table presence first.
- Latest `0.1.20` visible-method/copy pass: race/class/signature variants use distinct scene-action affordances so the resolver can actually surface them, but visible buttons no longer append reusable technique suffixes such as `у ритм`, `через ревізію`, `точним рухом` or `по-домашньому`; the resolver also preserves at least one risky method when the scene supports risk, duplicated legacy starter/cellar actions route through explicit canonical alias maps, and content tests reject copied outcome skeletons in the latest top-up/generated regression set.
- Latest `0.1.20` rollback HP guard: daily-action rollback uses the stored HP injury audit as a guard, restoring HP only when current HP still equals the committed post-claim `hpLoss.after`; later damage/healing/max-HP changes remain untouched while XP/gold/items still roll back through guarded compensation.
- Latest `0.1.20` copy cleanup: remaining generated-family, general-scene and top-up outcomes no longer use quoted object-fragment skeletons such as `Домовленість «…»`, `Сліди «…»` or `Ремісничий підхід до «…»`; content tests normalize case, quoted spans and punctuation to catch repeated long sentence frames across unrelated active scene methods.
