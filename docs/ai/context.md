...
- Current status (authoritative over the two historical implementation-summary bullets below): repository package `0.4.4` merged through PR `#191` and was observed in production at `67bd02cdd1e9c73e9f8fbe8ebdcffb7ac279af4c`. PR `#192` is the unnumbered GroupCombat SQLite delivery hotfix for production `P1008`/expired `P2028` contention, stale cards and the remaining serial-delivery latency: active callbacks return after the initiating player's card while a session-scoped tail drains allies serially; scheduler/terminal delivery still awaits the roster, failures stay isolated, and recent recovery rows wait 13 seconds. Combat state, balance, rewards, schema, migrations, flags and `0.4.5` remain unchanged; exact-hotfix Telegram QA and deployment remain pending.
