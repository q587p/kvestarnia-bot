...
- Active implementation target: `0.4.8 — Guild Weekly Goal`, on `codex/0.4.8-guild-weekly-goal` from merged `0.4.7` PR #196. One default-off Monday `00:00 Europe/Kyiv` goal counts thirteen successful ordinary left-passage party expeditions through immutable session/User contributor receipts and typed oldest-first cross-period reconciliation. Historical active membership at terminal `completedAt` selects one guild; support counts equally. Completion grants one exact `13` non-spendable guild Glory source, normal-severity privacy-safe Chronicle fact and durable User-level rewardless 1/3/13-calendar-week achievements, but no combat/material reward or guild XP; `Корчмарський обхід`, other daily quests and personal activities grant no Glory, and enabled player-facing reward/hub/Book/lore surfaces state that source boundary. The Lore Board exposes `Ґільдійний статут` and `Книга слави` under its dedicated main `🏰 Ґільдії` category, while the Guild Nest place record remains under locations. Multiple guild completions under one `sourcePeriodKey` count once and first durable threshold provenance never rewrites. Achievement Telegram delivery uses leased `PENDING/CLAIMED/SENT/PERMANENT_FAILURE` state, bounded exponential retry and sanitized failure categories, while the closed-alpha operator report labels its aggregate reconciliation/Glory/notification totals `cumulative-current`. The private hub shows progress, Glory and weekly place; any current Character at the Guild Nest can read the guild-ranking-only five-row `✨ Слава` and current-week `🏁 Першість` boards, while only active guild aggregates appear and own-place recovery is member-specific. `/dev_guild_weekly finish` uses a stable QA-only override. Confirmed `/dev_delete_account` atomically removes the reset identity from weekly rows, recomputes surviving overrides and freezes retained deleted-guild combat sessions weekly-ineligible before their credited evidence is removed; surviving Users retain immutable scalar source provenance and their exact entitlement/delivery row even when the founder-owned live period disappears. `GUILD_WEEKLY_GOAL_ENABLED=false` blocks all weekly/Glory reads and writes without blocking ordinary parties or deleting evidence. Automated validation, owning PR and CI are gates; Manual Telegram QA, merge, migration deployment, target flag and production availability remain separate evidence.
- Referral is a public first-touch User-level relation for fresh Telegram Users. Exact `ref1_*` capture runs before presence and atomically creates the fresh User plus accepted attribution and frozen policy version; eligible links continue directly into ordinary onboarding without a consent card, while ineligible paths create none. A current Character owns one stable link from the Korchma board or `/invite`; the main keyboard remains three rows. Accepted direct invitees create frozen whole-bundle entitlements at levels `3/5/8/13`, totalling one dense bandage, six Field Kits, `276` Iskrokamin and `1830` gold. Missing Character, payout flag-off, restart/remort and Telegram failure remain durable automatic retry; Telegram activity wakes the shared non-overlapping scheduler immediately, and there is no claim button. Local-only `/dev_delete_account ПІДТВЕРДЖУЮ` atomically returns an isolated QA identity to a pre-User state and is production-unregistered; it is not a production deletion policy.
- Before sharing, the inviter dashboard exposes the exact `3/5/8/13` bundles with canonical Ukrainian item names, including after Character deletion while the stable User-level link survives, but no separate aggregate totals line; policy/economic evidence remains `1 dense / 6 field kits / 276 Iskrokamin / 1830 gold`. Its stable URL has 13 distinct reachable Ukrainian forward-ready share drafts; `🎲 Інший текст` never rotates the token. Like a raid share card, each draft has only that one button, contains the raw stable link, and renders the optional live guild crest before the bold current inviter name plus the active cosmetic title through shared escaped social presentation; it never projects the guild name or shows name quotes, share/copy/back controls, a variant counter or footer. The concise private arrival notice contains only the frozen invitee name, race, class and title; payout notices contain only the committed stage and bundle. Dashboard history otherwise shows only current invitee Character name/level, historical stage marks and aggregates, excluding Telegram identity, presence, quests, inventory, balance, remort and invitee guild identity. Dashboard and bounded scheduler recovery independently project the rewardless first/thirteenth-arrival achievements from the canonical first and thirteenth attribution IDs and persisted arrival timestamps, including onto replacement Characters; User-keyed outbox notices remain exact-once across Character/service recreation and Chronicle recovery remains independent. `/invite` appears in enabled player help, and catalog coverage rejects directly registered commands omitted from `/help`. `REFERRAL_FOUNDATION_ENABLED=false` blocks new capture while accepted relations still record stages; `REFERRAL_REWARD_PAYOUTS_ENABLED` controls delivery. First accepted Character arrival durably freezes the original Character/name/time and eventually emits one normal `referral.arrived` Chronicle fact in `all`/`adv` under the ordinary Character-arrival dedupe key even after ActivityEvent failure/restart; progress/payouts emit no referral-specific public rows and public referrals never join or reward guilds.
- Guild founding requires level 7 before the first remort or level 3 after at least one remort; joining remains free and has no level/remort gate. A target's private guild opt-in screen sends a separate compact forwardable card rather than mixing code management into the shared artifact; referral, guild, duel, party/Big Barrel and tavern-game invitation cards all use the shared forwardable social presenter.
- Private forming/active hero identity uses `Ґільдія: <герб> <назва>`. Guild-enabled nearby rows, duel confirmations/results, ordinary/Big Barrel/GroupCombat rosters, Bard and class interactions, tavern games, game/combat/milestone boards and Kvestarnia chronicles prefix only the escaped live crest; combat snapshots store no guild name or role, and disabled mode keeps legacy non-guild reads.
- The completed `0.4.3` exhaustive twenty-consumable catalog remains active without `CONSUMABLE_MANATKA_USES_ENABLED`; existing combat-surface flags retain their original scope and take-away purchases remain `0.4.12`.
...
- GroupCombat live cards show one `🗡️ Вдарити`, one defend action and one button per canonically available class/race/gear ability. Attack and explicit single-enemy or ally/self abilities commit directly when exactly one canonical target remains and open a target-only picker for two or more; auto-target abilities commit directly. Enemy focus begins on the living party leader, then follows the highest actual participant damage from the previous resolved turn with roster-order ties; active cards and each newly stored journal turn mark `🎯 ціль ворогів`, recap actions announce changes, and legacy snapshots without focus evidence remain unmarked. New/relationally rebuilt state carries additive `enemyFocusVersion: 1`; an active unmarked v2/v3 state ignores old cumulative threat, presents and targets the living leader once, then persists exact resolved-turn damage, with zero/tied damage falling back by roster order. `↩️ До дій` is read-only, reply pickers are temporary, and final target callbacks retain canonical stale/duplicate/timeout validation. Party Boss keeps its established distinct layout. Item rows remain usefulness-gated; old reply labels remain read aliases. Durable message-reference/control publication, bounded losing-candidate cleanup, refresh adoption and terminal delivery contracts remain unchanged. The underlying proof remains available only behind `GROUP_COMBAT_PROOF_ENABLED` outside production.
  ...
  Timeout remains resource-free guard. Item consumption shares the winning action/turn transaction; Dense Bandage keeps five-own-action cooldown and Field Kit stays once per fight. Missing frozen inventory rewardlessly invalidates instead of retrying forever: every relational contribution/settlement row is rewritten to the exact fallback. Invalid v1/v2 repair artifacts preserve up to 13 relational participants within the state cap; larger/unrepresentable corruption releases all owned leases/statuses
  ...
  Terminal CAS writes one immutable zero-reward plan whose participant rows and receipts match state/relational identity and contribution before independent replay. Pending settlement is exactly zero-attempt/receipt-free/unsettled; completed receipts keep positive attempts and settledAt. Character resources, XP, gold, items, quests, achievements and activity rewards remain unchanged.
  Universal terminal statistics record eight truthful dimensions—damage, healing, prevention, control, damage taken, actions, special actions and defensive turns—only where canonical evidence exists. Completed results, journals and statistics are unlisted read-only capability artifacts available to any holder of the opaque battle/result token, including from shared group cards; active actions and private mutable state remain participant-authorized.
  ...
  Canonical participant cards remain private-DM, monotonic and repairable with truthful remaining time, contribution output and bounded journal.
  ...
  proof/presenter maxima are `20,518` state, `1,523` terminal-card fixture and `46` GroupCombat action callback; across 42 deterministic reachable 3×6 rosters, the rolling production journal worst case measured `64,827` with 10 active plus 6 retained expired effects and `709` bytes of headroom after adding the compact timed-effect projection and explicit single-target enemy names, while initial 3×6 state/card measured `15,777`/`536`. The 24-case
...
- `0.4.2` is the first production-capable GroupCombat consumer, but its entry
  ...
  `0.4.3` activates the exhaustive twenty-id typed immediate consumable catalog through `item-use-v5` without a catalog-specific rollout flag. GroupCombat item menus page five useful rows at a time. Turn duels remain item-free and take-away purchases stay in `0.4.12`.
  `0.4.4` is merged. `0.4.5` plus PR #193 add independent User-level guild identity and replay-safe activation Chronicles behind `GUILD_FOUNDATION_ENABLED`; guilds reuse ordinary parties and own no combat state. `0.4.6` is merged at PR #195 and adds independent User-level Referral Foundation behind two default-off flags; it owns no guild/party/combat state. PR #196 merged `0.4.7` universal terminal battle statistics, richer Daily Korchma rewards and atomic Forge dismantling; its deployment and Manual Telegram QA remain unproven. `0.4.8` owns the receipt-based guild weekly goal, non-spendable Glory, bounded guild-only Book and ability-parity audit; it owns no individual ranking or `0.4.15` guild XP/cosmetics.
...
- `docs/tasks/0.4.8-guild-weekly-goal.md` and `docs/qa/0.4.8-guild-weekly-goal-qa.md` — current release contract and pending Manual Telegram QA; `docs/design/group-combat-ability-parity.md` records the cross-mode audit.
- `docs/tasks/0.4.4-bugfix-polish.md` — previous merged GroupCombat polish release contract; deployment and manual-QA evidence remain separate.
...
- Do not stop or refresh that bot during ordinary implementation. At a manual checkpoint, tell the user to run `refresh-local-bot.cmd`; build/typecheck/tests run in the main checkout, and never kill all Node processes.
