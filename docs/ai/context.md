...
- `0.4.2` is the first production-capable GroupCombat consumer, but its entry
  ...
  `0.4.4` is merged. `0.4.5` plus PR #193 add independent User-level guild identity and replay-safe activation Chronicles behind `GUILD_FOUNDATION_ENABLED`; guilds reuse ordinary parties and own no combat state. `0.4.6` adds independent User-level Referral Foundation behind two default-off flags; it owns no guild/party/combat state. Guild and referral manual Telegram QA, migration deployment and target enablement remain unproven. `0.4.7` owns universal read-only terminal battle statistics, richer Daily Korchma rewards whose LUCK affects only the bounded identity-branch chance rather than canonical candidate weights, and atomic Forge dismantling. Guild weekly work shifts to `0.4.8`; the accepted forward sequence continues through Guild Cosmetic Progression at `0.4.15`.
