...
- `0.1.21` reuses the short-turn idea for solo/training fights without adding a new scheduler or schema column: active `CombatState` stores `turnExpiresAt`, new turns set it to about `23` seconds, and battle-surface restore/callback paths lazily commit a canonical basic attack when the deadline has passed. Safe side-surface redirects after the deadline do not auto-attack for the hero; they commit a missed-turn skip where the monster still acts and the next turn opens. Long `expires_at` remains a session cleanup boundary, not the per-turn timer.
- `0.1.21` also stores persistent monster-fight context and bark state inside `CombatState`: `context` freezes the `Europe/Kyiv` world snapshot and applied trait effects at start, while `barks` stores deterministic line selection/emission state and turn summaries store `monsterBarkId`. No schema migration is required; parser compatibility must keep old rows readable and preserve the new optional fields when present.
