...
- `v1:tavern:raid` створює lightweight pending action через годинний `CharacterCooldown` key з prefix `tavern.friday-barrel-raid.pending` і period id `YYYY-MM-DDTHH:23`, з випадковим завершенням через 5–8 хвилин, а не одразу видає reward. У `0.0.16` period id явно є server-clock bucket-ом у `UTC`, не київським локальним wall-time і не user-facing timestamp-ом.
- Новий raid period відкривається на 23-й хвилині кожної години за серверним корчемним часом. З 04:00 до 08:22:59 за цим самим часом нові старти повертають audit-break copy про переоблік; уже pending рейди все ще можуть завершитись. О 08:23 рейд знову доступний у поточному period bucket, а далі лічильник перемикається за звичайним правилом 23-ї хвилини.
