...
`0.1.18` adds `♟️ Покрокова дуель` on top of the duel ledger. It persists `DuelChallenge.mode`, adds one-to-one `duel_combat_sessions`, append-only `duel_combat_actions`, active combat leases, optimistic turn versioning and a durable 23-second timeout poller. Turn-based duel actions use the shared pure combat primitive extracted from `src/domain/combat`, while Telegram notifications remain best-effort after committed DB state.
...
- `0.1.18` ships the first durable per-turn deadline for turn-based duels: around `23` seconds, persisted on the session row and consumed by expected turn/version. The timeout path applies a deterministic basic attack for the silent actor, renders cards best-effort and preserves idempotency. Ordinary monster fights and `/spar` can reuse the model later; long session expiry can remain a cleanup fallback, but it should not be the player-facing wait model for a stuck turn.
