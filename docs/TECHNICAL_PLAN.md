...
- `0.1.18` ships the first durable per-round deadline for turn-based duels: around `23` seconds, persisted on the session row and consumed by expected turn/version. The timeout path applies deterministic basic attacks for missing choices, reveals the resolved round, renders cards best-effort and preserves idempotency. Ordinary monster fights and `/spar` can reuse the model later; long session expiry can remain a cleanup fallback, but it should not be the player-facing wait model for a stuck turn.
