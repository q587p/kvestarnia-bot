...
- `0.1.24` adds a separate Shynok sale path instead of widening level barter. It sells only eligible priced, known, unequipped, unreserved and unprotected content stacks for a fixed basket-level 42% payout. It does not add item instances, buyback, trading, player markets or sell-equipped overrides.
