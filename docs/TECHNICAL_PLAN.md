...
`0.1.6` extends the problem quest wrapper without a schema migration. Stage issue/reward state uses existing `daily_actions` rows with `local_date = once`; stage progress counts won ordinary `solo_combat_sessions` with `created_at > issue.created_at`, excluding the training doppelganger monster id. The first stage is also explicitly issued from Shynok and keeps the legacy reward key `quest.thirteen-small-problems` for compatibility; later stages use `quest.problem-chain.*` keys and are issued only after an explicit Shynok action after turn-in.
