...
Для майбутніх ремортів не робити другий прохід до 13 рівня таким самим швидким, як перший. Орієнтир із MUD-досвіду: кожне нове життя може піднімати XP-планку за рівень; у Квестарні flat `+200 XP` за рівень було б забагато, тому перший draft має бути пропорційним. Потенційна формула для remort-adjusted total XP: `base_threshold + round(remort_count * 1000 * ((level - 1) / 12)^2)`. Для першого реморту це дає приблизно `2300 total XP` до 13 рівня, із помітнішою вагою після 9 рівня. Це planning note, не runtime у `0.1.2`.
