...
Для `Низ` passage-втручань `monster.level` у цій формулі означає ефективний рівень після вибору проходу: правий прохід після `0.1.19` падає на `-5` effective levels із safe clamp і платить скромніше, прямий лишає базову ставку, лівий піднімає ризик і винагороду. Easy/right reward fractions округлюються вниз; normal/hard лишають звичайне округлення. Антифарм XP перевіряє окремо збережений `baseMonsterLevel` до втручання: якщо базовий монстр уже був надто слабким для героя, лишаються старі стиснуті bands `3 XP` / `2 XP`; якщо розрив зʼявився тільки через легший правий прохід, це не farming.
