...
`0.2.6` Passage Search is a tiny optional side roll, not a primary XP path. Each node has a `13` minute start cooldown; `Спуск до Низу` uses a `23` second safe search, while passage cards use a `42` second risky search tied to the frozen passage monster preview. During the short monster-rest window, a passage can instead start a safe 42-second search with no encounter token and a small passage-tier floor (`left > straight > right`). Search grants no XP, quest progress, buffs, titles or hidden odds preview. Safe descent finds at most tiny gold or one low-power bandage chance; passage searches scale only small gold/bandage odds by passage tier and LUCK. Danger on a risky passage replaces the reward with the stored monster attack, so risk never pays and attacks on the same completion.
