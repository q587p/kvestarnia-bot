...
Після реморту другий прохід до 13 рівня не має бути таким самим швидким, як перший. Орієнтир із MUD-досвіду: кожне нове життя може піднімати XP-планку за рівень; у Квестарні flat `+200 XP` за рівень було б забагато, тому `0.1.2` використовує пропорційну добавку до total XP: `base_threshold + round(remort_count * 1000 * ((level - 1) / 12)^2)`. Для першого реморту це дає приблизно `2300 total XP` до 13 рівня, із помітнішою вагою після 9 рівня. Це runtime-правило першого prestige slice, а не фінальний баланс.
