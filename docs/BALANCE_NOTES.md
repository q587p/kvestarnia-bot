...
`0.2.13` Postal Manatka Delivery is a convenience gold sink, not a player-to-player gold transfer or market price. A package accepts only `1..5` distinct eligible stack types and `1..93` units per type. The delivery fee is charged from the sender only on recipient acceptance, in the same transaction as item movement, using the tested formula `13 + 3 * selected type count + selected unit count`. If the sender lacks that gold at acceptance time, no item moves and no fee is charged.
