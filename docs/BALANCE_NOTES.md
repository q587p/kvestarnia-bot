...
`0.2.7` Player Abilities keeps class/race power bounded inside existing PvE combat. Race abilities are identity buttons with modest damage or small hero-only support fallback; they use their own cooldowns and should not outpace class skills. Refreshed class AoE skills trade lower per-target damage for hitting every living enemy once, while support-ready ally scopes degrade to the acting hero until party runtime exists. Ability costs currently stay in `0..5` mana and `1..4` own-action cooldowns; broader balance tuning should use simulator/playtest results rather than hidden per-class exceptions. Race/class abilities do not alter rewards, Yeger progress, loot, monster selection, PvP, remort math or economy.
