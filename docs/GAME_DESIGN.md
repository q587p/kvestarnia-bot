...
- Після натискання `🍺 У рейд на бочку` герой має входити в pending raid на випадкові 1–3 хвилини, як і каже шинкар.
...
- Таверна показує короткий блок `За столами`, щоб відповідати на питання «хто ще зараз швендяє Квестарнею?» без Telegram-стеження.
- Тавернова бочка й перша шаурма мають кнопку `👥 Учасники`, яка ділить учасників на `🟢 Активні` та `🟡 Притихли`.
...
Поточна реалізація scene-based: немає повної location/session/raid system, тому присутність прив’язана до останньої відомої місцини й стабільних ids маленьких сцен.
...
## Команди MVP
Поточна command surface у `0.0.9`:
- `/start`
- `/hero`, `/profile`, `/me`
- `/tavern`, `/raid`
- `/adventure`, `/quest`
