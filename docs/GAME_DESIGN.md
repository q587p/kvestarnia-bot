...
- `location.korchma.hall` — Зала корчми, головний hub із переходами до столу зі справами, шинку, бочки, Єгерського кутка, дошки вістей, підвалу й надвір.
...
- Hub показує актуальні starter actions: `🌯 Підозріла шаурма`, `⚔️ Сутичка з підозрілим монстром`, `🧹 Підвальна справа` або старші справи, якщо герой уже виріс.
...
- Підвальна справа відкривається з 2 рівня і лишається новачковою на рівнях 2-3. До 2 рівня locked-рядок живе в архіві без кнопки `🧹 У підвал`; з 4 рівня основний список показує `Справа не до миші`, а кнопка `🧹 У підвал` веде в старшу підвальну справу. Старі мишачі callback-и не видають reward і не переносять presence у підвал після gate.
...
- Відкриття hub позначає героя у `location.korchma.quest_table`; вхід до Шинку позначає `location.korchma.bar`; вхід до підвалу позначає `location.korchma.cellar`; вихід до зали очищає stale adventure/raid ids.
- Майбутній борг: `Тринадцять дрібних проблем` і підвальні справи мають здаватися корчмарю в `location.korchma.bar`, а не віддалено біля Столу зі справами чи з підвалу. Після здачі перших `13` проблем у Шинку Корчмар може видати наступну аналогічну справу на `42` проблеми з більшою нагородою; це має бути окремий continuation quest, а не автоматичне розтягування першого списку.
