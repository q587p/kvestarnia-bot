...
Цей пакет описує малий корчемний group hook для Квестарні без зміни runtime-коду.
...
- Груповий рейд природно підхоплює ту ж логіку, але переносить її в соціяльний простір.
- Мала групова сцена краще відповідає Telegram, ніж великий MMO-рейд із десятком екранів.

## Later raid MVP shape

Майбутній груповий hook має виглядати так:
...
- Не прив’язувати equipment shell або starter gear до рейду: поточне спорядження не має змінювати доступність, шкоду або нагороди групового hook-а.
...
- Equipment shell і starter gear не мають змінювати HP, mana, damage, reward tables або доступність рейду.
...
Для першого raid-релізу кращий варіант — не карати гравця, а дати зрозумілу, коротку відмову.
...
- `location.korchma.hall` може лишатися place id для старту соціяльного hook-а.
...
XP: +12
Золото: +7
Манатка: Квиток мокрого пригодника
```
...
## Соціяльні actions backlog

Після Phase 2 duel/invite primitives природно йти в ширші соціяльні actions. Частина з них може прийти до рейдів, якщо не потребує великого party combat state machine.
...
- Соціяльні action cooldowns.

Ці елементи краще описати окремо в `docs/backlog/social-actions-backlog.md`, щоб не змішувати їх із session design.

## Future Codex prompt draft

### Codex implementation prompt — First Group Barrel Hook
