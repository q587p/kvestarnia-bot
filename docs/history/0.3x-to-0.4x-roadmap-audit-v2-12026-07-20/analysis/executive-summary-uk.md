...
## Що не є blocker, але мусить мати адресу

Rogue reputation, ширший Hunt/Єгер і колекції не варто вставляти перед `0.4.0`.
Але Старий жертовник, greeting buff, їжа, carried consumables та public resale /
Korchmar recycling були помилково згорнуті в один deferred-булет без задач і
промптів. Виправлена черга лишає `0.4.0`–`0.4.4` за мультибоями/ґільдіями, а
дає цим старим promises явні `0.4.5`–`0.4.11` slices. Cosmetic guild progression
перенесено на data-gated `0.4.12`.

Поточний 42% продаж манаток Корчмарю вже shipped; deferred є лише resale shelf
та recycling. Food status і carried consumable — різні моделі й різні задачі.
Item instances для bounded first slices не потрібні.
