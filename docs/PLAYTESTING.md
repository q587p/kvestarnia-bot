...
16. Level-up має бути видимим без ручного ремонту старих персонажів. На `🏅 Пропамʼятна дошка` блок `Реморти Тринадцятки` має показувати всі відомі реморти, а кнопки `Реморт 1`, `Реморт 2` тощо мають відкривати перші зарубки рівнів саме для вибраного реморту, якщо для нього є дані.
