...
19. У `Ярус I: Сутерени Корчми` перевір лівий/прямий/правий проходи: правий має нижчий рівень і скромнішу винагороду з округленням униз, прямий нормальну, лівий значно вищий рівень і щедрішу за центр.
...
4. Очікування: обидва учасники отримують battle card із двома іменами, HP/маною, номером ходу, поточним ходом і коротким нагадуванням про 23 секунди на старті.
5. На ході натисни `⚔️ Атакувати`.
6. Очікування: хід просувається рівно один раз, обидві картки оновлюються best-effort, persistent HP/мана персонажа не списуються.
7. Натисни стару кнопку того самого ходу або кнопку неактивного гравця.
8. Очікування: бот показує поточний канонічний стан і не проводить другий хід.
9. Натисни класову дію, коли вона доступна, і перевір, що витрата мани/cooldown збігається з бойовим правилом `/fight`.
10. Дай одному ходу прострочитися або перезапусти бота й відкрий дуель після дедлайну.
11. Очікування: прострочений хід ідемпотентно стає звичайною атакою мовчазного учасника, без дубля при повторному відкритті.
12. Під час активної покрокової дуелі натисни `Квести`, `Корчма` або інший пригодовий маршрут.
13. Очікування: combat lock повертає до канонічної картки дуелі; самі duel turn callback-и не блокуються.
14. Натисни `🏳️ Здатися`.
15. Очікування: дуель завершується, результат replay-safe, active lock знято, `🔁 Реванш` створює покроковий реванш.
