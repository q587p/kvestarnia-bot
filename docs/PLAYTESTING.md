...
3a. With a higher-level/lucky `class.ranger`, repeat several crafts and verify a successful savings roll can preserve `1-5` ordinary bandages without allowing the craft below the up-front `8` / `13` ordinary-bandage requirement; with a non-ranger class, verify the same recipes spend the fixed cost.
