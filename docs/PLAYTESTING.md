...
4. Очікування: видно preview, який прямо каже, що це не `/restart`: рівень, XP, золото, активні бої й екіпірування скинуться, але історія ремортів і частина безпечної памʼяті збережуться.
5. Зміни звертання, расу або клас через inline-кнопки.
6. Очікування: preview оновлюється без reset-а персонажа.
7. Обери до 5 eligible манаток.
8. Очікування: екіпіровані, priced power/effect, protected, story і безцінні манатки не зʼявляються як selectable; protected/story речі не мають губитися під час reset-а.
9. Натисни confirm.
10. Очікування: персонаж стає 1 рівня з 0 XP/золота, новою анкетою, очищеним екіпіруванням, закритими active solo fights і малим capped memory bonus.
...
13. Відкрий `/hero`.
14. Очікування: видно кількість ремортів і capped memory rank, якщо він більший за нуль.
...
- `/remort` — після 13 рівня відкриває explicit prestige reset із preview, capped memory bonus і без прихованого wipe.
