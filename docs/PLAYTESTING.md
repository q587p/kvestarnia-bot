...
16. На 13-й перемозі у старшому бою очікування: показується одноразова нагорода `+35 XP`, `+10 золота` й `Жетон тринадцяти дрібних проблем`; повторне натискання не дублює винагороду.
17. На герої 7-8 рівня старший бій має брати найближчого доступного звичайного монстра, а не випадкову дрібноту 1-2 рівня; якщо контентного рівня ще бракує, XP за сильно слабшого монстра має бути `1`.
...
8. Очікування: якщо активного старшого бою немає, стартує бій проти ordinary non-boss монстра з тегом `undead`, `ghost`, `cursed` або `unquiet`; якщо активний бій уже є, бот повертає його без створення другого.
9. Виграй цільовий старший бій і повернись до `/hunt`.
10. Очікування: прогрес росте тільки за перемоги після старту справи; lost/fled/expired і wrong-tag монстри не рахуються.
...
2. Очікування: Підвальна справа locked до 2 рівня, в активному списку її немає, action-кнопка `🧹 У підвал` не показується, presence не переходить у підвал.
3. Відкрий `📦 Архів` і перевір, що locked-рядок підвальної справи видно там.
4. На персонажі 2-3 рівня, коли денна шаурма й сутичка вже витрачені, відкрий `/quest` або `🗺️ Квести` усередині корчми.
5. Обери «Підвальну справу».
...
10. На персонажі 4+ рівня `/cellar` або quest-hub підвал має відкрити `Справа не до миші`, позначити presence у `Підвал корчми` і не показувати старі мишачі action-кнопки.
...
15. Після отримання пляшки перевір, що підвальний екран веде до `🍻 Шинку`, а не показує `🍾 Здати Корчмарю` чи `🎒 Лишити собі` в підвалі.
...
21. Повторний confirm callback має показати той самий результат і не додавати ще одну output-манатку.
...
26. Manual confirm має зʼїсти саме вибрані 5 units, створити рівно 1 output, а повторний confirm має показати той самий результат без нового списання.
