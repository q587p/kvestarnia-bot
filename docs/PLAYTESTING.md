...
4. Перевір, що бот показує три підходи:
   ...
   - хитрий/класовий;
   - ризиковий.
5. Обери одну справу й перевір, що замість старої лінійки `safe/flair/risky` видно 3-4 авторські методи під сцену, расу, клас або титул.
6. Обери метод і перевір результат: текст короткий, метод згадано, нагорода ідемпотентна, exact шансів і майбутньої винагороди не показано до натискання.
7. Якщо метод має малу золоту ціну, перевір персонажа з достатнім золотом і без нього: без золота справа не зараховується й гаманець не змінюється.
8. Повторне натискання старої кнопки в тому самому 93-хвилинному вікні не дублює XP, золото або манатки.
9. Старий `safe/flair/risky` callback з історії має показати stale-refresh/current offer, а не тихо обрати новий метод.
10. У локальному режимі виклич `/dev_adventure_reset` і перевір, що `/adventure` показує інші три справи в цьому самому вікні, а старі кнопки стають застарілими.
11. За активної persistent-сутички `/adventure` має показати safety-стан і не відкривати другу пригоду.
...
18. На герої 7-8 рівня старший бій має брати найближчого доступного звичайного монстра, а не випадкову дрібноту 1-2 рівня; якщо контентного рівня ще бракує, XP/золото мають лишатися малими через рівень самого монстра, а не через окремий штраф за рівень героя.
19. У `Ярус I: Сутерени Корчми` перевір лівий/прямий/правий проходи: правий має брати доступного монстра приблизно на 3-5 рівнів нижче й давати скромніший XP з округленням униз, прямий лишає baseline XP, лівий має вищий ризик і XP вище прямого, а золото після перемоги може коливатися від нуля до рівня героя; нульове золото має помітно частіше компенсуватися манаткою.
20. Під час активного бою відкрий `🎒 Манатки`, `/inventory` і detail/equipment callback-и з торби.
21. Очікування: манатки відкриваються як side surface, а не показують combat-lock redirect; пригодові маршрути на кшталт `Квести` чи `Корчма` все ще повертають до бою.
...
9. Натисни стару кнопку того самого раунду або повторний вибір після запису.
10. Очікування: бот показує поточний канонічний стан і не проводить другий раунд.
11. Дай одному раунду прострочитися або перезапусти бота й відкрий дуель після дедлайну.
12. Очікування: пропущений вибір ідемпотентно стає звичайною атакою мовчазного учасника, без дубля при повторному відкритті.
13. Під час активної покрокової дуелі натисни `Квести`, `Корчма` або інший пригодовий маршрут.
14. Очікування: combat lock повертає до канонічної картки дуелі; самі duel turn callback-и не блокуються.
15. Натисни `🏳️ Здатися`.
16. Очікування: дуель завершується, результат replay-safe, active lock знято, результат показує малий XP для обох учасників, `🔁 Реванш` створює покроковий реванш.
...
5. Обери «Льохову справу» й перевір, що кнопки можуть включати сценовий, race/class/signature і малий paid-метод.
...
11. `/cellar` працює як secondary fallback command, а не як окремий великий activity engine.
12. На персонажі 4+ рівня `/cellar` або quest-hub льох має відкрити `Справа не до миші`, позначити presence у `Льох корчми` і не показувати старі мишачі action-кнопки.
13. Якщо золота вистачає, натисни `🧀 Купити пломбу`.
14. Очікування: золото списується один раз, `Сирна пломба Корчмаря` зʼявляється один раз, повторний callback не списує золото вдруге.
15. Натисни `🐭 Домовитись без пломби` на іншому тестовому персонажі або зі штучним deterministic roll.
16. Очікування: success видає `Пляшка Пінного Міражу` максимум один раз, failure ставить cooldown для повторної roleplay-спроби, але route через пломбу лишається доступним.
17. Після отримання пляшки перевір, що льоховий екран веде до `🍻 Шинку`, а не показує `🍾 Здати Корчмарю` чи `🎒 Лишити собі` в льосі.
